...
本指南演示了如何为部署在 LangSmith Deployments 中的 AI 智能体应用程序实现一个全面的 CI/CD（持续集成/持续部署）管道。在此示例中，您将使用开源的 [LangGraph](/oss/langgraph/overview) 框架来编排和构建智能体，并使用 [LangSmith](/langsmith/home) 进行可观测性和评估。此管道基于 [cicd-pipeline-example 仓库](https://github.com/langchain-ai/cicd-pipeline-example)。
...
有关特定的测试方法，请参阅 [LangGraph 测试文档](/oss/langgraph/test)；有关离线评估的全面概述，请参阅 [评估方法指南](/langsmith/evaluation-approaches)。

### 先决条件

在设置 CI/CD 管道之前，请确保您具备以下条件：

- <Icon icon="robot" /> 一个 AI 智能体应用程序（在此示例中是使用 [LangGraph](/oss/langgraph/overview) 构建的）
