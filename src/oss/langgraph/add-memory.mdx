...
AI 应用需要[内存](/oss/concepts/memory)来在多次交互中共享上下文。在 LangGraph 中，你可以添加两种类型的内存：

* 将[短期内存](#add-short-term-memory)作为代理[状态](/oss/langgraph/graph-api#state)的一部分，以支持多轮对话。
* 添加[长期内存](#add-long-term-memory)以跨会话存储特定于用户或应用层的数据。

## 添加短期内存

**短期**内存（线程级别[持久化](/oss/langgraph/persistence)）使代理能够跟踪多轮对话。要添加短期内存，请执行以下操作：
...
:::python
<Accordion title="示例：使用 [MongoDB](https://pypi.org/project/langgraph-checkpoint-mongodb/) 检查点">
  ```
  ...
  ```
    ...
    <Note>
    **设置**
    要使用 MongoDB 检查点，你需要一个 MongoDB 集群。如果你还没有，请遵循[此指南](https://www.mongodb.com/docs/guides/atlas/cluster/)来创建一个集群。
    </Note>
...
如果你的图包含[子图](/oss/langgraph/use-subgraphs)，你只需要在编译父图时提供 checkpointer。LangGraph 将自动将 checkpointer 传播给子图。
...
如果你希望子图拥有自己的内存，你可以用适当的 checkpointer 选项编译它。这在[多代理](/oss/langchain/multi-agent)系统中很有用，如果你希望代理能够跟踪它们内部的消息历史记录。
...
使用长期内存来存储特定于用户或应用程序的数据，并跨对话会话持久化。
...
在图的内存存储中启用语义搜索，以便图代理可以通过语义相似性搜索存储中的项。
...
启用了[短期内存](#add-short-term-memory)后，长对话可能会超出 LLM 的上下文窗口。常见解决方案包括：

* [修剪消息](#trim-messages)：在调用 LLM 之前移除前 N 条或后 N 条消息
* 从 LangGraph 状态中永久[删除消息](#delete-messages)
* 对历史消息进行[摘要](#summarize-messages)替换，用摘要代替早期消息
...
:::python
大多数 LLM 都有一个最大的支持上下文窗口（以 token 为单位）。决定何时截断消息的一种方法是计算消息历史中的 token 数量，并在接近该限制时进行截断。如果你使用的是 LangChain，你可以使用 `trim_messages` 工具，并指定要保留的 token 数量，以及处理边界时使用的`strategy`（例如 `keep the last max_tokens`）。
:::
:::js
大多数 LLM 都有一个最大的支持上下文窗口（以 token 为单位）。决定何时截断消息的一种方法是计算消息历史中的 token 数量，并在接近该限制时进行截断。如果你使用的是 LangChain，你可以使用 `trimMessages` 工具，并指定要保留的 token 数量，以及处理边界时使用的`strategy`（例如 `keep the last maxTokens`）。
:::
...
你可以从图状态中删除消息以管理消息历史记录。当你想要移除特定消息或清除整个消息历史记录时，这很有用。

:::python
要从图状态中删除消息，你可以使用 `RemoveMessage`。为了使 `RemoveMessage` 生效，你需要使用带有 @[`add_messages`] [Reducer](/oss/langgraph/graph-api#reducers) 的状态键，例如 [`MessagesState`](/oss/langgraph/graph-api#messagesstate)。
...
:::js
要从图状态中删除消息，你可以使用 `RemoveMessage`。为了使 `RemoveMessage` 生效，你需要使用带有 @[`messagesStateReducer`][messagesStateReducer] [Reducer](/oss/langgraph/graph-api#reducers) 的状态键，例如 `MessagesZodState`。
...
<Warning>
删除消息时，**请确保**生成的消息历史记录有效。请检查你所使用的 LLM 供应商的限制。例如：

* 有些供应商期望消息历史以 `user` 消息开始
* 大多数供应商要求带工具有调用的 `assistant` 消息后紧跟相应的 `tool` 结果消息。
</Warning>
...
截断或删除消息（如上所示）的问题在于，您可能会丢失因修剪消息队列而丢失的信息。因此，一些应用程序会通过使用聊天模型来更复杂地处理消息历史记录，即进行总结。

![](/oss/images/summary.png)


:::python
可以使用提示和编排逻辑来总结消息历史记录。例如，在 LangGraph 中，你可以扩展 [`MessagesState`](/oss/langgraph/graph-api#working-with-messages-in-graph-state) 以包含一个 `summary` 键：
...
然后，你可以生成聊天历史记录的摘要，使用任何现有摘要作为下一个摘要的上下文。可以在 `messages` 状态键中积累了一定数量的消息后调用此 `summarize_conversation` 节点。
...
:::js
可以使用提示和编排逻辑来总结消息历史记录。例如，在 LangGraph 中，你可以在状态中包含一个 `summary` 键，与 `messages` 键保持一致：
...
然后，你可以生成聊天历史记录的摘要，使用任何现有摘要作为下一个摘要的上下文。可以在 `messages` 状态键中积累了一定数量的消息后调用此 `summarizeConversation` 节点。
...
```
  ...
  // 我们将在 `messages` 键（MessagesZodState 已经拥有的）之外添加一个 `summary` 属性
  ...
  const workflow = new StateGraph(GraphState)
    ...
    .addConditionalEdges(
      ...
      // 这意味着在调用 `conversation` 节点之后采取这些边。
      "conversation",
      // 其次，我们传入将决定下一个调用哪个节点的函数。
      shouldContinue,
    )
    // 我们现在从 `summarize_conversation` 到 END 添加一条正常边。
    // 这意味着在调用 `summarize_conversation` 之后结束。
...
**LangMem** 是一个由 LangChain 维护的库，它提供了管理代理长期内存的工具。请参阅[LangMem 文档](https://langchain-ai.github.io/langmem/)以获取使用示例。
:::

## 数据库管理

如果你正在使用任何基于数据库的持久化实现（如 Postgres 或 Redis）来存储短期和/或长期内存，你需要先运行迁移来设置所需的 schema，然后才能将其与数据库一起使用。

按照惯例，大多数特定于数据库的库会在 checkpointer 或 store 实例上定义一个 `setup()` 方法，该方法会运行所需的迁移。然而，你应该查阅你所使用的 @[`BaseCheckpointSaver`] 或 @[`BaseStore`] 的特定实现，以确认确切的方法名称和用法。

我们建议将迁移作为专用的部署步骤运行，或者你可以在服务器启动时确保它们被运行。