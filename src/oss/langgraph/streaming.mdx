...
:::python
将一个或多个以下流式处理模式作为列表传递给 @[`stream`][CompiledStateGraph.stream] 或 @[`astream`][CompiledStateGraph.astream] 方法：
:::

:::js
将一个或多个以下流式处理模式作为列表传递给 @[`stream`][CompiledStateGraph.stream] 方法：
:::
...
:::python
LangGraph 图公开了 @[`stream`][Pregel.stream]（同步）和 @[`astream`][Pregel.astream]（异步）方法，它们以迭代器的形式生成流式输出。
...
:::js
LangGraph 图公开了 @[`stream`][Pregel.stream] 方法，该方法以迭代器的形式生成流式输出。
...
<Accordion title="扩展示例：流式处理更新">
  ...
  for await (const chunk of await graph.stream(
    { topic: "ice cream" },
    // 设置 streamMode: "updates" 以便在每个节点运行后仅流式传输图状态的更新
    // 还可以使用其他流式处理模式。有关详细信息，请参阅支持的流式处理模式
    { streamMode: "updates" }
  )) {
    console.log(chunk);
...
:::js
可以将数组作为 `streamMode` 参数传递，以同时流式传输多种模式的数据。
...
:::python
要在流式输出中包含来自[子图](/oss/langgraph/use-subgraphs)的输出，可以在父图的 `.stream()` 方法中设置 `subgraphs=True`。这将流式传输来自父图和任何子图的输出。
...
当使用 Python < 3.11 且代码包含异步操作时，您必须明确地将 @[`RunnableConfig`] 传递给 `ainvoke()` 以启用正确的流式处理。请参阅[使用 Python < 3.11 进行异步处理](#async)了解详细信息，或升级到 Python 3.11+。
</Warning>
...
// "messages" 流式处理模式返回一个元组迭代器 [messageChunk, metadata]
...
const graph = // ... a graph that uses these LLMs
...
:::js
// "messages" 流式处理模式返回一个 [messageChunk, metadata] 元组
// 其中 messageChunk 是 LLM 流式传输的 Token，metadata 是一个字典
// 包含 LLM 被调用的图节点以及其他信息
for await (const [msg, metadata] of await graph.stream(
  inputs,
...
2. 在调用 `.stream()` 时将 `streamMode: "custom"` 设置为在流中获取自定义数据。您可以组合多种模式（例如，`["updates", "custom"]`），但至少有一种必须是 `"custom"`。
...
:::js
您可以使用 `streamMode: "custom"` 从**任何 LLM API** 流式传输数据——即使该 API **不**实现 LangChain 聊天模型接口也是如此。
