...
LangGraph 实现了一个流式传输（streaming）系统，用于实时展示更新。流式传输对于增强基于 LLM 构建的应用程序的响应能力至关重要。通过逐步显示输出，即使在完整响应准备好之前，流式传输也能显著改善用户体验（UX），尤其是在处理 LLM 的延迟时。

使用 LangGraph 流式传输可以实现以下功能：
...
* <Icon icon="square-binary" size={16} /> [**流式传输 LLM Token**](#messages) — 从图中的任何位置捕获 Token 流：节点内部、子图或工具中。
* <Icon icon="table" size={16} /> [**流式传输自定义数据**](#stream-custom-data) — 直接从工具函数发送自定义更新或进度信号。
* <Icon icon="layer-plus" size={16} /> [**使用多种流式传输模式**](#stream-multiple-modes) — 可在 `values`（完整状态）、`updates`（状态增量）、`messages`（LLM Token + 元数据）、`custom`（任意用户数据）或 `debug`（详细跟踪）中选择。

## 支持的流式传输模式

:::python
将一个或多个以下流式传输模式作为列表传递给 @[`stream`][CompiledStateGraph.stream] 或 @[`astream`][CompiledStateGraph.astream] 方法：
:::

:::js
将一个或多个以下流式传输模式作为列表传递给 @[`stream`][CompiledStateGraph.stream] 方法：
:::

| 模式 | 描述 |
| :--- | :--- |
| `values` | 在图的每一步之后，流式传输状态的完整值。 |
| `updates` | 在图的每一步之后，流式传输状态的更新（增量）。如果在同一步中进行了多次更新（例如，运行了多个节点），这些更新将分别流式传输。 |
...
:::python
LangGraph 图公开 @[`stream`][Pregel.stream]（同步）和 @[`astream`][Pregel.astream]（异步）方法，以生成流式传输的输出来作为迭代器。
:::
...
:::js
LangGraph 图公开 @[`stream`][Pregel.stream] 方法，以生成流式传输的输出来作为迭代器。
:::
...
<Accordion title="扩展示例：流式传输更新">
  ...
  # stream() 方法返回一个迭代器，用于生成流式传输的输出
  for chunk in graph.stream(  # [!code highlight]
      {"topic": "ice cream"},
      # 设置 stream_mode="updates" 以仅在每个节点运行后流式传输图状态的更新
...
```
  ...
  for await (const chunk of await graph.stream(
    { topic: "ice cream" },
    // 设置 streamMode: "updates" 以仅在每个节点运行后流式传输图状态的更新
    // 其他流式传输模式也可用。有关详细信息，请参阅支持的流式传输模式
    { streamMode: "updates" }
  )) {
    console.log(chunk);
...
:::python
您可以在 `stream_mode` 参数中传递一个列表，以一次流式传输多种模式。
...
:::js
您可以在 `streamMode` 参数中传递一个数组，以一次流式传输多种模式。
...
使用 `updates` 和 `values` 流式传输模式，可以在图执行过程中流式传输图的状态。

* `updates` 流式传输图每一步后的状态**更新**。
* `values` 流式传输图每一步后的状态**完整值**。
...
<Tabs>
    <Tab title="updates">
    使用此选项仅流式传输节点每一步返回的**状态更新**。流式传输的输出包括节点名称以及更新内容。
...
:::python
要将 [子图](/oss/langgraph/use-subgraphs) 的输出包含在流式传输的输出中，可以在父图的 `.stream()` 方法中设置 `subgraphs=True`。这将流式传输父图和任何子图的输出。

输出将作为 `(namespace, data)` 元组流式传输，其中 `namespace` 是一个包含子图被调用的节点路径的元组，例如 `("parent_node:<task_id>", "child_node:<task_id>")`。
...
使用 `debug` 流式传输模式，可以流式传输图执行期间尽可能多的信息。流式传输的输出包括节点名称以及完整状态。
...
使用 `messages` 流式传输模式，可以从图的任何部分（包括节点、工具、子图或任务）**逐 Token** 流式传输大型语言模型（LLM）的输出。

:::python
[`messages` 模式](#supported-stream-modes) 的流式传输输出是一个 `(message_chunk, metadata)` 元组，其中：

* `message_chunk`：来自 LLM 的 Token 或消息片段。
* `metadata`：一个字典，包含有关图节点和 LLM 调用的详细信息。

> 如果您的 LLM 未作为 LangChain 集成提供，您可以使用 `custom` 模式流式传输其输出。有关详细信息，请参阅[与任何 LLM 配合使用](#use-with-any-llm)。

<Warning>
**Python < 3.11 中异步需要手动配置**
当在 Python < 3.11 上使用异步代码时，您必须明确地将 @[`RunnableConfig`][RunnableConfig] 传递给 `ainvoke()` 以启用正确的流式传输。请参阅 [异步与 Python < 3.11](#async) 获取详细信息或升级到 Python 3.11+。
</Warning>
...
def call_model(state: MyState):
    """调用 LLM，就某个主题生成笑话"""
    # 即使是使用 .invoke 而不是 .stream 运行 LLM 时也会发出消息事件
...
# "messages" 流式传输模式返回一个 (message_chunk, metadata) 元组的迭代器
# 其中 message_chunk 是 LLM 流式传输的 Token，metadata 是一个字典
# 包含 LLM 被调用的图节点信息和其他信息
...
const callModel = async (state: z.infer<typeof MyState>) => {
  // 调用 LLM，就某个主题生成笑话
  // 注意：即使是使用 .invoke 而不是 .stream 运行 LLM 时也会发出消息事件
...
// "messages" 流式传输模式返回一个 [messageChunk, metadata] 元组的迭代器
...
您可以为 LLM 调用关联 `tags` (标签) 以按 LLM 调用筛选流式传输的 Token。
...
# metadata 包含 LLM 调用信息，包括标签
async for msg, metadata in graph.astream(
    {"topic": "cats"},
    stream_mode="messages",  # [!code highlight]
):
    # 按 metadata 中的 tags 字段筛选流式传输的 Token，仅包含
...
<Accordion title="扩展示例：按标签筛选">
  ...
  async def call_model(state, config):
        topic = state["topic"]
        print("Writing joke...")
        # 注意：为 python < 3.11 明确传递 config 是必需的
        # 因为上下文变量支持在之前才添加到那里：https://docs.python.org/3/library/asyncio-task.html#creating-tasks
        # 明确传递 config 是为了确保正确传播上下文变量
        # 这对于在异步代码中使用 Python < 3.11 是必需的。请参阅异步部分了解更多详情
...
要仅从特定节点流式传输 Token，请使用 `stream_mode="messages"` 并按流式传输元数据中的 `langgraph_node` 字段筛选输出：

:::python
```python
# "messages" 流式传输模式返回一个 (message_chunk, metadata) 元组
# 其中 message_chunk 是 LLM 流式传输的 Token，metadata 是一个字典
# 包含 LLM 被调用的图节点信息和其他信息
for msg, metadata in graph.stream(
    ...
    # 按 metadata 中的 langgraph_node 字段筛选流式传输的 Token
    # 以仅包含来自指定节点的 Token
...
:::js
// "messages" 流式传输模式返回一个 [messageChunk, metadata] 元组
// 其中 messageChunk 是 LLM 流式传输的 Token，metadata 是一个字典
// 包含 LLM 被调用的图节点信息和其他信息
for await (const [msg, metadata] of await graph.stream(
  inputs,
...
2. 调用 `.stream()` 或 `.astream()` 时设置 `stream_mode="custom"` 以在流中获取自定义数据。您可以组合多种模式（例如，`["updates", "custom"]`），但至少要有一个是 `"custom"`。

<Warning>
**Python < 3.11 的异步中没有 @[`get_stream_writer`]**
在 Python < 3.11 上运行的异步代码中，@[`get_stream_writer`] 将无法工作。
相反，向您的节点或工具添加一个 `writer` 参数，并手动传递它。
...
1. 使用 `LangGraphRunnableConfig` 中的 `writer` 参数来发出自定义数据。
2. 调用 `.stream()` 时设置 `streamMode: "custom"` 以在流中获取自定义数据。您可以组合多种模式（例如，`["updates", "custom"]`），但至少要有一个是 `"custom"`。
...
<Tabs>
    ...
    const queryDatabase = tool(
      ...
      },
      {
        ...
        description: "查询数据库。",
        schema: z.object({
          query: z.string().describe("要执行的查询。"),
        }),
      }
    );
...
:::python
您可以使用 `stream_mode="custom"` 从**任何 LLM API** 流式传输数据——即使该 API **不**实现 LangChain chat model 接口也是如此。

这使得您可以集成提供自身流式传输界面的原始 LLM 客户端或外部服务，使 LangGraph 能够为自定义设置提供高度灵活性。
...
:::js
您可以使用 `streamMode: "custom"` 从**任何 LLM API** 流式传输数据——即使该 API **不**实现 LangChain chat model 接口也是如此。
          ...
          [
              {
                  "role": "user",
                  "content": (
                      "你能告诉我我在以下地点可能找到什么物品吗: '{place}'。 "
                      "请列出至少 3 件这样的物品，用逗号分隔。 "
                      "并包含每件物品的简短描述。"
                  ),
              }
          ],
...
如果您的应用程序混合了支持流式传输和不支持流式传输的模型，您可能需要为不支持流式传输的模型明确禁用流式传输。
...
这限制了 LangGraph 自动传播上下文的能力，并在两个关键方面影响 LangGraph 的流式传输机制：

1. 您**必须**明确地将 [`RunnableConfig`][RunnableConfig] 传递给异步 LLM 调用（例如 `ainvoke()`），因为回调不会自动传播。
2. 您在异步节点或工具中**不能**使用 @[`get_stream_writer`]——您必须直接传递一个 `writer` 参数。

<Accordion title="扩展示例：带有手动配置的异步 LLM 调用">
  ```python
  from typing import TypedDict
  from langgraph.graph import START, StateGraph
  from langchain.chat_models import init_chat_model
  ...
  # 接受 config 作为异步节点函数的一个参数
  async def call_model(state, config):
      topic = state["topic"]
      print("Generating joke...")
      # 将 config 传递给 model.ainvoke() 以确保正确传播上下文
...
<Accordion title="扩展示例：带有流式写入器的异步自定义流式传输">
  ```python
  from typing import TypedDict
