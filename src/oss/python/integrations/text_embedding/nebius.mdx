...
[Nebius AI Studio](https://studio.nebius.ai/) 通过统一的接口提供对高质量嵌入模型（embedding models）的 API 访问。Nebius 嵌入模型可以将文本转换成数值向量，这些向量捕获了文本的语义信息，因此可用于语义搜索、聚类和推荐等各种应用场景。

## 概览 (Overview)

`NebiusEmbeddings` 类通过 LangChain 提供了对 Nebius AI Studio 嵌入模型的访问。这些嵌入可用于语义搜索、文档相似性匹配以及其他需要文本向量表示的自然语言处理（NLP）任务。

### 集成详情 (Integration details)

- **提供商 (Provider)**: Nebius AI Studio
- **模型类型 (Model Types)**: 文本嵌入模型
- **主要用例 (Primary Use Case)**: 生成文本的向量表示，用于语义相似度和检索
- **可用模型 (Available Models)**: 各种嵌入模型，包括 BAAI/bge-en-icl 等
- **维度 (Dimensions)**: 因模型而异（通常为 1024-4096 维）

## 设置 (Setup)

### 安装 (Installation)

可以通过 pip 安装 Nebius 集成包：
...
Nebius 需要一个 API 密钥。该密钥可以作为初始化参数 `api_key` 传入，或者设置为环境变量 `NEBIUS_API_KEY`。您可以通过在 [Nebius AI Studio](https://studio.nebius.ai/) 创建账户来获取 API 密钥。
...
`NebiusEmbeddings` 类可以携带可选参数（如 API 密钥和模型名称）进行实例化：
...
embeddings = NebiusEmbeddings(
    # api_key="YOUR_API_KEY",  # 也可以直接传入 API 密钥
...
支持的模型列表可以在 [studio.nebius.com/?modality=embedding](https://studio.nebius.com/?modality=embedding) 查看。

## 索引和检索 (Indexing and Retrieval)

嵌入模型通常用于检索增强生成 (RAG) 流程中，用于数据的索引构建和后续的检索。以下示例展示了如何将 `NebiusEmbeddings` 与向量存储一起用于文档检索。
...
对于轻量级应用，您也可以使用 `InMemoryVectorStore`：
...
您可以直接使用 `NebiusEmbeddings` 类来生成文本的嵌入，而无需使用向量存储。

### 嵌入单个文本 (Embedding a Single Text)
...
### 文档相似性示例 (Document Similarity Example)
