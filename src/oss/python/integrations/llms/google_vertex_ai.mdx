...
<Danger>
    **已弃用**
    ...
    此集成已被弃用，并将在未来的版本中移除。请改用 [`GoogleGenerativeAI`](/oss/integrations/llms/google_generative_ai) 代替。有关完整的 [发布说明和迁移指南](https://github.com/langchain-ai/langchain-google/discussions/1422)，请参阅。
</Danger>

<Warning>
**您当前位于记录 Google Vertex 文本补全模型用法的页面。Google 的许多模型都是 [聊天补全模型](/oss/langchain/models)。**

您可能在寻找 [这个页面](/oss/integrations/chat/google_vertex_ai/)。
</Warning>

**注意：** 这 [Vertex AI Generative API](https://cloud.google.com/vertex-ai/docs/generative-ai/learn/overview) 与 `Google Generative AI` 集成的区别在于，它通过 `Google Cloud` 暴露了该 API。

VertexAI 暴露了 Google Cloud 中所有可用的基础模型。

如需获取可用模型的完整且最新的列表，请访问 [VertexAI 文档](https://cloud.google.com/vertex-ai/generative-ai/docs/models)

## 设置

默认情况下，Google Cloud 不会使用客户数据来训练其作为 Google Cloud AI/ML 隐私承诺一部分的基础模型。有关 Google 如何处理数据的更多详细信息，也可以在 [Google 的客户数据处理附录 (CDPA)](https://cloud.google.com/terms/data-processing-addendum) 中找到。

要使用 `Vertex AI Generative AI`，您必须安装 `langchain-google-vertexai` Python 包，并且还需要：

- 为您的环境配置凭据（gcloud、工作负载身份等...）
- 将指向服务账号 JSON 文件的路径存储在 `GOOGLE_APPLICATION_CREDENTIALS` 环境变量中

此代码库使用 `google.auth` 库，该库首先查找上述应用程序凭据变量，然后查找系统级的身份验证。
...
我们可以使用 `generate` 方法来获取除文本补全之外的额外元数据，例如 [安全属性](https://cloud.google.com/vertex-ai/docs/generative-ai/learn/responsible-ai#safety_attribute_confidence_scoring)。
...
### 可选操作：管理 [安全属性](https://cloud.google.com/vertex-ai/docs/generative-ai/learn/responsible-ai#safety_attribute_confidence_scoring)

- 如果您的用例要求您管理安全属性的阈值，您可以使用以下代码片段进行操作

>注意：我们建议在调整安全属性阈值时要极其谨慎
...
```python
# 您也可以将 safety\_settings 传递给 generate 方法
...
您可以使用不同的基础模型来专攻不同的任务。
有关可用模型的更新列表，请访问 [VertexAI 文档](https://cloud.google.com/vertex-ai/docs/generative-ai/model-reference/overview)
...
<p>
<img src="data:image/png;base64," />
</p>
...
请注意，您也可以指向完整的 GCS 路径（以 `gs://` 开头）而不是本地路径，从而使用存储在 GCS 中的图像。

您还可以将先前的聊天历史记录传递给 LLM：
...
请注意，您也可以将存储在 GCS 中的图像（只需将 `url` 指向以 `gs://` 开头的完整 GCS 路径，而不是本地路径）传递给模型。
...
请注意，使用 Gemini 模型时，您也可以传递视频（只需将 `url` 指向完整的 GCS 路径，以 `gs://` 开头，而不是本地文件）。

## Vertex 模型库 (Vertex Model Garden)

Vertex 模型库 [公开了](https://cloud.google.com/vertex-ai/docs/start/explore-models) 可在 Vertex AI 上部署和服务的开源模型。

数百个流行的 [开源模型](https://cloud.google.com/vertex-ai/generative-ai/docs/model-garden/explore-models#oss-models)，如 Llama、Falcon 等，可用于 [一键部署](https://cloud.google.com/vertex-ai/generative-ai/docs/deploy/overview)

如果您成功部署了来自 Vertex 模型库的模型，您可以在控制台或通过 API 中找到相应的 Vertex AI [端点](https://cloud.google.com/vertex-ai/docs/general/deployment#what_happens_when_you_deploy_a_model)。
...
> Llama 是 Meta 开发的一系列开源权重模型，您可以在 Vertex AI 上对其进行微调和部署。Llama 模型是预训练和微调的生成文本模型。您可以在 Vertex AI 上部署 Llama 2 和 Llama 3 模型。
[Llama 在 Vertex 模型库上的官方文档](https://cloud.google.com/vertex-ai/generative-ai/docs/open-models/use-llama) 了解更多信息

要在 Vertex 模型库上使用 Llama，您必须首先 [将其部署到 Vertex AI 端点](https://cloud.google.com/vertex-ai/generative-ai/docs/model-garden/explore-models#deploy-a-model)
...
> Falcon 是由 [Falcon](https://falconllm.tii.ae/) 开发的一系列开源权重模型，您可以在 Vertex AI 上对其进行微调和部署。Falcon 模型是预训练和微调的生成文本模型。

要在 Vertex 模型库上使用 Falcon，您必须首先 [将其部署到 Vertex AI 端点](https://cloud.google.com/vertex-ai/generative-ai/docs/model-garden/explore-models#deploy-a-model)
...
### Vertex AI 模型库上的 Gemma

> [Gemma](https://ai.google.dev/gemma) 是一组轻量级的、生成式人工智能 (AI) 开源模型。Gemma 模型可用于在您的应用程序和硬件、移动设备或托管服务上运行。您也可以使用微调技术自定义这些模型，使其在执行对您和用户重要的任务时表现出色。Gemma 模型基于 [Gemini](https://cloud.google.com/vertex-ai/generative-ai/docs/multimodal/overview) 模型，旨在供 AI 开发社区扩展和进一步发展。

要在 Vertex 模型库上使用 Gemma，您必须首先 [将其部署到 Vertex AI 端点](https://cloud.google.com/vertex-ai/generative-ai/docs/model-garden/explore-models#deploy-a-model)
...
```python
# TODO：添加 "YOUR PROJECT"、"YOUR REGION" 和 "YOUR ENDPOINT_ID"
...
## 在 Vertex AI 上的 Anthropic

> Vertex AI 上的 [Anthropic Claude 3](https://cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-claude) 模型通过 API 提供完全托管和无服务器的模型。要在 Vertex AI 上使用 Claude 模型，请直接向 Vertex AI API 端点发送请求。由于 Anthropic Claude 3 模型使用托管 API，因此无需预配或管理基础设施。
...
我们不建议使用名称中不包含以 @ 符号开头的后缀的 Anthropic Claude 3 模型版本（claude-3-opus、claude-3-sonnet 或 claude-3-haiku）。
