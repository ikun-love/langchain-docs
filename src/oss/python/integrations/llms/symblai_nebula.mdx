...
[Nebula](https://symbl.ai/nebula/) 是由 [Symbl.ai](https://symbl.ai) 构建的大型语言模型（LLM）。它经过训练，旨在对人类对话执行生成任务。Nebula 擅长建模对话的细微细节并在对话上执行任务。

Nebula 文档：[docs.symbl.ai/docs/nebula-llm](https://docs.symbl.ai/docs/nebula-llm)

此示例将介绍如何使用 LangChain 与 [Nebula 平台](https://docs.symbl.ai/docs/nebula-llm)进行交互。

请确保您已准备好 API 密钥。如果您还没有，请[申请一个](https://info.symbl.ai/Nebula_Private_Beta.html)。
...
使用对话记录（Transcript）和指令来构建提示（Prompt）。
