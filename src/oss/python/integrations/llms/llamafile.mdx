...
[Llamafile](https://github.com/Mozilla-Ocho/llamafile) 允许用户通过**单个文件**的方式来分发和运行大型语言模型（LLM）。

Llamafile 通过将 [llama.cpp](https://github.com/ggerganov/llama.cpp) 与 [Cosmopolitan Libc](https://github.com/jart/cosmopolitan) 结合到一个统一的框架中来实现这一点，该框架将 LLM 的所有复杂性都整合到一个名为 "llamafile" 的**单文件可执行程序**中。这个文件可以直接在大多数计算机上本地运行，**无需进行任何安装**。

## 设置 (Setup)

1. 下载您想要使用的模型的 llamafile 文件。您可以在 [HuggingFace](https://huggingface.co/models?other=llamafile) 上找到许多 Llamafile 格式的模型。在本指南中，我们将下载一个较小的模型：`TinyLlama-1.1B-Chat-v1.0.Q5_K_M`。注意：如果您没有安装 `wget` 命令，可以直接通过这个 [链接](https://huggingface.co/jartine/TinyLlama-1.1B-Chat-v1.0-GGUF/resolve/main/TinyLlama-1.1B-Chat-v1.0.Q5_K_M.llamafile?download=true) 下载模型。

```bash
wget https://huggingface.co/jartine/TinyLlama-1.1B-Chat-v1.0-GGUF/resolve/main/TinyLlama-1.1B-Chat-v1.0.Q5_K_M.llamafile
```

2. 使 llamafile 变为可执行文件。首先，打开一个终端（如果您尚未打开）。**如果您使用的是 MacOS、Linux 或 BSD 系统，**您需要使用 `chmod` 命令为该新文件授予执行权限（见下文）。**如果您使用的是 Windows 系统，**请将文件名重命名，并在末尾添加 ".exe"（模型文件应命名为 `TinyLlama-1.1B-Chat-v1.0.Q5_K_M.llamafile.exe`）。

```bash
chmod +x TinyLlama-1.1B-Chat-v1.0.Q5_K_M.llamafile  # 如果您使用的是 MacOS、Linux 或 BSD，则运行此命令
```
...
现在，您可以向 llamafile 的 REST API 发送调用请求了。默认情况下，llamafile 服务器监听在 [localhost:8080](http://localhost:8080) 地址。完整的服务器文档可以在 [此处](https://github.com/Mozilla-Ocho/llamafile/blob/main/llama.cpp/server/README.md#api-endpoints) 找到。您可以直接通过 REST API 与 llamafile 交互，但在这里我们将展示如何使用 LangChain 与其进行交互。
...
要流式传输（Stream）Token，请使用 `.stream(...)` 方法：
