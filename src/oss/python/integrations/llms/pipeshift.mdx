...
这将帮助您开始使用 LangChain 的 Pipeshift 完成模型（LLM）。有关 `Pipeshift` 功能和配置选项的详细文档，请参阅 [API 参考](https://dashboard.pipeshift.com/docs)。
...
| 类 (Class) | 包 (Package) | 
本地化 (Local) | 可序列化 (Serializable) | [JS 支持](https://js.langchain.com/docs/integrations/llms/pipeshift) | 下载量 (Downloads) | 版本 (Version) |
| :--- | :--- | :---: | :---: |  :---: | :---: | :---: |
...
要访问 Pipeshift 模型，您需要创建一个 Pipeshift 帐户、获取一个 API 密钥，并安装 `langchain-pipeshift` 集成包。
...
如果您希望对模型调用进行自动化的跟踪，您可以通过取消注释以下内容来设置您的 [LangSmith](https://docs.smith.langchain.com/) API 密钥：
...
有关所有 `Pipeshift` 功能和配置的详细文档，请访问 API 参考：[dashboard.pipeshift.com/docs](https://dashboard.pipeshift.com/docs)