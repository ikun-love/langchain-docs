...
<Warning>
**您当前正在查看记录 Google 模型作为文本补全模型用法的页面。许多流行的 Google 模型是 [聊天补全模型](/oss/langchain/models)。**

您可能想查看 [此页面](/oss/integrations/chat/google_generative_ai/)。
</Warning>

一份关于如何将 [Google Generative AI](https://developers.generativeai.google/) 模型与 LangChain 结合使用的指南。注意：这与 Google Cloud Vertex AI [集成](/oss/integrations/llms/google_vertex_ai) 是分开的。
