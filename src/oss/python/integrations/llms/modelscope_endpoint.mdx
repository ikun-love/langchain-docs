...
ModelScope (ModelScope：[主页](https://www.modelscope.cn/) | [GitHub](https://github.com/modelscope/modelscope)) 是基于“模型即服务”（Model-as-a-Service, MaaS）理念构建的平台。它致力于汇集 AI 社区中最先进的机器学习模型，并简化在实际应用中利用这些 AI 模型的过程。本仓库中开源的核心 ModelScope 库提供了接口和实现，允许开发者执行模型的推理、训练和评估。这将帮助您使用 LangChain 入门 ModelScope 的完成模型（LLMs）。

## 概述

### 集成详情

| 供应商 (Provider) | 类 (Class) | 包 (Package) | 本地 (Local) | 可序列化 (Serializable) | 下载量 (Downloads) | 版本 (Version) |
| :--- | :--- | :---: | :---: |  :---: | :---: | :---: |
...
要访问 ModelScope 模型，您需要创建一个 ModelScope 账户，获取一个 SDK 令牌（Token），并安装 `langchain-modelscope-integration` 集成包。

### 凭证 (Credentials)

请前往 [ModelScope](https://modelscope.cn/) 注册 ModelScope 账户并生成一个 [SDK 令牌](https://modelscope.cn/my/myaccesstoken)。完成此操作后，设置环境变量 `MODELSCOPE_SDK_TOKEN`：
