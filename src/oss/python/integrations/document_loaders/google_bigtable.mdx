...
本指南（Notebook）将介绍如何使用 [Bigtable](https://cloud.google.com/bigtable) 通过 `BigtableLoader` 和 `BigtableSaver` 来 [保存、加载和删除 LangChain 文档](/oss/integrations/document_loaders)。
...
默认情况下，@[`Document`] 对象的 `metadata` 映射将包含一个键：`rowkey`，其值为行的 rowkey 值。要向该映射添加更多项，请使用 `metadata_mapping`。
