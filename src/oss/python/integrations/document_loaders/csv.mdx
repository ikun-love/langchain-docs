...
>一个[逗号分隔值 (CSV)](https://en.wikipedia.org/wiki/Comma-separated_values) 文件是使用逗号分隔值的文本文件。文件的每一行都是一条数据记录。每条记录包含一个或多个字段，字段之间用逗号分隔。

使用单行文档加载 [CSV](https://en.wikipedia.org/wiki/Comma-separated_values) 数据。
...
有关支持哪些 csv 参数的更多信息，请参阅 [csv 模块](https://docs.python.org/3/library/csv.html) 文档。
...
使用 `source_column` 参数，可以为从 CSV 中每行创建的文档指定一个源。否则，`file_path` 将被用作所有从 CSV 文件创建的文档的源。

当使用从 CSV 文件加载的文档进行回答链（chains that answer questions）时，这非常有用。
...
您也可以使用 `UnstructuredCSVLoader` 加载表格。使用 `UnstructuredCSVLoader` 的一个优点是，如果您在 `"elements"` 模式下使用它，表的 HTML 表示形式将在元数据中可用。
