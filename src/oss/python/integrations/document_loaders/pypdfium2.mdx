...
本指南快速概述了如何开始使用 `PyPDF` [文档加载器](https://python.langchain.com/docs/concepts/document_loaders)。有关所有 DocumentLoader 功能和配置的详细文档，请访问 [API 参考](https://python.langchain.com/api_reference/community/document_loaders/langchain_community.document_loaders.pdf.PyPDFium2Loader.html)。
...
额外的元数据特定于每个解析器。
这些信息（例如用于对 PDF 进行分类）可能会很有帮助。

## 分割模式与自定义页面分隔符

加载 PDF 文件时，您可以通过两种不同的方式将其分割：

- 按页分割 (By page)
- 作为单一文本流 (As a single text flow)

默认情况下，PyPDFLoader 会将 PDF 分割成一个单一的文本流。
...
在此模式下，PDF 按页分割，生成的 Documents 元数据包含页码。但在某些情况下，我们可能希望将 PDF 作为单个文本流处理（这样就不会将某些段落截断）。在这种情况下，您可以使用 *single* 模式：
...
从逻辑上讲，在此模式下，`page_number` 元数据会消失。以下是关于如何在*单一*模式下的文本流中清晰识别页面结束位置的方法：

### 在 *single* 模式下添加自定义 *pages_delimiter* 以识别页面结束位置
...
这可以简单地是 `\n`，或者是 `\f` 来明确指示页面换页，或者使用 `\<!-- PAGE BREAK -->`，以便在 Markdown 查看器中注入而不会产生视觉效果。
...
许多文档加载器都涉及解析文件。这些加载器之间的区别通常在于解析文件的方式，而不是加载文件的方式。例如，您可以使用 `open` 来读取 PDF 或 Markdown 文件的二进制内容，但需要不同的解析逻辑才能将该二进制数据转换为文本。

因此，将解析逻辑与加载逻辑分离会很有帮助，这样可以更容易地重复使用给定的解析器，而与数据是如何加载的无关。
...
有关 `PyPDFium2Loader` 所有功能和配置的详细文档，请访问 API 参考：[python.langchain.com/api_reference/community/document_loaders/langchain_community.document_loaders.pdf.PyPDFium2Loader.html](https://python.langchain.com/api_reference/community/document_loaders/langchain_community.document_loaders.pdf.PyPDFium2Loader.html)
```