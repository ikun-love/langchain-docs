...
本指南快速概述了 `PDFMiner` [文档加载器](https://python.langchain.com/docs/concepts/document_loaders) 的入门用法。如需了解所有 __ModuleName__Loader 功能和配置的详细文档，请参阅 [API 参考](https://python.langchain.com/api_reference/community/document_loaders/langchain_community.document_loaders.pdf.PDFMinerLoader.html)。
...
使用 `PDFMinerLoader` 不需要任何凭证。

如果你想为你的模型调用获得自动化的最佳跟踪效果，你也可以通过取消注释以下内容来设置你的 [LangSmith](https://docs.langchain.com/langsmith/home) API 密钥：
...
## 分页模式（Splitting mode）和自定义页面分隔符（custom pages delimiter）

加载 PDF 文件时，你可以通过两种不同的方式将其拆分：

- 按页（By page）
- 作为单个文本流（As a single text flow）

默认情况下，`PDFMinerLoader` 会按页拆分 PDF。
...
在此模式下，PDF 按页拆分，生成的 Documents 元数据包含页码。但在某些情况下，我们可能希望将 PDF 作为单个文本流来处理（这样就不会将某些段落切断）。在这种情况下，你可以使用 *single* 模式：
...
### 在 *single* 模式下添加自定义的 *pages\_delimiter* 以识别页面结束的位置
...
这可以简单地是 `\n`，或者 `\f` 以清晰地指示页面更改，或者 `\<!-- PAGE BREAK -->` 以无视觉效果地注入 Markdown 查看器中。

## 从 PDF 中提取图像

你可以选择以下三种不同解决方案从 PDF 中提取图像：

- rapidOCR（轻量级光学字符识别工具）
- Tesseract（高精度 OCR 工具）
- 多模态语言模型（Multimodal language model）

你可以调整这些函数以选择提取图像的输出格式，选项包括 *html*、*markdown* 或 *text*。

结果将插入在页面文本的倒数第二个和倒数第一个段落之间。
...
许多文档加载器都涉及解析文件。这些加载器之间的主要区别在于解析逻辑，而不是加载文件的方式。例如，你可以使用 `open` 来读取 PDF 或 Markdown 文件的二进制内容，但需要不同的解析逻辑才能将该二进制数据转换为文本。

因此，将解析逻辑与加载逻辑分离会有所帮助，这样可以更轻松地重用给定的解析器，而无需考虑数据是如何加载的。
...
这有助于将文本按语义分块为各个部分，因为输出的 HTML 内容可以被 `BeautifulSoup` 解析，以获取关于字体大小、页码、PDF 标题/页脚等更结构化和丰富的信息。
...
for c in content:
    ...
    if fs == cur_fs:
        ...
        cur_text = c.text
snippets.append((cur_text, cur_fs))
# 注意：上述逻辑非常直观。人们还可以添加更多策略，例如删除重复的片段（因为 PDF 中的页眉/页脚会出现在多页上，所以如果我们发现重复项，可以认为它们是冗余信息）
```
...
for s in snippets:
    ...
    # 如果当前片段的字体大小 <= 前一个部分的 =内容 => 内容属于同一部分（如果需要，也可以为子部分创建树状结构，但这可能需要更多思考，并且可能依赖于数据）
    if (
    ...
    # 如果当前片段的字体大小 > 前一个部分的内容，但小于前一个部分的标题，也创建一个新部分（例如，PDF 标题将具有最大的字体大小，但我们不希望它吞并所有部分）
    metadata = {"heading": s[0], "content_font": 0, "heading_font": s[1]}
    metadata.update(docs[0].metadata)
    semantic_snippets.append(Document(page_content="", metadata=metadata))
    cur_idx += 1
...
如需了解 `PDFMinerLoader` 所有功能和配置的详细文档，请参阅 API 参考：[python.langchain.com/api_reference/community/document_loaders/langchain_community.document_loaders.pdf.PDFMinerLoader.html](https://python.langchain.com/api_reference/community/document_loaders/langchain_community.document_loaders.pdf.PDFMinerLoader.html)
