...
>[Spreedly](https://docs.spreedly.com/) 是一项服务，它允许您安全地存储信用卡信息，并利用这些信息针对任意数量的支付网关和第三方 API 执行交易。它通过同时提供卡标记化/安全存储服务（vault service）以及网关和接收器集成服务来实现这一目标。Spreedly 标记化的支付方式存储在 `Spreedly` 中，这使得您可以独立存储卡信息，然后根据您的业务需求将该卡信息传递给不同的终端点。

本 Notebook 介绍如何将数据从 [Spreedly REST API](https://docs.spreedly.com/reference/api/v1/) 加载到可被 LangChain 摄取（ingest）的格式中，并提供向量化（vectorization）的示例用法。

注意：本 Notebook 假设已安装以下软件包：`openai`, `chromadb`, 和 `tiktoken`。
...
Spreedly API 需要一个访问令牌（access token），您可以在 Spreedly 管理控制台（Admin Console）中找到它。

此文档加载器目前不支持分页（pagination），也不支持需要额外参数的更复杂的对象访问。它还需要一个 `resource` 选项来定义您想要加载的对象类型。
