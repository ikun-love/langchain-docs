...
[Docling](https://github.com/DS4SD/docling) 将 PDF、DOCX、PPTX、HTML 等格式解析为一个丰富的统一表示形式，包括文档布局、表格等，使其为 RAG 等生成式 AI 工作流程做好准备。

此集成通过 `DoclingLoader` 文档加载器提供了 Docling 的能力。
...
- 利用 Docling 丰富的格式进行高级的、面向文档原生的知识锚定（grounding）。

`DoclingLoader` 支持两种不同的导出模式：

- `ExportType.DOC_CHUNKS`（默认）：如果您希望将每个输入文档分块，然后捕获每个单独的块作为下游独立的 LangChain Document，或者
- `ExportType.MARKDOWN`：如果您希望将每个输入文档捕获为独立的 LangChain Document。

此示例通过参数 `EXPORT_TYPE` 允许探索这两种模式；根据设置的值，示例管道会相应地进行设置。
...
> 提示：为获得最佳转换速度，请尽可能使用 GPU 加速；例如，如果在 Colab 上运行，请使用支持 GPU 的运行时环境。
...
- `chunker` (可选): 要使用的任何特定的 Docling 分块器实例（用于文档块模式）
...
> 注意：在此案例中，可以忽略显示 `"Token indices sequence length is longer than the specified maximum sequence length..."` 的消息——更多详情请见 [此处](https://github.com/DS4SD/docling-core/issues/119#issuecomment-2577418826)。
...
> - 以下示例管道使用 HuggingFace 的推理 API；如需增加 LLM 配额，可通过环境变量 `HF_TOKEN` 提供 token。
> - 此管道的依赖项可以按如下方式安装（`--no-warn-conflicts` 针对 Colab 的预填充 Python 环境；如需严格使用，可自由移除）：
...
milvus_uri = str(Path(mkdtemp()) / "docling.db")  # 或根据需要设置
...
请注意，这些来源包含丰富的知识锚定信息，包括段落标题（即章节）、页码和精确的边界框。
