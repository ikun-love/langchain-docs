...
[Docling](https://github.com/DS4SD/docling) 可以解析 PDF、DOCX、PPTX、HTML 等多种格式，并将其转换为丰富的统一表示形式（包括文档布局、表格等），使其能够直接用于生成式 AI 工作流，例如 RAG（检索增强生成）。
...
- 利用 Docling 丰富的格式（针对文档原生的）进行高级应用。

`DoclingLoader` 支持两种不同的导出模式：

- `ExportType.DOC_CHUNKS` (默认)：如果您希望将每个输入文档进行分块（Chunking），并**随后将每个单独的块捕获为下游 LangChain Document**；或者
- `ExportType.MARKDOWN`：如果您希望将每个输入文档捕获为**单独的 LangChain Document**。

此示例通过参数 `EXPORT_TYPE` 演示了这两种模式；根据设置的值，示例处理管道会相应地进行配置。
...
> 为了获得最佳转换速度，请尽可能利用 GPU 加速；例如，如果在 Colab 上运行，请使用启用 GPU 的运行时环境。
...
- `file_path`: 源路径，可以是单个字符串（URL 或本地文件）或这些路径的可迭代对象。
- `converter` (可选): 要使用的任何特定的 Docling 转换器实例。
- `convert_kwargs` (可选): 用于转换执行的任何特定关键字参数（kwargs）。
- `export_type` (可选): 要使用的导出模式：`ExportType.DOC_CHUNKS` (默认) 或 `ExportType.MARKDOWN`。
- `md_export_kwargs` (可选): 任何特定的 Markdown 导出 kwargs（仅用于 Markdown 模式）。
- `chunker` (可选): 要使用的任何特定的 Docling 分块器实例（用于文档分块模式）。
- `meta_extractor` (可选): 要使用的任何特定的元数据提取器。
...
> 注意：如果出现提示 `"Token indices sequence length is longer than the specified maximum sequence length..."` 的消息，在这种情况下可以忽略——更多详情请见[此处](https://github.com/DS4SD/docling-core/issues/119#issuecomment-2577418826)。
...
> - 以下示例管道使用 HuggingFace 的推理 API；为了增加 LLM 配额，可以通过环境变量 `HF_TOKEN` 提供 token。
> - 该管道所需的依赖项可以如下方所示安装（`--no-warn-conflicts` 选项是针对 Colab 预装 Python 环境的；如果希望更严格地使用，可以删除此选项）：
...
milvus_uri = str(Path(mkdtemp()) / "docling.db")  # 或按需设置
...
请注意，源信息中包含了丰富的接地（grounding）信息，包括段落标题（即章节）、页码和精确的边界框（bounding box）。
