...
通过运行 `p.chromium.launch(headless=True)`，我们启动了一个**无头（headless）**版本的 Chromium 实例。
...
**注意**: 如果您正在使用 Jupyter Notebook，在加载文档之前，可能还需要安装并应用 `nest_asyncio`，如下所示：
