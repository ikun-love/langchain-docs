...
> [Polaris AI DataInsight](https://datainsight.polarisoffice.com/playground) 是一个文档解析器，它能从各种文件格式中提取文档元素（文本、图像、复杂表格、图表等）并将其转换为结构化的 JSON 格式，从而使其易于集成到 RAG（检索增强生成）系统中。
...
loader = PolarisAIDataInsightLoader(
    ...
    mode="page",  # 可选值："element"（元素级）、"page"（页级）或 "single"（单个）。(默认是 "single")
)

docs = loader.load()  # 或者使用 loader.lazy_load()
