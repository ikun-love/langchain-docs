...
Diffbot 的 Extract API 需要一个 API 令牌（Token）。请遵循这些说明 [获取免费的 API 令牌](/oss/integrations/providers/diffbot#installation-and-setup)，然后设置一个环境变量。
...
要继续将数据加载到知识图谱（Knowledge Graph）中，请遵循 [`DiffbotGraphTransformer` 指南](/oss/integrations/graphs/diffbot/#loading-the-data-into-a-knowledge-graph)。
```