...
> [Google BigQuery](https://cloud.google.com/bigquery) 是一个无服务器、高性价比的企业级数据仓库，它能跨云运行，并随着您的数据量进行扩展。
`BigQuery` 是 `Google Cloud Platform`（谷歌云平台）的一部分。

使用每行一个文档的方式加载 `BigQuery` 查询。
...
```python
# 注意，`id` 列被返回了两次，其中一次被重命名（使用别名）为 `source`
ALIASED_QUERY = """
SELECT
  id,
  dna_sequence,
