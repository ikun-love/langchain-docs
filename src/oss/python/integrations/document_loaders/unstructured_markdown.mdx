...
本指南为您概述如何快速入门使用 UnstructuredMarkdown [文档加载器](https://python.langchain.com/docs/concepts/document_loaders)。有关 __ModuleName__Loader 所有功能和配置的详细文档，请参阅 [API 参考](https://python.langchain.com/api_reference/community/document_loaders/langchain_community.document_loaders.markdown.UnstructuredMarkdownLoader.html)。
...
您可以通过两种模式运行加载器："single"（单文档模式）和 "elements"（元素模式）。如果使用 "single" 模式，文档将作为一个单独的 @[`Document`] 对象返回。如果使用 "elements" 模式，`unstructured` 库会将文档拆分成不同的元素，例如 `Title`（标题）和 `NarrativeText`（叙述性文本）。您可以在 `mode` 参数之后传入额外的 `unstructured` 关键字参数（kwargs）来应用不同的 `unstructured` 设置。
...
在此示例中，我们将使用 `elements` 模式进行加载，该模式将返回 Markdown 文档中不同元素的列表：
...
如您所见，从 `example.md` 文件中提取了 29 个元素。正如预期的那样，第一个元素是文档的标题：
