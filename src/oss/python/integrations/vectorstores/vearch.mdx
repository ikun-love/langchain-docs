...
>[Vearch](https://vearch.readthedocs.io) 是用于深度学习和 AI 应用的向量搜索基础设施。

## 设置 (Setting up)

请遵循[这些说明](https://vearch.readthedocs.io/en/latest/quick-start-guide.html#)。
...
# 向量数据通常已经初始化，因此我们不需要 document 参数可以直接创建对象。
...
# 在实际应用中，我们通常会限制相似度的阈值。以下方法可以设置此阈值。
...
```markdown
## 删除和获取文档 (delete and get function need to maintain docids)
## 你的 docid
