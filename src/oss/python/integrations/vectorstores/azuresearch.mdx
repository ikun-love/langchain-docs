...
我们假设您使用的是 @[`OpenAIEmbeddings`]，但如果您使用的是 Azure OpenAI，请改用导入 @[`AzureOpenAIEmbeddings`]。
...
创建 @[`OpenAIEmbeddings`] 和 AzureSearch 类的实例。完成此步骤后，您的 Azure AI 搜索资源上应该有一个空的搜索索引。集成模块提供了一个默认的模式（schema）。
