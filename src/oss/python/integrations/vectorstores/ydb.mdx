...
> [YDB](https://ydb.tech/) 是一个多功能的开源分布式 SQL 数据库，它将高可用性和可扩展性与强一致性及 ACID 事务相结合。它可以同时处理事务性（OLTP）、分析性（OLAP）和流式工作负载。

本 Notebook 展示了如何使用与 `YDB` 向量存储相关的各项功能。
...
本 Notebook 不需要凭证，只需确保已按照上述说明安装了必要的包即可。

如果你希望对模型调用进行一流的自动化跟踪，可以通过取消注释以下代码设置你的 [LangSmith](https://docs.smith.langchain.com/) API 密钥：
...
创建向量存储并添加相关文档后，你很可能希望在链或代理执行过程中查询它。
...
### 转换为检索器 (Query by turning into retriever)

你也可以将向量存储转换为检索器，以便在链中使用更方便。

以下是如何将向量存储转换为检索器，然后使用简单的查询和筛选器调用该检索器：
...
- [代理 RAG](/oss/langgraph/agentic-rag)
