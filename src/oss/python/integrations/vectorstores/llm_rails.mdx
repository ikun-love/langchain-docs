...
>[LLMRails](https://www.llmrails.com/) 是一个用于构建生成式 AI（GenAI）应用的 API 平台。它提供了一个易于使用的 API，用于文档索引和查询。这些功能由 LLMRails 管理，并针对性能和准确性进行了优化。
有关如何使用此 API 的更多信息，请参阅 [LLMRails API 文档](https://docs.llmrails.com/)。

您需要使用 `pip install -qU langchain-community` 安装 `langchain-community` 才能使用此集成。

本 Notebook 展示了如何使用与 `LLMRails` 集成相关的 LangChain 功能。
请注意，与其他同类集成不同，LLMRails 提供了一个端到端的托管服务（managed service）用于检索增强生成（RAG），其中包括：

1. 一种从文档文件中提取文本并将其分块成句子的方法。
2. 自己的嵌入模型和向量存储——每个文本片段都被编码成一个向量嵌入，并存储在 LLMRails 的内部向量存储中。
3. 一个查询服务，可以自动将查询编码成嵌入（embedding），并检索最相关的文本片段（包括对 [混合搜索](https://docs.llmrails.com/datastores/search) 的支持）。

所有这些功能都在此 LangChain 集成中得到支持。

# 设置 (Setup)

要将 LLMRails 与 LangChain 结合使用，您需要拥有一个 LLMRails 账户。要开始使用，请执行以下步骤：

1. 如果您还没有 LLMRails 账户，请 [注册](https://console.llmrails.com/signup)。
2. 接下来，您需要创建 API 密钥来访问 API。在语料库视图（corpus view）中，点击 **"API Keys"** 选项卡，然后点击 **"Create API Key"** 按钮。给您的密钥起一个名字。点击 "Create key"，您现在就拥有了一个有效的 API 密钥。请妥善保管此密钥。

要将 LangChain 与 LLMRails 结合使用，您需要拥有以下两个值：`api_key`。
您可以通过以下两种方式将它们提供给 LangChain：

1. 在您的环境中设置这两个环境变量：`LLM_RAILS_API_KEY` 和 `LLM_RAILS_DATASTORE_ID`。
...
1. 在创建 `LLMRails` 向量存储对象时，将它们作为参数提供：
...
要向您的数据存储中添加文本，您首先需要访问 [数据存储](https://console.llmrails.com/datastores) 页面并创建一个。点击“创建数据存储 (Create Datastore)”按钮，为您的数据存储选择一个名称和嵌入模型。然后从新创建的数据存储设置中获取您的数据存储 ID。
...
## 带有分数的相似性搜索 (Similarity search with score)

有时我们可能希望执行搜索，同时获取相关性分数（relevancy score），以便了解特定结果的好坏程度。
...
## LLMRails 作为检索器 (LLMRails as a Retriever)

LLMRails 与所有其他 LangChain 向量存储一样，最常被用作 LangChain 的检索器（Retriever）：
