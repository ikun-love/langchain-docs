...
>[Lantern](https://github.com/lanterndata/lantern) 是一个开源的向量相似性搜索工具，用于 `Postgres` 数据库。
...
本 Notebook 展示了如何使用 Postgres 向量数据库（`Lantern`）。
...
# 或者你可以通过 `LANTERN_CONNECTION_STRING` 环境变量传递它
```
...
最大边际相关性搜索优化了与查询的相似性 **以及** 所选文档之间的多样性。
...
上面，我们从头创建了一个向量存储。然而，我们通常想要使用一个现有的向量存储。
要做到这一点，我们可以直接初始化它。
...
如果你有一个现有的集合，可以通过设置 `pre_delete_collection=True` 来使用 `from_documents` 来覆盖它。
这将删除集合，然后再重新填充它。
