...
[Couchbase](http://couchbase.com/) 是一款屡获殊荣的分布式 NoSQL 云数据库，它为您的所有云、移动、AI 和边缘计算应用提供了无与伦比的多功能性、性能、可扩展性和财务价值。Couchbase 通过为开发人员提供代码辅助和为应用程序提供向量搜索来拥抱 AI。

向量搜索（Vector Search）是 Couchbase 中[全文搜索服务](https://docs.couchbase.com/server/current/learn/services-and-indexes/services/search-service.html)（Search Service）的一部分。

本教程将解释如何在 Couchbase 中使用向量搜索。您可以结合使用 [Couchbase Capella](https://www.couchbase.com/products/capella/) 和您自管理的 Couchbase Server。
...
如果您希望获得一流的自动模型调用跟踪，您还可以取消注释以下内容来设置您的 [LangSmith](https://docs.langchain.com/langsmith/home) API 密钥：
...
现在，我们将设置要在向量搜索中使用的 Couchbase 集群中的 bucket、scope 和 collection 名称。

对于此示例，我们使用默认的 scope 和 collection。
...
创建向量存储后，我们可以通过添加和删除不同项目与其进行交互。

### 向向量存储中添加项目 (Add items to vector store)
...
一旦创建了向量存储并将相关的文档添加进去，在运行链或代理时很可能会希望对其进行查询。
...
#### 相似性搜索（带分数） (Similarity search with Score)

您还可以通过调用 `similarity_search_with_score` 方法来获取结果的分数。

```python
results = vector_store.similarity_search_with_score("明天会热吗?", k=1)
...
您可以通过在 Couchbase 搜索服务支持的文档的文本或元数据上指定任何过滤器来过滤搜索结果。
...
您可以通过搜索中的 `fields` 参数指定要从文档中返回的字段。这些字段将作为返回 `Document` 中 `metadata` 对象的一部分返回。您可以获取索引中存储的任何字段。文档的 `text_key` 将作为文档的 `page_content` 的一部分返回。

如果您没有指定要获取的字段，则返回索引中存储的所有字段。

如果要获取元数据中的某个字段，您需要使用 `.` 来指定它。
...
filter_on_source = search.MatchQuery("news", field="metadata.source")
retriever.invoke("银行抢劫是一种犯罪", filter=filter_on_source)
```
...
结果将基于向量搜索和搜索服务支持的搜索的结果的组合。每个分量搜索的分数相加，得到结果的总分数。
...
我们均匀地向元数据中添加三个字段：`date` 在 2010 到 2020 之间，`rating` 在 1 到 5 之间，`author` 设置为 John Doe 或 Jane Doe。
...
query = "总统对 Ketanji Brown Jackson 说了些什么"
...
我们可以通过指定模糊度（fuzziness）来搜索部分匹配。这在您想搜索搜索查询的微小变体或拼写错误时非常有用。

这里，"Jae" 与 "Jane" 非常接近（模糊度为 1）。
...
我们可以搜索在 `metadata.date` 这样的日期字段上处于日期范围内的文档。

```python
query = "有提到独立（independence）吗?"
results = vector_store.similarity_search(
    query,
    search_options={
        "query": {
...
我们可以搜索在 `metadata.rating` 这样的数字字段上的范围内（包括边界）的文档。
...
不同的搜索查询可以使用 AND（conjuncts）或 OR（disjuncts）操作符组合。

在此示例中，我们检查评级在 3 到 4 之间且日期在 2015 到 2018 年之间的文档。
...
混合搜索的结果可能包含不满足所有搜索参数的文档。这是由于[评分的计算方式](https://docs.couchbase.com/server/current/search/run-searches.html#scoring)决定的。
分数是向量搜索分数和混合搜索中查询分数之和。如果向量搜索分数很高，则组合分数会高于完全匹配混合搜索中所有查询的结果。
...
在此示例中，我们检查评级在 3 到 5 之间且在文本字段中匹配字符串 "independence" 的文档。
...
类似地，您可以在 `search_options` 参数中使用任何受支持的查询方法，如地理距离 (Geo Distance)、多边形搜索 (Polygon Search)、通配符 (Wildcard)、正则表达式 (Regular Expressions) 等。有关可用查询方法及其语法的更多详细信息，请查阅文档。
...
### 问题：我没有在搜索结果中看到我指定的某些字段

在 Couchbase 中，我们只能返回存储在搜索索引中的字段。请确保您尝试在搜索结果中访问的字段是搜索索引的一部分。

处理此问题的一种方法是在索引中动态索引和存储文档的字段。

- 在 Capella 中，您需要进入“高级模式”(Advanced Mode)，然后在“常规设置”(General Settings) 的下拉菜单下勾选“\[X] 存储动态字段”(Store Dynamic Fields) 或 “\[X] 索引动态字段”(Index Dynamic Fields)。
- 在 Couchbase Server 中，在索引编辑器（非快速编辑器 Quick Editor）的“高级”(Advanced) 下拉菜单下，勾选“\[X] 存储动态字段”(Store Dynamic Fields) 或 “\[X] 索引动态字段”(Index Dynamic Fields)。
...
这很可能是因为文档中的 `metadata` 字段没有被 Couchbase 搜索索引索引和/或存储。为了索引文档中的 `metadata` 字段，您需要将其添加为索引中的子映射（child mapping）。

如果您选择映射索引中的所有字段，您将能够搜索所有元数据字段。或者，为了优化索引，您可以选择 `metadata` 对象内要索引的特定字段。您可以参考[文档](https://docs.couchbase.com/cloud/search/customize-index.html)了解有关索引子映射的更多信息。
...
过滤器是用于限制在搜索索引中搜索的文档的[预过滤](https://docs.couchbase.com/server/current/vector-search/pre-filtering-vector-search.html#about-pre-filtering)（pre-filters）。它在 Couchbase Server 7.6.4 及更高版本中可用。

混合查询是额外的搜索查询，可用于调整从搜索索引返回的结果。

过滤器和混合搜索查询都具有相同的功能，只是语法略有不同。过滤器是[SearchQuery](https://docs.couchbase.com/python-sdk/current/howtos/full-text-searching-with-sdk.html#search-queries) 对象，而混合搜索查询是[字典](https://docs.couchbase.com/server/current/search/search-request-params.html)。
