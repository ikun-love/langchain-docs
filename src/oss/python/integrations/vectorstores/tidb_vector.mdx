...
> [TiDB Cloud](https://www.pingcap.com/tidb-serverless) 是一个全面的数据库即服务（DBaaS）解决方案，提供专用（Dedicated）和无服务器（Serverless）两种选项。TiDB Serverless 目前正在将内置的向量搜索功能集成到 MySQL 生态系统中。有了这项增强功能，您无需引入新的数据库或额外的技术栈，就可以使用 TiDB Serverless 无缝地开发 AI 应用。请在 [pingcap.com/ai](https://pingcap.com/ai) 创建一个免费的 TiDB Serverless 集群，并开始使用向量搜索功能。

本指南将提供一个详细的教程，介绍如何利用 TiDB Vector 功能，展示其特性和实际应用场景。
...
配置您需要的 OpenAI 和 TiDB 主机设置。在此 Notebook 中，我们将遵循 TiDB Cloud 提供的标准连接方法，以建立安全高效的数据库连接。

```python
# 在这里我们使用 import getpass
import getpass
import os

if "OPENAI_API_KEY" not in os.environ:
    os.environ["OPENAI_API_KEY"] = getpass.getpass("OpenAI API Key:")
# 从 TiDB Cloud 控制台复制的模板
...
TiDB 支持余弦距离（cosine）和欧几里得距离（l2），其中 'cosine' 是默认选项。

下面的代码片段在 TiDB 中创建了一个名为 `TABLE_NAME` 的表，该表专为向量搜索进行了优化。成功执行此代码后，您将可以直接在 TiDB 数据库中查看和访问 `TABLE_NAME` 表。
...
执行搜索时使用元数据过滤器，以检索与所应用的过滤器对齐的特定数量的最近邻结果。

## 支持的元数据类型

TiDB Vector Store 中的每个向量都可以与元数据配对，元数据被组织成一个 JSON 对象中的键值对。键是字符串（strings），值可以是以下类型：
...
请注意，元数据过滤器中的每个键值对都被视为一个单独的过滤子句，并且这些子句使用逻辑运算符 AND 进行组合。
...
在 LangChain 中，检索器（Retriever）是一个响应非结构化查询来检索文档的接口，它比向量存储提供了更广泛的功能。以下代码演示了如何将 TiDB Vector 用作检索器。
...
让我们来看一个高级用例场景：一位旅行社正在为客户定制旅行报告，这些客户希望机场具备特定的便利设施，例如干净的休息室和素食选项。这个过程包括：

- 在机场评论中进行语义搜索，以提取满足这些便利条件的机场代码。
- 随后执行 SQL 查询，将这些代码与航线信息连接起来，详细说明符合客户偏好的航空公司和目的地。

首先，让我们准备一些与航空相关的（airpod related，此处可能原意为 airport/flight related）数据：
...
您可以使用 `.drop_vectorstore()` 方法移除 TiDB Vector Store。
