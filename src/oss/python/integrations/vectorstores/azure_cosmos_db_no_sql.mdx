...
此 Notebook 展示了如何利用这个集成的 [向量数据库](https://learn.microsoft.com/en-us/azure/cosmos-db/vector-database) 来存储集合中的文档、创建索引，并使用近似最近邻（ANN）算法（如 COS（余弦距离）、L2（欧几里得距离）和 IP（内积））执行向量搜索查询，以定位与查询向量最接近的文档。

Azure Cosmos DB 是驱动 OpenAI ChatGPT 服务的数据库。它提供了个位数的毫秒响应时间、自动且即时的可伸缩性，以及在任何规模下都能保证的速度。

Azure Cosmos DB for NoSQL 目前提供预览版的向量索引和搜索功能。此功能旨在处理高维向量，从而能够在任何规模下实现高效、准确的向量搜索。现在，您可以直接在文档中与数据一起存储向量。这意味着数据库中的每个文档不仅可以包含传统的无模式（schema-free）数据，还可以包含高维向量作为文档的其他属性。这种数据与向量的共同定位（colocation）实现了高效的索引和搜索，因为向量与它们所代表的数据存储在同一个逻辑单元中。这简化了数据管理、AI 应用架构以及基于向量的操作的效率。
...
[注册](https://azure.microsoft.com/en-us/free/) 以获得终身免费访问权限，立即开始使用。
...
## 创建 AzureCosmosDB NoSQL 向量搜索 (Creating AzureCosmosDB NoSQL Vector Search)
...
# 使用 AzureCosmosDBNoSql 插入文档及其嵌入向量
...
## 带分数的向量搜索 (Vector Search with Score)
...
## 带过滤的向量搜索 (Vector Search with filtering)
...
## 全文搜索 BM 25 排序 (Full Text Search BM 25 Ranking)
...
## 带过滤的混合搜索 (Hybrid Search with filtering)
...
## 带自定义投影的混合搜索 (Hybrid Search with custom projection)
