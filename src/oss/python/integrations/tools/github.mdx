...
请[点击此处](https://docs.github.com/en/apps/creating-github-apps/registering-a-github-app/registering-a-github-app)遵循说明来创建和注册一个 GitHub 应用。请确保您的应用具有以下[仓库权限](https://docs.github.com/en/rest/overview/permissions-required-for-github-apps?apiVersion=2022-11-28)：

* 提交状态 (Commit statuses) (只读)
* 内容 (Contents) (读写)
* Issue (读写)
* 元数据 (Metadata) (只读)
* 拉取请求 (Pull requests) (读写)

应用注册后，您必须授予您的应用权限以访问您希望它操作的每个仓库。请在 [github.com 上的应用设置](https://github.com/settings/installations) 中使用此功能。

#### 3. 设置环境变量

在初始化代理之前，需要设置以下环境变量：

* **GITHUB_APP_ID** - 在应用的常规设置中找到的六位数字
* **GITHUB_APP_PRIVATE_KEY** - 应用的私钥 `.pem` 文件的位置，或者该文件的完整文本内容（作为字符串）。
* **GITHUB_REPOSITORY** - 您的机器人将操作的 GitHub 仓库名称。必须遵循格式 `{username}/{repo-name}`。*请确保应用已首先添加到此仓库！*
* 可选: **GITHUB_BRANCH** - 机器人提交到的分支。默认为 `repo.default_branch`。
* 可选: **GITHUB_BASE_BRANCH** - 仓库的基础分支，拉取请求将基于此分支创建。默认为 `repo.default_branch`。
...
1. **获取 Issue (Get Issues)** - 从仓库中获取 issue 列表。

2. **获取 Issue 详情 (Get Issue)** - 获取特定 issue 的详细信息。

3. **对 Issue 评论 (Comment on Issue)** - 在特定 issue 上发布评论。

4. **创建拉取请求 (Create Pull Request)** - 从机器人的工作分支向基础分支创建拉取请求。
...
* **获取最新发布 (Get Latest Release)** - 从仓库获取最新的发布版本。

* **获取发布列表 (Get Releases)** - 从仓库获取最新的 5 个发布版本。

* **获取指定发布 (Get Release)** - 通过标签名称（例如 `v1.0.0`）从仓库获取特定的发布版本。
...
```python
example_query = "Issue 24888 的标题是什么?"
...
Issue 24888 的标题是 "Standardize KV-Store Docs"。
```
