...
这将帮助您开始使用 SQL 数据库 [工具包](/oss/langchain/tools#toolkits)。有关 `SQLDatabaseToolkit` 所有功能和配置的详细文档，请前往 [API 参考](https://python.langchain.com/api_reference/community/agent_toolkits/langchain_community.agent_toolkits.sql.toolkit.SQLDatabaseToolkit.html)。

`SQLDatabaseToolkit` 中的工具旨在与 `SQL` 数据库进行交互。

一个常见的应用场景是使智能体（Agent）能够使用关系数据库中的数据（可能以迭代方式，例如从错误中恢复）来回答问题。

**⚠️ 安全注意事项 ⚠️**

构建 SQL 数据库的问答系统需要执行模型生成的 SQL 查询。这样做存在固有的风险。请确保您的数据库连接权限始终根据您的链/智能体的需求进行尽可能严格的限制。这将减轻（但不能消除）构建由模型驱动的系统的风险。
...
为了进行演示，我们将从 LangChain [Hub](https://smith.langchain.com/hub) 中获取一个提示词。我们还将需要 `langgraph` 来演示如何将工具包与智能体一起使用。注意：这对于仅使用工具包本身不是必需的。
...
下面，我们使用这些对象来实例化工具包。我们首先创建一个数据库对象。

本指南使用基于 [这些说明](https://database.guide/2-sample-databases-sqlite/) 的示例 `Chinook` 数据库。

下面我们将使用 `requests` 库来拉取 `.sql` 文件并创建一个内存中的 SQLite 数据库。请注意，此方法轻量级，但存储是临时的且不线程安全。如果您愿意，可以按照说明将文件保存到本地作为 `Chinook.db`，并通过 `db = SQLDatabase.from_uri("sqlite:///Chinook.db")` 实例化数据库。
...
遵循 [SQL 问答教程](/oss/langchain/sql-agent)，下面我们为一个简单的问答智能体配备工具包中的工具。首先，我们拉取一个相关的提示词模板，并填充其所需的参数：
...
`SQLDatabaseToolkit` 实现了一个 [.get_context](https://python.langchain.com/api_reference/community/agent_toolkits/langchain_community.agent_toolkits.sql.toolkit.SQLDatabaseToolkit.html#langchain_community.agent_toolkits.sql.toolkit.SQLDatabaseToolkit.get_context) 方法，作为在提示词或其他上下文中使用的便利方法。

**⚠️ 免责声明 ⚠️**：智能体可能会生成插入/更新/删除查询。当不期望发生这种情况时，请使用自定义提示词或创建一个没有写入权限的 SQL 用户。
...
对于事务性 SQL 数据库，如果上述表之一包含数百万行，该查询可能会给使用相同数据库的其他应用程序带来麻烦。
...
有关 `SQLDatabaseToolkit` 所有功能和配置的详细文档，请前往 [API 参考](https://python.langchain.com/api_reference/community/agent_toolkits/langchain_community.agent_toolkits.sql.toolkit.SQLDatabaseToolkit.html)。
```