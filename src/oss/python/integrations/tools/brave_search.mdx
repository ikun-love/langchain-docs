...
本 Notebook 介绍了如何使用 Brave Search 工具。
请访问 [Brave 网站](https://brave.com/search/api/) 注册一个免费账户并获取 API 密钥。
...
# 或者，如果你想从环境变量 BRAVE_SEARCH_API_KEY 中获取 api_key，并将 search_kwargs 设置为空
# tool = BraveSearch()

# 或者，如果你只想提供 api_key，并将 search_kwargs 设置为空
# tool = BraveSearch.from_api_key(api_key=api_key)

# 或者，如果你只想提供 search_kwargs，并从环境变量 BRAVE_SEARCH_API_KEY 中读取 api_key
