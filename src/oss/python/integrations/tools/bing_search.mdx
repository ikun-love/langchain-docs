...
> [Bing 搜索](https://learn.microsoft.com/en-us/bing/search-apis/bing-web-search/) 是一项 Azure 服务，它提供安全、无广告、且能感知地理位置的搜索结果，能从数十亿个网页文档中提取相关信息。通过利用 Bing 梳理数十亿网页、图片、视频和新闻的能力，仅需一次 API 调用，即可帮助您的用户从全球互联网中找到他们想要的信息。

## 设置

遵循[说明](https://learn.microsoft.com/en-us/bing/search-apis/bing-web-search/create-bing-search-service-resource)创建 Azure Bing 搜索 v7 服务资源，并获取订阅密钥。
...
通过 Bing 搜索运行查询，并返回摘要（snippet）、标题（title）和链接（link）元数据。
...
这里我们展示如何将其作为代理（agent）的一部分使用。我们将使用 OpenAI Functions Agent，因此需要设置和安装所需的依赖项。我们还将使用 [LangSmith Hub](https://smith.langchain.com/hub) 来拉取提示模板（prompt），因此也需要安装相应的依赖。
