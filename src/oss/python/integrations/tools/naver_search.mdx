...
`Naver Search Tool` 提供了一个简单的界面，用于搜索 Naver 并获取结果。
...
**搜索 (Search)**：`Naver Search Tool` 提供了一个简单的界面，用于搜索 Naver 并获取结果。

## 设置 (Setup)

### 配置 API 凭证

要使用 Naver 搜索，您需要获取 API 凭证。请遵循以下步骤：

登录 [Naver Developers 门户](https://developers.naver.com/main/)。
创建一个新应用，并启用 Search API。
从“Application List”（应用列表）部分获取您的 **`NAVER_CLIENT_ID`** 和 **`NAVER_CLIENT_SECRET`**。
...
[{'title': 'Seoul shares rise for 4th day on tech gains; won at 2-week low',
  ...
  'description': 'stocks-summary Seoul shares rise for 4th day on tech gains; won at 2-week low SEOUL, March 20 (Yonhap) -- Seoul shares extended their winning streak to a fourth day on the back of gains... ',
...
`Naver Search tool` 可以集成到 LangChain 代理中，以处理更复杂的任务。下面我们演示如何设置一个可以搜索 Naver 以获取最新信息的代理。
