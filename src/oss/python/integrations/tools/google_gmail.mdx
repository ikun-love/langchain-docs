...
本指南将帮助您开始使用 Gmail [工具包](/oss/langchain/tools#toolkits)。此工具包与 Gmail API 交互，可以读取消息、起草和发送邮件等。要查看所有 GmailToolkit 功能和配置的详细文档，请访问 [API 参考](https://python.langchain.com/api_reference/google_community/gmail/langchain_google_community.gmail.toolkit.GmailToolkit.html)。

## 设置

要使用此工具包，您需要设置凭据，具体说明请参阅 [Gmail API 文档](https://developers.google.com/gmail/api/quickstart/python#authorize_credentials_for_a_desktop_application)。下载 `credentials.json` 文件后，您就可以开始使用 Gmail API 了。
...
要启用对各个工具的自动化跟踪，请设置您的 [LangSmith](https://docs.langchain.com/langsmith/home) API 密钥：
...
在底层，`googleapi` 资源是使用以下方法创建的。
您也可以手动构建一个 `googleapi` 资源以获得更多的身份验证控制。
...
# 可以在此处查看可用的权限范围 (scopes) https://developers.google.com/gmail/api/auth/scopes
...
下面我们展示如何将工具包集成到 [Agent](/oss/langchain/agents) 中。
...
要查看所有 `GmailToolkit` 功能和配置的详细文档，请访问 [API 参考](https://python.langchain.com/api_reference/community/agent_toolkits/langchain_community.agent_toolkits.gmail.toolkit.GmailToolkit.html)。