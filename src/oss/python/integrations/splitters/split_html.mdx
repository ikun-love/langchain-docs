...
[HTMLHeaderTextSplitter](https://python.langchain.com/api_reference/text_splitters/html/langchain_text_splitters.html.HTMLHeaderTextSplitter.html) 是一个“感知结构”的[文本分割器](/oss/integrations/splitters)，它在 HTML 元素级别上分割文本，并为给定块的每个“相关”标题添加元数据。它可以逐元素返回块，也可以将具有相同元数据的元素组合起来，目标是 (a) （大致上）在语义上将相关文本分组，以及 (b) 保留编码在文档结构中的富含上下文的信息。它可以与其他文本分割器一起作为分块管道的一部分使用。

它类似于 Markdown 文件的 [MarkdownHeaderTextSplitter](/oss/integrations/splitters/markdown_header_metadata_splitter)。
...
与 [HTMLHeaderTextSplitter](#using-htmlheadertextsplitter) 概念相似，`HTMLSectionSplitter` 是一个“感知结构”的[文本分割器](/oss/integrations/splitters)，它在元素级别上分割文本并为每个相关标题添加元数据。它允许您按部分分割 HTML。
...
这在处理数据表或列表等内容时尤为重要，分割内容可能导致上下文丢失或混淆。生成的 @[`Document`] 对象保留了这些元素的完整结构，确保了信息的上下文相关性得以保持。
...
```
#### 解释
在此示例中，我们为 `iframe` 标签定义了一个自定义处理程序，它将它们转换为类似 Markdown 的链接。当分割器处理 HTML 内容时，它使用此自定义处理程序根据您提供的自定义逻辑来转换 `iframe` 标签，同时保留表格和列表等其他元素。生成的 @[`Document`] 对象显示了 iframe 如何根据您提供的自定义逻辑进行处理。
