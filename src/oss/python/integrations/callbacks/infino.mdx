...
>[Infino](https://github.com/infinohq/infino) 是一个可扩展的遥测存储系统，专为日志、指标和跟踪（trace）而设计。Infino 可以作为独立的观测性解决方案运行，也可以作为您观测性技术栈中的存储层。

本示例展示了如何在使用 `LangChain` 和 [Infino](https://github.com/infinohq/infino) 调用 OpenAI 和 ChatOpenAI 模型时，跟踪以下信息：
...
## 示例 1：LangChain OpenAI 问答；向 Infino 发布指标和日志

```python
# 在此处设置您的密钥。
# os.environ["OPENAI_API_KEY"] = "YOUR_API_KEY"

# 创建回调处理器。这会将延迟、错误、Token 使用情况、Prompt 以及 Prompt 响应记录到 Infino。
...
# 要向 OpenAI 模型提问的问题数量。我们在这里限制一个较小的数字，以节省运行此演示的费用。
num_questions = 10
...
for question in questions:
    ...
    # 我们将问题发送到 OpenAI API，并附带 Infino 回调。
...
我们现在使用 `matplotlib` 创建延迟、错误和消耗 Token 数量的图表。
