---
title: Google Cloud Vertex AI 重新排序器
---

> [Vertex Search 排名 API](https://cloud.google.com/generative-ai-app-builder/docs/ranking) 是 [Vertex AI Agent Builder](https://cloud.google.com/generative-ai-app-builder/docs/builder-apis) 中的独立 API 之一。它接收一个文档列表，并根据这些文档与查询的相关性对它们进行重新排序 (rerank)。与仅关注文档和查询语义相似性的嵌入 (embeddings) 相比，排名 API 可以为您提供关于文档回答给定查询效果的精确分数。排名 API 可用于在检索到候选文档的初始集合后，提高搜索结果的质量。

> 排名 API 是无状态的，因此在调用 API 之前不需要索引文档。您所需要做的就是传入查询和文档。这使得该 API 非常适合对来自任何文档检索器的文档进行重新排序。

> 更多信息，请参阅 [Rank and rerank documents](https://cloud.google.com/generative-ai-app-builder/docs/ranking)（排名和重新排序文档）。
...
在此示例中，我们将使用 [Google 维基百科页面](https://en.wikipedia.org/wiki/Google) 来演示 Vertex 排名 API 的工作原理。

我们使用标准的管道流程：`加载 (load) -> 分割 (split) -> 嵌入 (embed data)`。
...
print(f"您的 {len(data)} 个文档已被分割成 {len(splits)} 个块")

if vectordb is not None:  # 如果 vectordb 已存在，则删除其集合
...
# 使用 VertexAIRanker 作为重排序器 (Reranker) 创建 ContextualCompressionRetriever
retriever_with_reranker = ContextualCompressionRetriever(
    base_compressor=reranker, base_retriever=basic_retriever
)
```

### 测试 Vertex 排名 API (Testing out the Vertex Ranking API)

让我们使用相同的查询对 `basic_retriever` 和 `retriever_with_reranker` 进行查询，并比较检索到的文档。

排名 API 接收来自 `basic_retriever` 的输入，并将其传递给排名 API。

排名 API 用于提高排名的质量，并确定一个分数，该分数指示每条记录与查询的相关性。

您可以看到未排序 (Unranked) 文档和已排序 (Ranked) 文档之间的差异。排名 API 将最相关的文档移到 LLM 上下文窗口的顶部，从而帮助它通过推理形成更好的答案。
...
让我们检查一下排名前几位的文档。我们观察到检索器仍然返回相关的 LangChain 类型的 [文档](https://python.langchain.com/api_reference/core/documents/langchain_core.documents.base.Document.html)，但作为元数据字段的一部分，我们还收到了来自排名 API 的 `relevance_score`（相关性分数）。
...
这展示了一个完整的 RAG 链的示例，其中包含一个简单的 Prompt 模板，演示了如何使用 Vertex 排名 API 执行重新排序。
...
reranker = VertexAIRank(
    ...
    location_id=RANKING_LOCATION_ID,
    ranking_config="default_ranking_config",
    title_field="source",  # 您现有文档中的元数据字段键
    top_n=5,
)

# k 的值也可以设置为一个更高的值，以调整性能
# 例如：# of docs: basic_retriever(100) -> reranker(5)
basic_retriever = vectordb.as_retriever(search_kwargs={"k": 5})  # 获取前 5 个文档
