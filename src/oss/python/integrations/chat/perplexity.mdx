...
本文将帮助您开始使用 Perplexity 的 [Chat 模型](/oss/langchain/models)。有关 `ChatPerplexity` 所有功能和配置的详细文档，请参阅 [API 参考](https://python.langchain.com/api_reference/perplexity/chat_models/langchain_perplexity.chat_models.ChatPerplexity.html)。
...
| [工具调用](/oss/langchain/tools) | [结构化输出](/oss/langchain/structured-output) | JSON 模式 | [图像输入](/oss/langchain/messages#multimodal) | 音频输入 | 视频输入 | [Token 级别流式传输](/oss/langchain/streaming#llm-tokens) | 原生异步 | [Token 使用量](/oss/langchain/models#token-usage) | [Logprobs](/oss/langchain/models#log-probabilities) |
| :---: | :---: | :---: | :---: |  :---: | :---: | :---: | :---: | :---: | :---: |
| ❌ | ✅ | ✅ | ❌ | ❌ | ❌ | ✅ | ❌ | ✅ | ❌ |
...
请访问 [此页面](https://www.perplexity.ai/) 注册 Perplexity 并生成 API 密钥。完成该步骤后，请设置 `PPLX_API_KEY` 环境变量：
...
提供的代码假设您的 PPLX\_API\_KEY 已在您的环境变量中设置。如果您想手动指定 API 密钥并选择不同的模型，可以使用以下代码：

```python
chat = ChatPerplexity(temperature=0, pplx_api_key="YOUR_API_KEY", model="sonar")
```

您可以在 [此处](https://docs.perplexity.ai/docs/model-cards) 查看可用模型的列表。为了确保结果可复现，我们可以通过在此 Notebook 中获取输入的方式动态设置 API 密钥。
...
您可以像通常那样格式化和构造提示词。在下面的示例中，我们要求模型讲一个关于猫的笑话。
...
您还可以通过 `ChatPerplexity` 类使用特定于 Perplexity 的参数。例如，像 `search_domain_filter`、`return_images`、`return_related_questions` 或 `search_recency_filter` 这样的参数可以通过 `extra_body` 参数使用，如下所示：
...
Perplexity 通常会提供它所查阅的网页列表（“search\_results”）。
您无需传入任何特殊参数——该列表位于 `response.additional_kwargs["search_results"]` 中。
