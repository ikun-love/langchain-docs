...
<Warning>
    此页面引用了由 [xAI](https://docs.x.ai/docs/overview) 提供的 Grok 模型——请勿与 Groq 混淆，后者是一家独立的人工智能硬件和软件公司。请参阅 [Groq 提供商页面](/oss/integrations/providers/groq)。
</Warning>

[xAI](https://console.x.ai/) 提供了一个 API 用于与 Grok 模型进行交互。

<Tip>
    **API 参考**

    有关所有功能和配置选项的详细文档，请前往 @[`ChatXAI`] API 参考。
</Tip>
...
| [工具调用](/oss/langchain/tools) | [结构化输出](/oss/langchain/structured-output) | JSON 模式 | [图像输入](/oss/langchain/messages#multimodal) | 音频输入 | 视频输入 | [Token 级流式传输](/oss/langchain/streaming#llm-tokens) | 原生异步 | [Token 使用情况](/oss/langchain/models#token-usage) | [Logprobs](/oss/langchain/models#log-probabilities) |
| :---: | :---: | :---: | :---: |  :---: | :---: | :---: | :---: | :---: | :---: |
| ✅ | ✅ | ❌ | ❌ | ❌ | ❌ | ✅ | ❌ | ✅ | ✅ |
...
ChatXAI 具有 [工具调用](https://docs.x.ai/docs#capabilities) API（在此处我们交替使用“工具调用”和“函数调用”），它允许您描述工具及其参数，并让模型返回包含要调用的工具和该工具输入的 JSON 对象。工具调用对于构建使用工具的链和代理，以及更普遍地从模型获取结构化输出，非常有用。
...
xAI 支持 [实时搜索](https://docs.x.ai/docs/guides/live-search) 功能，该功能使 Grok 能够利用网络搜索结果来增强其答案：
