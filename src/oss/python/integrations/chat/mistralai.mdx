...
这将帮助您开始使用 Mistral [聊天模型](/oss/langchain/models)。有关 `ChatMistralAI` 所有特性和配置的详细文档，请参阅 [API 参考](https://python.langchain.com/api_reference/mistralai/chat_models/langchain_mistralai.chat_models.ChatMistralAI.html)。`ChatMistralAI` 类是基于 [Mistral API](https://docs.mistral.ai/api/) 构建的。有关 Mistral 支持的所有模型的列表，请查看 [此页面](https://docs.mistral.ai/getting-started/models/)。
...
与 API 通信需要一个有效的 [API 密钥](https://console.mistral.ai/api-keys/)。完成此操作后，设置环境变量 `MISTRAL_API_KEY`：
...
有关所有属性和方法的详细文档，请前往 [API 参考](https://python.langchain.com/api_reference/mistralai/chat_models/langchain_mistralai.chat_models.ChatMistralAI.html)。