...
本文档将指导您开始使用 AWS Bedrock [聊天模型](/oss/langchain/models)。Amazon Bedrock 是一项全托管服务，通过单一 API 提供了来自 AI21 Labs、Anthropic、Cohere、Meta、Stability AI 和 Amazon 等领先 AI 公司的多种高性能基础模型（FMs）的选择，以及构建生成式 AI 应用所需的广泛功能，同时确保安全、隐私和负责任的 AI。借助 Amazon Bedrock，您可以轻松地针对您的用例试验和评估顶级 FM，使用微调（fine-tuning）和检索增强生成（RAG）等技术利用您的数据对其进行私有定制，并构建利用您的企业系统和数据源来执行任务的智能体（agents）。由于 Amazon Bedrock 是无服务器的，您无需管理任何基础设施，并且可以使用您已经熟悉的 AWS 服务安全地将生成式 AI 功能集成并部署到您的应用程序中。
...
| [工具调用](/oss/langchain/tools) | [结构化输出](/oss/langchain/structured-output) | JSON 模式 | [图像输入](/oss/langchain/messages#multimodal) | 音频输入 | 视频输入 | [逐个 Token 流式传输](/oss/langchain/streaming/) | 原生异步 (Native async) | [Token 用量](/oss/langchain/models#token-usage) | [日志概率](/oss/langchain/models#log-probabilities) |
| :---: | :---: | :---: | :---: |  :---: | :---: | :---: | :---: | :---: | :---: |
| ✅ | ✅ | ❌ | ✅ | ❌ | ❌ | ✅ | ❌ | ✅ | ❌ |
...
如果输入文档启用了引用，则可以生成引用。文档可以采用 Bedrock 的[原生格式](https://docs.aws.amazon.com/bedrock/latest/APIReference/API_runtime_DocumentBlock.html)或 LangChain 的[标准类型](/oss/langchain/messages#multimodal)来指定：
