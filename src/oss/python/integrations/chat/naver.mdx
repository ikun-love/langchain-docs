...
本指南提供了通过 CLOVA Studio 使用 Naver 的 HyperCLOVA X [聊天模型](https://python.langchain.com/docs/concepts/chat_models)的快速入门概述。要了解 ChatClovaX 所有功能和配置的详细文档，请参阅[API 参考](https://guide.ncloud-docs.com/docs/clovastudio-dev-langchain)。

[CLOVA Studio](http://clovastudio.ncloud.com/) 提供了多种聊天模型。您可以在 CLOVA Studio 指南的[文档](https://guide.ncloud-docs.com/docs/clovastudio-model)中找到最新模型的信息，包括成本、上下文窗口和支持的输入类型。
...
3. 创建 CLOVA Studio 测试应用 (Test App) 或服务应用 (Service App) 以便使用模型（参见[此处](https://guide.ncloud-docs.com/docs/clovastudio-playground-testapp)）
4. 颁发测试或服务 API 密钥（参见[此处](https://api.ncloud-docs.com/docs/ai-naver-clovastudio-summary#API%ED%82%A4)）
...
除了下面的 `invoke` 方法外，`ChatClovaX` 还支持批量处理 (batch)、流式传输 (stream) 及其异步功能。
...
CLOVA Studio 支持工具调用（也称为“[函数调用](https://api.ncloud-docs.com/docs/clovastudio-chatcompletionsv3-fc)”），它允许您描述工具及其参数，并让模型返回一个包含要调用的工具和该工具输入的 **JSON 对象**。这对于构建使用工具的链 (chains) 和智能体 (agents) 非常有用，并且通常可以从模型中获取结构化输出。

**注意**：您应该设置 `max_tokens` 大于 1024 才能在 CLOVA Studio 中使用工具调用功能。

### ChatClovaX.bind_tools()

使用 `ChatClovaX.bind_tools`，我们可以轻松地将 Pydantic 类、字典 schema、LangChain 工具甚至函数作为工具传递给模型。在底层，这些会被转换为 **OpenAI 兼容的工具 schema**，其格式如下：
...
对于支持的模型，您可以使用[结构化输出](https://api.ncloud-docs.com/docs/clovastudio-chatcompletionsv3-so)功能，强制模型以特定结构（如 Pydantic 模型、TypedDict 或 JSON）生成响应。

**注意**：结构化输出要求禁用思考模式 (Thinking mode)。需将 `thinking.effort` 设置为 `none`。
...
**注意**：CLOVA Studio 使用 json schema 方法支持结构化输出。将 `method` 设置为 `json_schema`。
...
对于支持的模型，当[思考](https://api.ncloud-docs.com/docs/clovastudio-chatcompletionsv3-thinking)功能启用时（默认启用），它将输出导致最终答案的逐步推理过程。

通过指定 `thinking` 参数来控制此功能——启用或禁用思考过程并配置其深度。
...
当启用思考模式时，您可以通过 `AIMessage.additional_kwargs` 中的 `thinking_content` 属性来访问思考过程。
...
### 使用微调模型 (Using fine-tuned models)

您可以通过将 `task_id` 传递给 `model` 参数来以以下格式调用微调模型：`ft:{task_id}`。

您可以在相应的测试应用或服务应用的详情中查看 `task_id`。
...
要了解 ChatClovaX 所有功能和配置的详细文档，请参阅[API 参考](https://guide.ncloud-docs.com/docs/clovastudio-dev-langchain)。