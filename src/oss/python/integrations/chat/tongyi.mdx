...
title: 通义千问
---
通义千问（Tongyi Qwen）是由阿里巴巴达摩院开发的大型语言模型（LLM）。它基于用户输入的自然语言，通过自然语言理解和语义分析来理解用户意图，并在不同领域和任务中为用户提供服务和协助。通过提供清晰详细的指令，您可以获得与预期更一致的结果。
在本 Notebook 中，我们将介绍如何将 `langchain` 与 [通义千问](https://www.aliyun.com/product/dashscope) 结合使用，主要关注 `langchain` 中 `langchain/chat_models` 包对应的 `Chat` 功能。
...
`ChatTongyi` 支持工具调用 API，它允许您描述工具及其参数，并让模型返回一个包含要调用的工具和对应输入的 JSON 对象。
...
messages = [
    HumanMessage(
        content="""请续写句子 \"Spring has arrived, and the earth\" 来表达春天的美好和大自然作者的喜悦。"""
    ),
    AIMessage(
