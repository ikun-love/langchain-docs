...
本 Notebook 展示了如何使用 `Llama2Chat` 包装器对 Llama-2 的 `LLM` 进行增强，以支持 [Llama-2 聊天提示格式](https://huggingface.co/blog/llama2#how-to-prompt-llama-2)。LangChain 中有多种 `LLM` 实现可以用作 Llama-2 聊天模型的接口，其中包括 [ChatHuggingFace](/oss/integrations/chat/huggingface)、[LlamaCpp](/oss/integrations/chat/llamacpp/)、[GPT4All](/oss/integrations/llms/gpt4all) 等，仅举几例。

`Llama2Chat` 是一个通用的包装器，它实现了 `BaseChatModel` 接口，因此可以作为 [聊天模型](/oss/langchain/models) 在应用程序中使用。`Llama2Chat` 会将一系列 `Messages` 转换为[所需的聊天提示格式](https://huggingface.co/blog/llama2#how-to-prompt-llama-2)，并将格式化后的提示作为 `str` 传递给被包装的 `LLM`。
...
要将 Llama-2 聊天模型与 [LlamaCPP](/oss/integrations/providers/llamacpp) `LMM` 结合使用，请使用[这些安装说明](/oss/integrations/providers/llamacpp#installation-and-setup)安装 `llama-cpp-python` 库。以下示例使用存储在本地 `~/Models/llama-2-7b-chat.Q4_0.gguf` 的量化模型 [llama-2-7b-chat.Q4_0.gguf](https://huggingface.co/TheBloke/Llama-2-7B-Chat-GGUF/resolve/main/llama-2-7b-chat.Q4_0.gguf)。
