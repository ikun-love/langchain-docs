...
>[Prediction Guard](https://predictionguard.com) 是一个安全、可扩展的 GenAI 平台，可以保护敏感数据、防止常见的 AI 故障，并且运行在经济实惠的硬件上。

## 概述

### 集成详情

此集成利用了 Prediction Guard API，其中包含各种安全防护和安全功能。
...
要访问 Prediction Guard 模型，请在此处 [here](https://predictionguard.com/get-started) 与我们联系，以获取 Prediction Guard API 密钥并开始使用。
...
```python
# 如果未传递 predictionguard_api_key，则默认行为是使用环境变量 `PREDICTIONGUARD_API_KEY`。
...
Prediction Guard 拥有一个工具调用 API，它允许您描述工具及其参数。这使模型能够返回一个包含要调用的工具及其输入的 JSON 对象。工具调用对于构建使用工具的链和代理，以及更 সাধারণভাবে从模型中获取结构化输出非常有用。

### ChatPredictionGuard.bind_tools()

使用 `ChatPredictionGuard.bind_tools()`，您可以将 Pydantic 类、dict 模式（schema）和 LangChain 工具作为工具传递给模型，这些工具随后会被重新格式化以供模型使用。
...
请注意，`AIMessage` 具有一个 `@[`tool\_calls`][AIMessage.tool_calls]` 属性。它以一种标准化的 `ToolCall` 格式包含数据，这种格式与模型提供商无关。
...
使用 Prediction Guard，您可以使用我们的输入检查来保护模型的输入，防止出现个人身份信息 (PII) 或提示注入 (prompt injections)。有关更多信息，请参阅 [Prediction Guard 文档](https://docs.predictionguard.com/docs/process-llm-input/)。
...
使用 Prediction Guard，您可以使用事实性检查来验证模型输出，以防止出现事实性错误或不准确信息，并使用毒性检查来防止出现有害响应（例如脏话、仇恨言论）。有关更多信息，请参阅 [Prediction Guard 文档](https://docs.predictionguard.com/docs/validating-llm-output)。
