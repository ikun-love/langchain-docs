...
本指南将帮助您开始使用 Elasticsearch [检索器](/oss/langchain/retrieval)。有关 `ElasticsearchRetriever` 所有特性和配置的详细文档，请访问 [API 参考](https://python.langchain.com/api_reference/elasticsearch/retrievers/langchain_elasticsearch.retrievers.ElasticsearchRetriever.html)。
...
对于向量搜索 (vector search)，我们仅为说明目的而使用随机嵌入（random embeddings）。在实际用例中，请选择一个可用的 LangChain [嵌入（Embeddings）](/oss/integrations/text_embedding) 类。
