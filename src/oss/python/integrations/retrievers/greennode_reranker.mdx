...
>[GreenNode](https://greennode.ai/) 是一家全球性人工智能解决方案提供商，并且是 **NVIDIA 优选合作伙伴 (Preferred Partner)**，为美国、中东和亚太地区的各企业提供从基础设施到应用的**全栈 AI 能力**。GreenNode 依托于世界一流的基础设施（LEED 金级认证、TIA‑942、Uptime Tier III），为企业、初创公司和研究人员提供全面的 AI 服务组合。

本指南将指导您入门 `GreenNodeRerank` 检索器。它使您能够利用内置连接器或集成您自己的数据源来执行文档搜索，并利用 GreenNode 的重排序能力来提高相关性。

### 集成详情

- **提供商**: [GreenNode Serverless AI](https://aiplatform.console.greennode.ai/playground)
- **模型类型**: 重排序模型 (Reranking models)
- **主要用例**: 基于语义相关性对搜索结果进行重排序
- **可用模型**: 包括 [BAAI/bge-reranker-v2-m3](https://huggingface.co/BAAI/bge-reranker-v2-m3) 和其他高性能重排序模型
- **评分**: 返回相关性分数，用于根据查询匹配度重新排序候选文档
...
如果您想从单个查询获取自动化的追踪（tracing），您还可以通过取消注释以下代码来设置您的 [LangSmith](https://docs.langchain.com/langsmith/home) API 密钥：
...
重排序模型通过根据语义相关性对初始搜索结果进行精炼和重新排序，来增强检索增强生成 (RAG) 工作流。下面的示例演示了如何将 GreenNodeRerank 与基础检索器集成，以提高检索文档的质量。
...
`GreenNodeRerank` 类可以独立使用，根据相关性分数对检索到的文档进行重排序。在某些场景下，这种功能特别有用：当主要检索步骤（如关键字或向量搜索）返回了一组广泛的候选结果时，需要一个辅助模型来利用更复杂的语义理解来精炼这些结果。此类接受查询和一组候选文档，并返回一个根据预测的相关性重新排序的列表。
...
GreenNodeRerank 可以无缝地在 LangChain RAG 管道中工作。下面是一个创建包含 GreenNodeRerank 的简单 RAG 链的示例：
