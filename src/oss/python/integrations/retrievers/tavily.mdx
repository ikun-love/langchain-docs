...
>[Tavily 搜索引擎 API](https://tavily.com) 是一个专为 AI 智能体（LLM）构建的搜索引擎，它能够快速、准确地提供实时的、基于事实的搜索结果。

我们可以将其用作 [检索器](/oss/langchain/retrieval)。它将展示此特定集成的功能。完成本部分学习后，您可能希望探索 [相关用例页面](/oss/langchain/rag)，以了解如何将此向量存储用作更大链条的一部分。
...
如果您希望从单个查询中获得自动化的追踪记录（tracing），您可以通过取消下面代码行的注释来设置您的 [LangSmith](https://docs.langchain.com/langsmith/home) API 密钥：
...
```python
chain.invoke("旷野之息在2020年卖出了多少份?")
```

```output
'截至 2020 年 8 月，塞尔达传说：旷野之息在全球 Nintendo Switch 和 Wii U 上的销量已超过 2010 万份。'
```
