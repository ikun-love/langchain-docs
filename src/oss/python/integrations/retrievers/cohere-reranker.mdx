...
>[Cohere](https://cohere.ai/about) 是一家加拿大的初创公司，提供自然语言处理模型，帮助企业改善人机交互体验。

本 Jupyter Notebook 演示了如何在检索器（retriever）中使用 [Cohere 的重排序（rerank）端点](https://docs.cohere.com/docs/reranking)。
...
我们首先初始化一个简单的向量存储检索器，并存储 2023 年国情咨文（State of the Union）的文本（分块存储）。我们可以将检索器设置为检索较多的文档（20 份）。
...
现在让我们用 `ContextualCompressionRetriever` 包装我们的基础检索器。我们将添加一个 `CohereRerank`，它使用 Cohere 的重排序端点来对返回的结果进行重新排序。
请注意，在 `CohereRerank` 中指定模型名称是必需的！
