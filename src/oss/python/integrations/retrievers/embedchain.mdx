...
> [Embedchain](https://github.com/embedchain/embedchain) 是一个 RAG（检索增强生成）框架，用于创建数据管道。它负责加载、索引、检索和同步所有数据。
>
> 它提供了一个[开源包](https://github.com/embedchain/embedchain)以及一个[托管平台解决方案](https://app.embedchain.ai/)。

本 Notebook 演示了如何使用一个利用 `Embedchain` 的检索器（Retriever）。
...
`EmbedchainRetriever` 有一个静态的 `.create()` 工厂方法，它接受以下参数：

* `yaml_path: string` (可选) -- YAML 配置文件路径。如果未提供，则使用默认配置。您可以浏览[文档](https://docs.embedchain.ai/)来了解各种定制选项。
...
# 如果您想进行定制，可以传入 YAML 配置文件路径
# retriever = EmbedchainRetiever.create(yaml_path="config.yaml")
```

# 添加数据

在 Embedchain 中，您可以添加任意数量的支持数据类型。您可以浏览我们的[文档](https://docs.embedchain.ai/)以查看支持的数据类型。
