...
>[you.com API](https://api.you.com) 是一套工具集，旨在帮助开发者利用最新、最准确、最相关的信息（这些信息可能未包含在他们的训练数据集中）来校准大型语言模型 (LLM) 的输出。

## 设置 (Setup)

检索器 (retriever) 位于 `langchain-community` 软件包中。
...
为了获得最佳的可观测性 (observability)，设置 [LangSmith](https://smith.langchain.com/) 也很有帮助（但并非必须）。
...
```python
# .results 返回每个 snippet 都解析为一个 Document 的结果
response = utility.results(query="What is the weather in NY")

# .results 应该包含一个对应于每个 `snippet` 的 Document
print(len(response))
...
# .invoke 应该包含一个对应于每个 `snippet` 的 Document
print(len(response))
