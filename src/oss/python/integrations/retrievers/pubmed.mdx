...
> [PubMed®](https://pubmed.ncbi.nlm.nih.gov/) 由 `The National Center for Biotechnology Information, National Library of Medicine`（美国国家生物技术资讯中心、美国国家医学图书馆）运营，收录了超过 3500 万条生物医学文献引文，这些文献来源于 `MEDLINE`、生命科学期刊和在线书籍。引文通常包含指向 `PubMed Central` 和出版商网站上全文内容的链接。

本 Notebook 将介绍如何使用 `PubMed` 作为检索器（Retriever）。
