...
本指南概述了 LangGraph v1 中的变化以及如何从早期版本进行迁移。有关更改的高级别概述，请参阅 [最新消息](/oss/releases/langgraph-v1) 页面。
...
LangGraph v1 与早期版本基本向后兼容。主要变化是弃用了 @[`create_react_agent`]，转而使用 LangChain 新的 @[`create_agent`] 函数。
...
| `ValidationNode` | 工具现在使用 @[`create_agent`] 自动验证输入 |
| `MessageGraph` | 使用带有 `messages` 键的 @[`StateGraph`]，类似于 @[`create_agent`] 提供的结构 |

## `create_react_agent` → `create_agent`

LangGraph v1 弃用了 @[`create_react_agent`] 预置函数。请使用 LangChain 的 @[`create_agent`]，它运行在 LangGraph 之上，并增加了一个灵活的中间件系统。
