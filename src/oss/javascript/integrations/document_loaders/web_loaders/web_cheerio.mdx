...
本 Notebook 提供了关于如何开始使用 [CheerioWebBaseLoader](/oss/integrations/document_loaders/) 的快速概览。如需了解 `CheerioWebBaseLoader` 所有功能和配置的详细文档，请前往 [API 参考](https://api.js.langchain.com/classes/langchain_community_document_loaders_web_cheerio.CheerioWebBaseLoader.html)。
...
然而，Cheerio 不会模拟网页浏览器，因此它无法执行页面上的 JavaScript 代码。这意味着它无法从需要 JavaScript 渲染的动态网页中提取数据。要完成此操作，您可以转而使用 [`PlaywrightWebBaseLoader`](/oss/integrations/document_loaders/web_loaders/web_playwright) 或 [`PuppeteerWebBaseLoader`](/oss/integrations/document_loaders/web_loaders/web_puppeteer)。
