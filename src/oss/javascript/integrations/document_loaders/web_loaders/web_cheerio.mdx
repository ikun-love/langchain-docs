...
本 Notebook 快速概述了如何开始使用 [CheerioWebBaseLoader](/oss/integrations/document_loaders/)。有关 `CheerioWebBaseLoader` 所有功能和配置的详细文档，请访问 [API 参考](https://api.js.langchain.com/classes/langchain_community_document_loaders_web_cheerio.CheerioWebBaseLoader.html)。
...
Cheerio 是一个快速且轻量级的库，它允许您使用类似 jQuery 的语法来解析和遍历 HTML 文档。您可以使用 Cheerio 从网页中提取数据，而无需在浏览器中渲染它们。

然而，Cheerio 不会模拟 Web 浏览器，因此它无法执行页面上的 JavaScript 代码。这意味着它无法从需要 JavaScript 渲染的动态网页中提取数据。要实现此目的，您可以使用 [`PlaywrightWebBaseLoader`](/oss/integrations/document_loaders/web_loaders/web_playwright) 或 [`PuppeteerWebBaseLoader`](/oss/integrations/document_loaders/web_loaders/web_puppeteer) 代替。
...
如果您想获取模型调用的自动化追踪，您可以通过取消注释以下行来设置您的 [LangSmith](https://docs.langchain.com/langsmith/home) API 密钥：
...
`CheerioWebBaseLoader` 在实例化加载器时支持额外的配置。这是一个如何使用 `selector` 字段的示例，它只加载提供的 HTML 类名中的内容：
...
有关 `CheerioWebBaseLoader` 所有功能和配置的详细文档，请访问 [API 参考](https://api.js.langchain.com/classes/langchain_community_document_loaders_web_cheerio.CheerioWebBaseLoader.html)。
```