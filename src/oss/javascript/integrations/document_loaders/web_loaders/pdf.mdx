...
本 Notebook 提供了关于如何开始使用 [WebPDFLoader](/oss/integrations/document_loaders/) 的快速指南。要查看 WebPDFLoader 所有功能和配置的详细文档，请前往 [API 参考](https://api.js.langchain.com/classes/langchain_community_document_loaders_web_pdf.WebPDFLoader.html)。
...
默认情况下，会为 PDF 文件中的**每一页**创建一个文档。您可以通过设置 `splitPages` 选项为 `false` 来更改此行为。

## 设置

要访问 `WebPDFLoader` 文档加载器，您需要安装 `@langchain/community` 集成，同时还需要安装 `pdf-parse` 包：

### 凭证 (Credentials)

如果您希望对您的模型调用进行自动化的跟踪，您可以通过取消注释以下代码来设置您的 [LangSmith](https://docs.langchain.com/langsmith/home) API 密钥：
...
默认情况下，我们使用 `pdf-parse` 打包的 `pdfjs` 构建，它与大多数环境（包括 Node.js 和现代浏览器）兼容。如果您想使用更新版本的 `pdfjs-dist`，或者想使用自定义构建的 `pdfjs-dist`，可以通过提供一个自定义的 `pdfjs` 函数来实现，该函数返回一个解析到 `PDFJS` 对象的 Promise。

在以下示例中，我们使用了 `pdfjs-dist` 的“传统/旧版 (legacy)”版本（请参阅 [pdfjs 文档](https://github.com/mozilla/pdf.js/wiki/Frequently-Asked-Questions#which-browsersenvironments-are-supported)），该版本包含了默认构建中未包含的多个 Polyfills（垫片）。
...
const blob = new Blob(); // 例如，来自文件输入

const customBuildLoader = new WebPDFLoader(blob, {
  // 您可能需要在导入的末尾添加 `.then(m => m.default)`
  // @lc-ts-ignore
...
PDF 文件种类繁多，这给文本阅读带来了挑战。加载器默认解析单个文本元素并将它们用空格连接起来。但是，如果您发现出现了过多的空格，这可能不是您想要的行为。在这种情况下，您可以像下面这样，将分隔符重置为空字符串：
...
要查看 WebPDFLoader 所有功能和配置的详细文档，请前往 [API 参考](https://api.js.langchain.com/classes/langchain_community_document_loaders_web_pdf.WebPDFLoader.html)。