...
[Tavily 的搜索 API](https://tavily.com) 专为 AI 智能体（LLM）构建的搜索引擎，它能快速提供实时、准确且基于事实的搜索结果。

## 概述

本文将帮助您快速入门 Tavily 搜索 API [检索器](/oss/langchain/retrieval)。有关 `TavilySearchAPIRetriever` 所有功能和配置的详细文档，请前往 [API 参考](https://api.js.langchain.com/classes/langchain_community_retrievers_tavily_search_api.TavilySearchAPIRetriever.html)。

### 集成详情

| 检索器 | 数据源 | 包 |
| :--- | :--- | :---: |
[`TavilySearchAPIRetriever`](https://api.js.langchain.com/classes/langchain_community_retrievers_tavily_search_api.TavilySearchAPIRetriever.html) | 网页信息 | [`@langchain/community`](https://npmjs.com/@langchain/community/) |

## 设置

您需要将 `TAVILY_API_KEY` 环境变量设置为您的 Tavily API 密钥，或者将其作为 `apiKey` 传入构造函数中。您可以通过 [其网站](https://tavily.com/) 注册获取密钥。

如果您想从单个查询中获取自动化的追踪信息，也可以通过取消注释以下代码来设置您的 [LangSmith](https://docs.langchain.com/langsmith/home) API 密钥：
...
有关 `TavilySearchAPIRetriever` 所有功能和配置的详细文档，请前往 [API 参考](https://api.js.langchain.com/classes/langchain_community_retrievers_tavily_search_api.TavilySearchAPIRetriever.html)。