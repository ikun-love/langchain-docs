...
本指南将帮助您开始使用 [AzionRetriever](/oss/langchain/retrieval)。有关 AzionRetriever 所有功能和配置的详细文档，请访问 [API 参考](https://api.js.langchain.com/classes/_langchain_community.retrievers_azion_edgesql.AzionRetriever.html)。

### 集成详情

| 检索器 (Retriever) | 自部署 (Self-host) | 云服务 (Cloud offering) | 包 (Package) | [Python 支持] |
| :--- | :---: | :---: | :---: | :---: |
...
如果您正在使用 OpenAI embeddings，对于本指南，您还需要设置您的 OpenAI 密钥：

```typescript
process.env.OPENAI_API_KEY = "YOUR_API_KEY";
```

如果您想从单独的查询中获取自动化的追踪 (tracing) 记录，您还可以通过取消注释以下内容来设置您的 [LangSmith](https://docs.langchain.com/langsmith/home) API 密钥：
...
const retriever = new AzionRetriever(embeddingModel,
  {dbName:"langchain",
   ...
   ftsTable:"documents_fts", // 存储 FTS (全文搜索) 索引的表
...
有关 AzionRetriever 所有功能和配置的详细文档，请访问 [API 参考](https://api.js.langchain.com/classes/_langchain_community.retrievers_azion_edgesql.AzionRetriever.html)。