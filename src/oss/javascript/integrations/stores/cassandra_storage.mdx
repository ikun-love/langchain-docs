...
此示例演示了如何使用 `CassandraKVStore` @[`BaseStore`] 集成来设置聊天记录存储。请注意，有一个 `CassandraChatMessageHistory` 集成可能更易于用于聊天记录存储；如果您需要一个具有可添加前缀键的更通用的键值存储，那么 `CassandraKVStore` 会非常有用。
