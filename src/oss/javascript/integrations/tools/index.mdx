---
title: 工具和工具包
---

[工具 (Tools)](/oss/langchain/tools) 是设计用来被模型调用的实用程序：它们的设计输入由模型生成，输出则返回给模型。

[工具包 (Toolkit)](/oss/langchain/tools#toolkits) 是旨在一起使用的工具集合。

## 集成平台

以下平台通过统一的界面提供了对多个工具和服务的访问：

| 工具/工具包 | 集成数量 | 定价 | 主要功能 |
|-------------|----------|------|--------------|
| [Composio](/oss/integrations/tools/composio) | 500+ | 提供免费套餐 | OAuth 处理、事件驱动的工作流、多用户支持 |

## 所有工具和工具包

<Columns cols={3}>
  <Card
    title="Azure Container Apps 动态会话"
    icon="link"
    href="/oss/integrations/tools/azure_dynamic_sessions"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="Connery Action 工具"
    icon="link"
    href="/oss/integrations/tools/connery"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="Composio"
    icon="link"
    href="/oss/integrations/tools/composio"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="Dall-E 工具"
    icon="link"
    href="/oss/integrations/tools/dalle"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="Decodo 工具"
    icon="link"
    href="/oss/integrations/tools/decodo"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="Discord 工具"
    icon="link"
    href="/oss/integrations/tools/discord_tool"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="DuckDuckGoSearch"
    icon="link"
    href="/oss/integrations/tools/duckduckgo_search"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="ExaSearchResults"
    icon="link"
    href="/oss/integrations/tools/exa_search"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="FalkorDB"
    icon="link"
    href="/oss/integrations/tools/falkordb"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="Gmail 工具"
    icon="link"
    href="/oss/integrations/tools/google_gmail"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="GOAT"
    icon="link"
    href="/oss/integrations/tools/goat"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="Google Calendar 工具"
    icon="link"
    href="/oss/integrations/tools/google_calendar"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="Google Places 工具"
    icon="link"
    href="/oss/integrations/tools/google_places"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="Google Routes 工具"
    icon="link"
    href="/oss/integrations/tools/google_routes"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="Google Scholar"
    icon="link"
    href="/oss/integrations/tools/google_scholar"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="Google Trends 工具"
    icon="link"
    href="/oss/integrations/tools/google_trends"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="JigsawStack 工具"
    icon="link"
    href="/oss/integrations/tools/jigsawstack"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="带 AWS Lambda 的智能体"
    icon="link"
    href="/oss/integrations/tools/lambda_agent"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="Python 解释器工具"
    icon="link"
    href="/oss/integrations/tools/pyinterpreter"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="SearchApi 工具"
    icon="link"
    href="/oss/integrations/tools/searchapi"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="Searxng Search 工具"
    icon="link"
    href="/oss/integrations/tools/searxng"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="SerpAPI"
    icon="link"
    href="/oss/integrations/tools/serpapi"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="StackExchange 工具"
    icon="link"
    href="/oss/integrations/tools/stackexchange"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="Stagehand AI Web 自动化工具包"
    icon="link"
    href="/oss/integrations/tools/stagehand"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="Tavily Crawl"
    icon="link"
    href="/oss/integrations/tools/tavily_crawl"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="Tavily Extract"
    icon="link"
    href="/oss/integrations/tools/tavily_extract"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="Tavily Map"
    icon="link"
    href="/oss/integrations/tools/tavily_map"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="Tavily Search"
    icon="link"
    href="/oss/integrations/tools/tavily_search"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="Web 浏览器工具"
    icon="link"
    href="/oss/integrations/tools/webbrowser"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="Wikipedia 工具"
    icon="link"
    href="/oss/integrations/tools/wikipedia"
    arrow="true"
    cta="查看指南"
  />
  <Card
    title="WolframAlpha 工具"
    icon="link"
    href="/oss/integrations/tools/wolframalpha"
    arrow="true"
    cta="查看指南"
  />
</Columns>

<Info>
    如果您想编写自己的工具，请参阅[此操作指南](/oss/langchain/tools#customize-tool-properties)。如果您想贡献集成，请参阅[贡献指南](/oss/contributing#add-a-new-integration)。
</Info>