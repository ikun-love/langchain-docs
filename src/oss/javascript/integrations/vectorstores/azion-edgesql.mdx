...
`AzionVectorStore` 用于在 Azion 的边缘平台（Edge Platform）上，直接使用 Edge SQL 管理和搜索基于向量嵌入的文档集合。

本指南将快速概述如何在 Azion EdgeSQL [向量存储](/oss/integrations/vectorstores) 中入门。有关 `AzionVectorStore` 所有功能和配置的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/_langchain_community.vectorstores_azion_edgesql.AzionVectorStore.html)。
...
如果您正在本指南中使用 OpenAI 嵌入，也需要设置您的 OpenAI 密钥：

```typescript
process.env.OPENAI_API_KEY = "YOUR_API_KEY";
```

如果您希望对模型调用进行自动化跟踪，您也可以通过取消注释以下内容来设置您的 [LangSmith](https://docs.langchain.com/langsmith/home) API 密钥：
...
一旦创建了向量存储并将相关文档添加进去后，您很有可能希望在链或代理运行时查询它。
