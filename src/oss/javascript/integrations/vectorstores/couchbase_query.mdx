...
(原名为 `CouchbaseVectorStore`)

- **查询与索引服务**: 使用 Couchbase 的查询服务 (SQL++)，而不是搜索服务。
- **无需索引**: 基本操作不需要预先配置的搜索索引。
- **SQL++ 语法**: 支持 `WHERE` 子句和 SQL++ 查询语法进行筛选。
- **向量函数**: 使用 `APPROX_VECTOR_DISTANCE` 函数进行相似性计算。
- **距离策略**: 支持多种距离策略（点积 Dot Product、余弦 Cosine、欧几里得 Euclidean、欧几里得平方 Euclidean Squared）。
...
您也可以使用任何其他支持的方法连接到集群。

有关连接到 Couchbase 集群的更多信息，请查阅 [Node SDK 文档](https://docs.couchbase.com/nodejs-sdk/current/hello-world/start-using-sdk.html#connect)。

```typescript
import { Cluster } from "couchbase";

const connectionString = "couchbase://localhost";
const dbUsername = "Administrator"; // 拥有查询的 bucket 的有效数据库用户（具有读取访问权限）
...
查询向量存储支持创建向量索引以提高搜索性能。有两种可用的索引类型：

### 超大规模索引 (Hyperscale index)
一种专用的向量索引，它利用 Couchbase 的向量索引功能，针对向量操作进行了优化：
...
await vectorStore.createIndex({
  ...
  fields: ["text", "metadata"], // 可选，默认为 text 字段
  whereClause: "type = 'document'", // 可选的筛选条件
  indexScanNprobes: 10, // 可选的调优参数
  indexTrainlist: 1000, // 可选的调优参数
});
```
...
一种通用用途的 GSI 索引，它将向量字段与标量字段包含在一起：
...
| 方面 | 超大规模索引 (Hyperscale Index) | 复合索引 (Composite Index) |
|--------|-----------------------|--------------------|
| **SQL++ 语法** | `CREATE VECTOR INDEX` | `CREATE INDEX` |
| **向量字段** | `(field VECTOR)` 带有 `INCLUDE` 子句 | `(field1, field2, vector_field VECTOR)` |
| **向量参数** | 支持所有向量参数 | 支持所有向量参数 |
| **优化** | 专为向量操作优化 | 带有向量支持的通用 GSI |
| **用例** | 純向量相似性搜索 | 混合向量和标量查询 |
| **性能** | 针对向量距离计算进行了优化 | 适用于混合查询 |
| **调优参数** | 支持 `indexScanNprobes`, `indexTrainlist` | 支持 `indexScanNprobes`, `indexTrainlist` |
| **限制** | 只有一个向量字段，使用 INCLUDE 包含其他字段 | 在多个索引键中只有一个向量字段 |
...
// 使用 OpenAIEmbeddings 需要 OpenAI API Key
const embeddings = new OpenAIEmbeddings({
...
- `DistanceStrategy.DOT` - [点积](https://docs.couchbase.com/server/current/vector-index/vectors-and-indexes-overview.html#dot) (默认值)
...
- `DistanceStrategy.EUCLIDEAN_SQUARED` - [欧几里得平方距离](https://docs.couchbase.com/server/current/vector-index/vectors-and-indexes-overview.html#euclidean-squared) (也称为 L2 Squared)

### 索引类型

- `IndexType.HYPERSCALE` - 专用的向量索引，用于实现最佳的向量搜索性能
...
2. **选择索引类型**:
   - 对于纯粹的向量搜索工作负载（主要执行相似性搜索），请使用**超大规模索引 (Hyperscale indexes)**。
   - 对于结合了向量相似性和标量字段筛选的混合查询，请使用**复合索引 (Composite indexes)**。
3. **调优参数**: 根据数据大小和性能要求调整 `indexScanNprobes` 和 `indexTrainlist`。
4. **尽早筛选**: 使用 `WHERE` 子句在向量计算之前减小搜索空间。
...
- 增加 `indexTrainlist` 参数（默认建议：每中心点约 50 个向量）。
- 确保您的集合中有足够的带有向量嵌入的文档。
- 对于少于 100 万个向量的集合，使用 `number_of_vectors / 1000` 作为中心点数量。
- 对于更大的集合，使用 `sqrt(number_of_vectors)` 作为中心点数量。
...
| 设置复杂度 | 较低 | 较高 |
| 性能 | 配合索引时表现良好 | 针对搜索进行了优化 |

## 常见问题解答

### 我需要在使用 `CouchbaseQueryVectorStore` 之前创建索引吗？

不需要。与基于搜索的 `CouchbaseSearchVectorStore` 不同，基于查询的实现可以在没有预建索引的情况下工作。但是，创建适当的向量索引（超大规模或复合索引）将显著提高查询性能。

### 我应该使用超大规模索引还是复合索引？

- 当您主要执行向量相似性搜索且对其他字段的筛选很少时，请使用**超大规模索引 (Hyperscale indexes)**。
- 当您经常在同一个查询中组合向量相似性与标量字段筛选时，请使用**复合索引 (Composite indexes)**。
- 了解如何[选择正确的向量索引](https://docs.couchbase.com/server/current/vector-index/use-vector-indexes.html)。

### 我可以在同一数据上同时使用 `CouchbaseQueryVectorStore` 和 `CouchbaseSearchVectorStore` 吗？

可以，它们都可以作用于相同的文档结构。但是，它们使用的服务（搜索 vs 查询）不同，并且具有不同的索引要求。
