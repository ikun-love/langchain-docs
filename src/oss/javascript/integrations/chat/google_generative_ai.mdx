...
本指南将帮助您开始使用 `ChatGoogleGenerativeAI` [聊天模型](/oss/langchain/models)。有关 `ChatGoogleGenerativeAI` 所有特性和配置的详细文档，请查阅 [API 参考](https://api.js.langchain.com/classes/langchain_google_genai.ChatGoogleGenerativeAI.html)。
...
<Tip>
您还可以通过 LangChain 的 VertexAI 和 VertexAI-web 集成来访问 Google 的 `gemini` 系列模型。点击 [此处](/oss/integrations/chat/google_vertex_ai) 阅读文档。
</Tip>
...
Google AI 的工具调用与 [其他模型的工具调用](/oss/langchain/tools) 大多相同，但在 Schema 方面有一些限制。
