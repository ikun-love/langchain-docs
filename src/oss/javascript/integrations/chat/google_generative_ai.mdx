...
[Google AI](https://ai.google.dev/) 提供多种聊天模型，包括强大的 Gemini 系列。有关最新模型、其功能、上下文窗口等方面的信息，请访问 [Google AI 文档](https://ai.google.dev/gemini-api/docs/models/gemini)。

这将帮助您开始使用 `ChatGoogleGenerativeAI` [聊天模型](/oss/langchain/models)。有关 `ChatGoogleGenerativeAI` 所有功能和配置的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_google_genai.ChatGoogleGenerativeAI.html)。

## 概述

### 集成详情

| 类 | 包名 | 本地 | 可序列化 | [PY 支持](https://python.langchain.com/docs/integrations/chat/google_generative_ai) | 下载量 | 版本 |
| :--- | :--- | :---: | :---: |  :---: | :---: | :---: |
...
<Tip>
您也可以通过 LangChain 的 VertexAI 和 VertexAI-web 集成来访问 Google 的 `gemini` 系列模型。点击 [此处](/oss/integrations/chat/google_vertex_ai) 阅读文档。
</Tip>
...
如果您希望对模型的调用进行自动跟踪，也可以通过取消注释以下代码来设置您的 [LangSmith](https://docs.langchain.com/langsmith/home) API 密钥：
...
Gemini 模型具有默认的安全设置，可以被覆盖。如果您的模型返回大量“安全警告”，您可以尝试调整模型的 `safety_settings` 属性。例如，要关闭对危险内容的安全性阻止，可以从 `@google/generative-ai` 包导入枚举，然后按如下方式构建您的 LLM：
...
Google AI 的工具调用与[其他模型的工具调用](/oss/langchain/tools)大部分相同，但在 Schema 方面有一些限制。

Google AI API 不允许工具 Schema 包含具有未知属性的对象。例如，以下 Zod Schema 将会引发错误：
...
Google 还提供了一个内置的搜索工具，可用于将内容生成与现实世界信息联系起来。以下是如何使用它的示例：
...
Google Generative AI 也支持代码执行。使用内置的 `CodeExecutionTool`，您可以让模型生成代码、执行它，并将结果用于最终的补全中：
...
上下文缓存允许您一次性将某些内容传递给模型，缓存输入的 token，然后在后续请求中引用这些缓存的 token 以减少费用。您可以使用 `GoogleAICacheManager` 类创建一个 `CachedContent` 对象，然后使用 `enableCachedContent()` 方法将 `CachedContent` 对象传递给您的 `ChatGoogleGenerativeAIModel`。
...
- 上下文缓存的最小输入 token 数量是 32,768，最大数量与给定模型的最大值相同。

## Gemini 提示词常见问题 (Gemini prompting FAQs)

截至本文档撰写时（2023/12/12），Gemini 对其接受的提示词的类型和结构有一些限制。具体来说：

1. 在提供多模态（图像）输入时，您最多只能提供 1 条“user”（用户）类型的消息。您不能传递多条消息（尽管单条用户消息可以包含多个内容条目）。
2. 系统消息不被原生支持，如果存在，它们将与第一条用户消息合并。
3. 对于常规聊天对话，消息必须遵循人/AI/人/AI 的交替模式。您不能连续提供 2 条 AI 消息或 2 条用户消息。
4. 如果消息违反了 LLM 的安全检查，它们可能会被阻止。在这种情况下，模型将返回一个空响应。

---

## API 参考

有关 `ChatGoogleGenerativeAI` 所有功能和配置的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_google_genai.ChatGoogleGenerativeAI.html)。