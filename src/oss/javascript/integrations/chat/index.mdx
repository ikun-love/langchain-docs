...
[聊天模型](/oss/langchain/models) 是以消息序列作为输入并返回消息作为输出的语言模型 <Tooltip tip="不遵循聊天模型接口的旧模型，它们使用接受字符串作为输入并返回字符串作为输出的接口。这些模型通常在其名称中不包含前缀 'Chat' 或包含后缀 'LLM'。">(与纯文本模型相对应)</Tooltip>。
...
| 模型 | 流式传输 (Stream) | JSON 模式 | [工具调用](/oss/langchain/tools/) | [`withStructuredOutput()`](/oss/langchain/structured-output#the-.withstructuredoutput-method) | [多模态](/oss/langchain/messages#multimodal) |
|-|-|-|-|-|-|
...
| [`BedrockChat`](/oss/integrations/chat/bedrock/) | ✅ | ❌ | 🟡 (仅 Bedrock Anthropic) | 🟡 (仅 Bedrock Anthropic) | 🟡 (仅 Bedrock Anthropic) |
...
某些模型提供商提供与 OpenAI 的（旧版）[Chat Completions API](https://platform.openai.com/docs/guides/completions) 兼容的端点。在这种情况下，您可以使用 [`ChatOpenAI`](/oss/integrations/chat/openai) 并设置自定义 `base_url` 来连接到这些端点。请注意，建立在 Chat Completions API 上的功能可能不完全被 `ChatOpenAI` 支持；在这种情况下，请考虑使用特定于提供商的类（如果可用）。
