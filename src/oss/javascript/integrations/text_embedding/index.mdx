...
LangChain 通过 @[Embeddings] 接口为文本嵌入模型（例如 OpenAI, Cohere, Hugging Face）提供了一个标准的接口。
...
- **documentEmbeddingStore**: 用于缓存文档嵌入的任何 [`BaseStore`](/oss/integrations/stores/) 实例。
...
在生产环境中，您通常会使用更健壮的持久化存储，例如数据库或云存储。请参阅 [stores 集成](/oss/integrations/stores/) 查看可用选项。
