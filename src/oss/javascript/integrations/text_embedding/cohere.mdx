...
这将帮助您开始使用 LangChain 中的 `CohereEmbeddings` [嵌入模型](/oss/integrations/text_embedding)。有关 `CohereEmbeddings` 功能和配置选项的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_cohere.CohereEmbeddings.html)。

## 概述

### 集成详情

| 类 (Class) | 包 (Package) | 本地 (Local) | [Py 支持](https://python.langchain.com/docs/integrations/text_embedding/cohere/) | 下载量 (Downloads) | 版本 (Version) |
| :--- | :--- | :---: | :---: | :---: | :---: |
...
要访问 Cohere 嵌入模型，您需要创建一个 Cohere 账户，获取一个 API 密钥，并安装 `@langchain/cohere` 集成包。

### 凭证 (Credentials)

前往 [cohere.com](https://cohere.com) 注册 `Cohere` 账户并生成 API 密钥。完成此操作后，设置 `COHERE_API_KEY` 环境变量：

```bash
export COHERE_API_KEY="your-api-key"
```

如果您希望对模型调用进行自动跟踪，您还可以通过取消注释以下内容来设置您的 [LangSmith](https://docs.langchain.com/langsmith/home) API 密钥：
...
现在我们可以实例化模型对象并生成聊天补全（在本例中是嵌入向量）：

```typescript
import { CohereEmbeddings } from "@langchain/cohere";

const embeddings = new CohereEmbeddings({
  apiKey: "YOUR-API-KEY", // 在 Node.js 环境中，它会默认读取 process.env.COHERE_API_KEY
  batchSize: 48, // 如果省略，默认值为 48。最大值为 96
  model: "embed-english-v3.0",
});
```

### 针对 Azure 上的 Cohere、AWS Bedrock 上的 Cohere 以及独立 Cohere 实例的自定义客户端

我们可以实例化一个自定义的 `CohereClient` 并将其传递给 `ChatCohere` 构造函数（注意：此处代码为 `CohereEmbeddings` 构造函数，但逻辑是相同的）。
...
嵌入模型通常用于检索增强生成 (RAG) 流程中，既可以作为数据索引的一部分，也可以在后续检索中使用。有关更详细的说明，请参阅 [**学习** 选项卡](/oss/learn/) 下的 RAG 教程。
...
您可以使用 `embedDocuments` 来嵌入多个文本，以用于索引。此方法的内部机制可能（但不一定）与嵌入查询的方法不同：
...
有关所有 CohereEmbeddings 功能和配置的详细文档，请访问 [API 参考](https://api.js.langchain.com/classes/langchain_cohere.CohereEmbeddings.html)。