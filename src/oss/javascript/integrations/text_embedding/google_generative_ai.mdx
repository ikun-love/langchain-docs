...
使用 LangChain，这将帮助您开始使用 Google 生成式 AI [嵌入模型](/oss/integrations/text_embedding)。有关 `GoogleGenerativeAIEmbeddings` 功能和配置选项的详细文档，请参阅[API 参考](https://api.js.langchain.com/classes/langchain_google_genai.GoogleGenerativeAIEmbeddings.html)。

## 概述

### 集成详情

| 类 (Class) | 包 (Package) | 本地 (Local) | [Python 支持](https://python.langchain.com/docs/integrations/text_embedding/google_generative_ai/) | 下载量 (Downloads) | 版本 (Version) |
| :--- | :--- | :---: | :---: |  :---: | :---: |
...
要访问 Google 生成式 AI 嵌入模型，您需要注册一个 Google AI 账户，获取一个 API 密钥，并安装 `@langchain/google-genai` 集成包。
...
如果您希望对模型调用进行自动化跟踪，您还可以通过取消注释以下内容来设置您的 [LangSmith](https://docs.langchain.com/langsmith/home) API 密钥：
...
LangChain 的 `GoogleGenerativeAIEmbeddings` 集成位于 `@langchain/google-genai` 包中。您也可以选择安装官方 SDK：
...
const embeddings = new GoogleGenerativeAIEmbeddings({
  model: "gemini-embedding-001", // 768 维度
...
嵌入模型通常用于检索增强生成 (RAG) 流程中，既可以作为数据索引的一部分，也可以用于后续的检索。有关更详细的说明，请参阅[**学习 (Learn)** 标签页](/oss/learn/)下的 RAG 教程。

下面展示了如何使用我们上面初始化的 `embeddings` 对象来索引和检索数据。在这个例子中，我们将使用演示用的 [`MemoryVectorStore`](/oss/integrations/vectorstores/memory) 来索引和检索一个示例文档。
...
在底层，向量存储和检索器的实现分别调用 `embeddings.embedDocument(...)` 和 `embeddings.embedQuery(...)` 来为 `fromDocuments` 中使用的文本和检索器的 `invoke` 操作中的文本创建嵌入向量。

您可以直接调用这些方法来为您自己的用例获取嵌入向量。

### 嵌入单个文本

您可以使用 `embedQuery` 来嵌入用于搜索的查询。这会生成一个针对特定查询的向量表示：
...
您可以使用 `embedDocuments` 来嵌入多个文本用于索引。此方法内部使用的方式（但不限于）可能与嵌入查询的方式不同：

```typescript
const text2 = "LangGraph 是一个用于使用 LLM 构建有状态、多智能体应用程序的库";
...
有关所有 `GoogleGenerativeAIEmbeddings` 功能和配置的详细文档，请前往[API 参考](https://api.js.langchain.com/classes/langchain_google_genai.GoogleGenerativeAIEmbeddings.html)。