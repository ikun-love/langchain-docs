```yaml
---
title: 字节跳动豆包嵌入模型 (ByteDanceDoubaoEmbeddings)
---

本文将帮助您开始使用 LangChain 中的字节跳动豆包 [嵌入模型](/oss/integrations/text_embedding)。有关 `ByteDanceDoubaoEmbeddings` 功能和配置选项的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/_langchain_community.embeddings_bytedance_doubao.ByteDanceDoubaoEmbeddings.html)。

## 概述

### 集成详情

| 类名 | 包名 | 本地 | Py 支持 | 下载量 | 版本 |
| :--- | :--- | :---: | :---: |  :---: | :---: |
...
您需要注册一个 [ARK API 密钥](https://console.volcengine.com/ark/region:ark+cn-beijing/apiKey)，并将其设置为名为 `ARK_API_KEY` 的环境变量。然后，您应该为嵌入模型 [创建一个入口点 (entrypoint)](https://console.volcengine.com/ark/region:ark+cn-beijing/endpoint)，并使用该入口点的名称作为 `model` 的值。

接下来，您需要安装 [`@langchain/community`](https://www.npmjs.com/package/@langchain/community) 包。

### 凭证 (Credentials)

如果您希望对模型调用进行自动追踪，您也可以取消注释以下内容来设置您的 [LangSmith](https://docs.langchain.com/langsmith/home) API 密钥：
...
LangChain 字节跳动豆包嵌入模型的集成位于 `@langchain/community` 包中：
...
const embeddings = new ByteDanceDoubaoEmbeddings({
  model: 'ep-xxx-xxx' // 您的入口点名称
});
```

## 索引和检索

嵌入模型通常用于检索增强生成 (RAG) 流程中，既作为数据索引的一部分，也用于后续的检索步骤。有关更详细的说明，请参阅 [**学习 (Learn)** 标签页](/oss/learn/) 下的 RAG 教程。

下面展示了如何使用我们上面初始化的 `embeddings` 对象来进行数据的索引和检索。在这个示例中，我们将使用演示用的 [`MemoryVectorStore`](/oss/integrations/vectorstores/memory) 索引和检索一个示例文档。
...
在底层，向量存储和检索器的实现分别调用 `embeddings.embedDocument(...)` 和 `embeddings.embedQuery(...)` 方法来为 `fromDocuments` 中使用的文本和检索器的 `invoke` 操作创建嵌入向量。
...
您可以使用 `embedDocuments` 来嵌入多个文本，用于数据索引。此方法所使用的内部机制可能（但不一定）与嵌入查询的方法不同：
...
有关 `ByteDanceDoubaoEmbeddings` 所有功能和配置的详细文档，请访问 [API 参考](https://api.js.langchain.com/classes/_langchain_community.embeddings_bytedance_doubao.ByteDanceDoubaoEmbeddings.html)。
```