...
这将帮助您开始使用 LangChain 中的 TogetherAIEmbeddings [嵌入模型](/oss/integrations/text_embedding)。有关 `TogetherAIEmbeddings` 功能和配置选项的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_community_embeddings_togetherai.TogetherAIEmbeddings.html)。

## 概述

### 集成详情

| 类 | 包 | 本地 | [Py 支持](https://python.langchain.com/docs/integrations/text_embedding/together/) | 下载量 | 版本 |
| :--- | :--- | :---: | :---: |  :---: | :---: |
...
您可以在 [此处](https://api.together.xyz/) 注册一个 Together 账户并创建一个 API 密钥。完成此操作后，设置 `TOGETHER_AI_API_KEY` 环境变量：

```bash
export TOGETHER_AI_API_KEY="your-api-key"
```

如果您想对模型的调用进行自动追踪，您还可以取消注释下方代码以设置您的 [LangSmith](https://docs.langchain.com/langsmith/home) API 密钥：
...
const embeddings = new TogetherAIEmbeddings({
  model: "togethercomputer/m2-bert-80M-8k-retrieval", // 默认值
});
```

## 索引和检索

嵌入模型通常用于检索增强生成 (RAG) 流程中，既可以作为数据索引的一部分，也可以用于后续的检索。有关更详细的说明，请参阅 [**学习** 选项卡](/oss/learn/) 下方的 RAG 教程。

下面展示了如何使用我们上面初始化的 `embeddings` 对象来索引和检索数据。在这个示例中，我们将使用演示用的 [`MemoryVectorStore`](/oss/integrations/vectorstores/memory) 来索引和检索一个示例文档。

```typescript
// 使用示例文本创建一个向量存储
import { MemoryVectorStore } from "@langchain/classic/vectorstores/memory";

const text = "LangChain 是用于构建上下文感知推理应用的框架";
...
您可以使用 `embedDocuments` 来嵌入多个文本用于索引。此方法使用的内部机制可能（但不一定需要）与嵌入查询的方法不同：

```typescript
const text2 = "LangGraph 是一个用于使用 LLM 构建有状态、多参与者应用的库";
...
有关 TogetherAIEmbeddings 所有功能和配置的详细文档，请前往 [API 参考](https://api.js.langchain.com/classes/langchain_community_embeddings_togetherai.TogetherAIEmbeddings.html)。