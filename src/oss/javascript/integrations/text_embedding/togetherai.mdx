...
这将帮助您使用 LangChain 入门 TogetherAIEmbeddings [嵌入模型](/oss/integrations/text_embedding)。有关 `TogetherAIEmbeddings` 特性和配置选项的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_community_embeddings_togetherai.TogetherAIEmbeddings.html)。
...
嵌入模型通常用于检索增强生成 (RAG) 流程中，既可用于数据索引，也可用于后续的检索。有关更详细的指导，请参阅 [**学习 (Learn)** 选项卡](/oss/learn/) 下的 RAG 教程。

下面展示了如何使用我们上面初始化的 `embeddings` 对象来索引和检索数据。在此示例中，我们将使用演示用的 [`MemoryVectorStore`](/oss/integrations/vectorstores/memory) 对一个示例文档进行索引并进行检索。
