...
要访问 OpenAIEmbeddings 嵌入模型，您需要创建一个 OpenAI 账户，获取一个 API 密钥，并安装 `@langchain/openai` 集成包。
...
const embeddings = new OpenAIEmbeddings({
  apiKey: "YOUR-API-KEY", // 在 Node.js 中，默认使用 process.env.OPENAI_API_KEY
  batchSize: 512, // 如果省略，默认值为 512。最大值为 2048
  model: "text-embedding-3-large",
});
```

如果您属于某个组织，您可以将 `process.env.OPENAI_ORGANIZATION` 设置为您 OpenAI 的组织 ID，或者在初始化模型时将其作为 `organization` 参数传入。

## 索引和检索

嵌入模型通常用于检索增强生成 (RAG) 流程中，既可以作为数据索引的一部分，也可以在后续检索时使用。有关更详细的说明，请参阅 [**学习 (Learn)** 选项卡下的 RAG 教程](/oss/learn/)。

在下面，您可以查看如何使用我们上面初始化的 `embeddings` 对象来进行数据索引和检索。在此示例中，我们将使用演示用的 [`MemoryVectorStore`](/oss/integrations/vectorstores/memory) 对样本文档进行索引和检索。
...
您可以使用 `embedQuery` 来嵌入查询（queries）用于搜索。这会生成一个特定于查询的向量表示：
...
您可以使用 `embedDocuments` 嵌入多个文本用于索引。此方法的内部处理方式可能（但不一定）与嵌入查询的处理方式不同：
...
对于 `text-embedding-3` 系列模型，您可以指定返回嵌入向量的大小（维度）。例如，默认情况下 `text-embedding-3-large` 返回维度为 3072 的嵌入向量：
...
通过传入 `dimensions: 1024`，我们可以将嵌入向量的大小减小到 1024：
...
您可以像这样传递 `configuration` 参数来定制 SDK 发送请求的基础 URL：
...
如果您在 Azure OpenAI 上托管服务，请参阅[专门的页面](/oss/integrations/text_embedding/azure_openai)。
