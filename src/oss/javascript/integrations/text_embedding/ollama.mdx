...
这将帮助您开始使用 LangChain 中的 Ollama [嵌入模型](/oss/integrations/text_embedding)。有关 `OllamaEmbeddings` 特性和配置选项的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_ollama.OllamaEmbeddings.html)。

## 概述

### 集成详情

| 类名 | 包名 | 本地 | [Python 支持](https://python.langchain.com/docs/integrations/text_embedding/ollama/) | 下载量 | 版本 |
| :--- | :--- | :---: | :---: |  :---: | :---: |
...
如果您希望对模型调用进行自动追踪，您可以通过取消注释下面的内容来设置您的 [LangSmith](https://docs.langchain.com/langsmith/home) API 密钥：
...
嵌入模型通常用于检索增强生成 (RAG) 流程中，既可用于数据索引，也可用于后续检索。有关更详细的说明，请参阅 [**学习 (Learn)** 选项卡下的 RAG 教程](/oss/learn/)。

下面展示了如何使用上面初始化的 `embeddings` 对象进行数据索引和检索。在此示例中，我们将使用演示用的 [`MemoryVectorStore`](/oss/integrations/vectorstores/memory) 来索引和检索示例文档。
...
在底层，向量存储和检索器的实现在调用 `fromDocuments` 时会调用 `embeddings.embedDocument(...)` 来创建文本的嵌入，在调用检索器的 `invoke` 操作时会调用 `embeddings.embedQuery(...)` 来创建查询的嵌入。

您可以直接调用这些方法来为您的用例获取嵌入向量。
...
您可以使用 `embedDocuments` 嵌入多个文本用于索引。该方法的内部机制可能（但不一定）不同于嵌入查询：
...
如需了解所有 `OllamaEmbeddings` 特性和配置的详细文档，请访问[API 参考](https://api.js.langchain.com/classes/langchain_ollama.OllamaEmbeddings.html)