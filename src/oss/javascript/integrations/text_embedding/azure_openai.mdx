...
[Azure OpenAI](https://azure.microsoft.com/products/ai-services/openai-service/) 是一项云服务，可帮助您使用来自 OpenAI、Meta 等公司的各种预构建和精选模型，快速开发生成式 AI 体验。

LangChain.js 使用 [OpenAI SDK](https://github.com/openai/openai-node) 中的新 Azure 集成，支持与 [Azure OpenAI](https://azure.microsoft.com/products/ai-services/openai-service/) 集成。

您可以在 [此页面](https://learn.microsoft.com/azure/ai-services/openai/overview) 上了解有关 Azure OpenAI 及其与 OpenAI API 区别的更多信息。如果您还没有 Azure 帐户，可以 [创建一个免费帐户](https://azure.microsoft.com/free/) 开始使用。

这将帮助您使用 LangChain 入门了解 AzureOpenAIEmbeddings [嵌入模型](/oss/integrations/text_embedding)。有关 `AzureOpenAIEmbeddings` 特性和配置选项的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_openai.AzureOpenAIEmbeddings.html)。


<Info>
**以前，LangChain.js 使用专用的 [Azure OpenAI SDK](https://github.com/Azure/azure-sdk-for-js/tree/main/sdk/openai/openai) 支持与 Azure OpenAI 集成。现在已弃用该 SDK，转而推荐使用 OpenAI SDK 中的新 Azure 集成，这样可以立即访问最新的 OpenAI 模型和功能，并实现 OpenAI API 和 Azure OpenAI 之间的无缝过渡。**


如果您正在使用已弃用的 SDK 和 Azure OpenAI，请参阅[迁移指南](#migration-from-azure-openai-sdk)以更新到新 API。
...
| 类 | 包 | 本地 | [Py 支持](https://python.langchain.com/docs/integrations/text_embedding/azure_openai/) | 下载量 | 版本 |
| :--- | :--- | :---: | :---: |  :---: | :---: |
...
您需要部署一个 Azure OpenAI 实例。您可以通过遵循[本指南](https://learn.microsoft.com/azure/ai-services/openai/how-to/create-resource?pivots=web-portal)在 Azure 门户上部署一个版本。

实例运行后，请确保您拥有实例名称和密钥。您可以在 Azure 门户中，在实例的“密钥和终结点 (Keys and Endpoint)”部分找到密钥。
...
如果您想获取模型调用的自动化跟踪，也可以通过取消注释以下行来设置 [LangSmith](https://docs.langchain.com/langsmith/home) API 密钥：
...
嵌入模型通常用于检索增强生成 (RAG) 流程中，既作为数据索引的一部分，也用于后续的检索。有关更详细的说明，请参阅[“学习 (Learn)”选项卡](/oss/learn/)下的 RAG 教程。

下面展示了如何使用我们上面初始化的 `embeddings` 对象来索引和检索数据。在此示例中，我们将使用演示 [`MemoryVectorStore`](/oss/integrations/vectorstores/memory) 来索引和检索示例文档。
...
在底层，向量存储和检索器的实现会调用 `embeddings.embedDocument(...)` 和 `embeddings.embedQuery(...)` 来为 `fromDocuments` 中使用的文本和检索器的 `invoke` 操作中使用的文本创建嵌入向量。

您可以直接调用这些方法来获取用于您自己用例的嵌入向量。

### 嵌入单个文本

您可以使用 `embedQuery` 嵌入用于搜索的查询。这将生成一个特定于查询的向量表示：
...
您可以使用 `embedDocuments` 嵌入多个文本用于索引。此方法的内部机制可能（但不一定）与嵌入查询的方法不同：
...
您可以传递一个 `configuration` 字段来指定自定义标头：
...
**注意：** 具体的 `api-key` 标头目前无法通过此方式覆盖，它将传递 `azureOpenAIApiKey` 的值。

## 从 Azure OpenAI SDK 迁移

如果您正在使用 `@langchain/azure-openai` 包及其已弃用的 Azure OpenAI SDK，可以按照以下步骤更新代码以使用新的 Azure 集成：

1. 安装新的 `@langchain/openai` 包并删除以前的 `@langchain/azure-openai` 包：
...
2. 将导入语句更新为使用来自 `@langchain/openai` 包的新 `AzureOpenAIEmbeddings` 类：
...
3. 更新您的代码以使用新的 `AzureOpenAIEmbeddings` 类并传递所需的参数：
   ...
   - 如果您之前使用的是 Azure 托管身份，现在需要使用构造函数中的 `azureADTokenProvider` 参数而不是 `credentials`，有关更多详细信息请参阅[Azure 托管身份](#using-azure-managed-identity)部分。

   - 如果您使用的是环境变量，现在必须设置 `AZURE_OPENAI_API_INSTANCE_NAME` 环境变量而不是 `AZURE_OPENAI_API_ENDPOINT`，并添加 `AZURE_OPENAI_API_VERSION` 环境变量以指定 API 版本。

---

## API 参考

有关所有 AzureOpenAIEmbeddings 特性和配置的详细文档，请前往[API 参考](https://api.js.langchain.com/classes/langchain_openai.AzureOpenAIEmbeddings.html)。