...
本文将帮助您开始使用 [Watsonx 文档压缩器](/oss/concepts/#document_compressors)。如需了解所有 Watsonx 文档压缩器功能和配置的详细文档，请前往 [API 参考](https://api.js.langchain.com/modules/_langchain_community.document_compressors_ibm.html)。

### 集成详情

| 类名 | 包 | [PY 支持](https://python.langchain.com/docs/integrations/retrievers/ibm_watsonx_ranker/) | 下载量 | 版本 |
| :--- | :--- | :---: | :---: | :---: |
...
一旦将这些变量设置到环境中并初始化对象，身份验证将自动进行。

也可以通过将这些值作为参数传递给新实例的方式来完成身份验证。
...
如果您想从单个查询中获得自动化的追踪（tracing），您也可以通过取消注释以下代码来设置您的 [LangSmith](https://docs.langchain.com/langsmith/home) API 密钥：
...
首先，设置一个基本的 RAG（检索增强生成）摄取管道，包括嵌入 (embeddings)、文本分割器 (text splitter) 和向量存储 (vector store)。我们将使用它来针对选定的查询对文档进行重排序 (rerank)：
...
将选定的文档传递给重排序器 (reranker)，并为每个文档接收特定的分数：
...
或者，您可以使用 `.compressDocuments()` 方法将文档与结果一起返回，如下所示：
...
如需了解所有 Watsonx 文档压缩器功能和配置的详细文档，请前往 [API 参考](https://api.js.langchain.com/modules/_langchain_community.document_compressors_ibm.html)。