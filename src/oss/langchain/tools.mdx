...
工具是 [智能体 (agents)](/oss/langchain/agents) 调用的组件，用于执行操作。它们通过允许模型通过明确定义的输入和输出来与世界进行交互，从而扩展了模型的 [能力](/oss/langchain/agents)。

工具封装了一个可调用的函数及其输入 schema（模式）。这些工具可以传递给兼容的 [聊天模型](/oss/langchain/models)，使模型能够决定是否调用工具以及使用哪些参数。在这些场景中，工具调用允许模型生成符合指定输入模式的请求。

<Note>
**服务端工具使用**

一些聊天模型（例如 [OpenAI](/oss/integrations/chat/openai)、[Anthropic](/oss/integrations/chat/anthropic) 和 [Gemini](/oss/integrations/chat/google_generative_ai)）具有在服务端执行的[内置工具](/oss/langchain/models#server-side-tool-use)，例如网络搜索和代码解释器。请参阅[提供商概览](/oss/integrations/providers/overview)，了解如何使用特定聊天模型访问这些工具。
</Note>
...
<Note>
如果您在工具中使用 `runtime.stream_writer`，则必须在 LangGraph 执行上下文内调用该工具。有关更多详细信息，请参阅[流式传输](/oss/langchain/streaming)。
</Note>
:::
