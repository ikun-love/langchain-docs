...
<Tip>
    如果使用 `langchain>=1.1`，`trigger` 和 `keep` 的 `fraction` 条件依赖于聊天模型的 [模型配置文件](/oss/langchain/models#model-profiles)。如果数据不可用，请使用其他条件或手动指定：
...
<ParamField body="model" type="string | BaseChatModel" required>
    用于生成摘要的模型。可以是模型标识符字符串（例如，`'openai:gpt-4o-mini'`）或 `BaseChatModel` 实例。有关更多信息，请参阅 @[`init_chat_model`][init_chat_model(model)]。
</ParamField>
...
:::js
<Tip>
    如果使用 `langchain@1.1.0`，`trigger` 和 `keep` 的 `fraction` 条件依赖于聊天模型的 [模型配置文件](/oss/langchain/models#model-profiles)。如果数据不可用，请使用其他条件或手动指定：
    ```typescript
...
在工具调用执行之前，暂停 Agent 执行以供人工批准、编辑或拒绝工具调用。[人工干预](/oss/langchain/human-in-the-loop) 在以下场景中很有用：
...
<Warning>
    人工干预中间件需要一个 [检查点 (checkpointer)](/oss/langgraph/persistence#checkpoints) 来在中断期间维护状态。
</Warning>
...
<Tip>
    有关完整的示例、配置选项和集成模式，请参阅 [人工干预文档](/oss/langchain/human-in-the-loop)。
</Tip>
...
:::js
<Callout icon="circle-play" iconType="solid">
    观看此 [视频指南](https://www.youtube.com/watch?v=tdOeUVERukA)，了解人工干预中间件的行为。
</Callout>
:::
...
<ParamField body="threadLimit" type="number">
    线程中所有运行的总工具调用次数限制（即对话）。在具有相同线程 ID 的多次调用中保持不变。需要检查点 (checkpointer) 来维护状态。`undefined` 表示无线程限制。
</ParamField>

<ParamField body="runLimit" type="number">
    单次调用（一个用户消息 → 响应周期）的最大工具调用次数。每次新用户消息都会重置。`undefined` 表示无运行限制。
...
<ParamField body="exitBehavior" type="string" default="continue">
    ...
    - `'continue'` (默认) - 使用错误消息阻止超出的工具调用，让其他工具和模型继续执行。模型根据错误消息决定何时结束。
    - `'error'` - 抛出 `ToolCallLimitExceededError` 异常，立即停止执行
    - `'end'` - 使用 ToolMessage 和 AI 消息立即停止执行，仅在限制单个工具时有效；如果其他工具仍有待处理的调用，则会抛出错误。
</ParamField>
:::
...
interface PIIMatch {
  text: string;    // 匹配到的文本
  start: number;   // 在 content 中的起始索引
  end: number;      // 在 content 中的结束索引
}
...
<ParamField body="maxTools" type="number">
    要选择的最大工具数量。如果模型选择的工具超过此数量，将只使用前 `maxTools` 个。如果未指定，则无限制。
</ParamField>

<ParamField body="alwaysInclude" type="string[]">
    无论选择如何都应包含的工具名称。这些不计入 `maxTools` 限制。
</ParamField>
:::
...
<ParamField body="maxRetries" type="number" default="2">
    初始调用后最大重试次数（默认总共 3 次尝试）。必须 >= 0。
</ParamField>

<ParamField body="tools" type="(ClientTool | ServerTool | string)[]">
    可选的工具或工具名称数组，用于应用重试逻辑。可以是 `BaseTool` 实例或工具名称字符串列表。如果为 `undefined`，则应用于所有工具。
</ParamField>

<ParamField body="retryOn" type="((error: Error) => boolean) | (new (...args: any[]) => Error)[]" default="() => true">
    应重试的错误构造函数数组，或一个接受错误并返回 `true` 表示应重试的函数。默认为重试所有错误。
</ParamField>
...
<ParamField body="onFailure" type="'error' | 'continue' | ((error: Error) => string)" default="continue">
    ...
    - 自定义函数 - 一个接受异常并返回 `ToolMessage` 内容字符串的函数，允许自定义错误格式化
...
<ParamField body="backoffFactor" type="number" default="2.0">
    指数退避的乘数。每次重试等待时间为 `initialDelayMs * (backoffFactor ** retryNumber)` 毫秒。设为 `0.0` 表示恒定延迟。必须 >= 0。
</ParamField>
...
<ParamField body="onFailure" type="'error' | 'continue' | ((error: Error) => string)" default="continue">
    ...
    - `'continue'` (默认) - 返回包含错误详情的 `AIMessage`，允许 Agent 潜在地处理失败
    - `'error'` - 重新抛出异常，停止 Agent 执行
    - 自定义函数 - 一个接受异常并返回 `AIMessage` 内容字符串的函数，允许自定义错误格式化
</ParamField>
...
<ParamField body="tools" type="(string | ClientTool | ServerTool)[]">
    要模拟的工具名称（string）或工具实例列表。如果为 `undefined`（默认），则模拟所有工具。如果为空数组 `[]`，则不模拟任何工具。如果包含工具名称/实例的数组，则仅模拟这些工具。
</ParamField>
