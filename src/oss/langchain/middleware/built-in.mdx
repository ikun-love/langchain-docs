...
LangChain 提供针对常见用例的预构建（Prebuilt）中间件。每个中间件都已准备好投入生产环境，并可根据您的具体需求进行配置。
...
| 中间件 | 描述 |
|------------|-------------|
| [摘要 (Summarization)](#summarization) | 在接近 Token 限制时自动总结对话历史。 |
| [人机协同 (Human-in-the-loop)](#human-in-the-loop) | 在工具调用执行前暂停执行，等待人工批准。 |
| [模型调用限制 (Model call limit)](#model-call-limit) | 限制模型调用次数，以防止产生过高的费用。 |
| [工具调用限制 (Tool call limit)](#tool-call-limit) | 通过限制调用次数来控制工具执行。 |
| [模型回退 (Model fallback)](#model-fallback) | 当主模型失败时，自动回退到备用模型。 |
| [PII 检测 (PII detection)](#pii-detection) | 检测并处理个人身份信息 (PII)。 |
| [待办事项列表 (To-do list)](#to-do-list) | 为 Agent 提供任务规划和跟踪能力。 |
| [LLM 工具选择器 (LLM tool selector)](#llm-tool-selector) | 在调用主模型之前，使用 LLM 来选择相关的工具。 |
| [工具重试 (Tool retry)](#tool-retry) | 使用指数退避（exponential backoff）策略自动重试失败的工具调用。 |
| [模型重试 (Model retry)](#model-retry) | 使用指数退避策略自动重试失败的模型调用。 |
| [LLM 工具模拟器 (LLM tool emulator)](#llm-tool-emulator) | 使用 LLM 模拟工具执行，用于测试目的。 |
| [上下文编辑 (Context editing)](#context-editing) | 通过修剪或清除工具使用记录来管理对话上下文。 |
| [Shell 工具 (Shell tool)](#shell-tool) | 向 Agent 暴露一个持久化的 Shell 会话，用于命令执行。 |
| [文件搜索 (File search)](#file-search) | 提供文件系统文件的 Glob 和 Grep 搜索工具。 |
...
在接近 Token 限制时自动总结对话历史，保留最近的消息同时压缩旧的上下文。摘要功能在以下情况很有用：
- 超过上下文窗口的长期对话。
- 具有广泛历史记录的多轮对话。
- 需要保留完整对话上下文的应用程序。
...
:::python
<Tip>
    如果使用 `langchain>=1.1`，`trigger` 和 `keep` 的 `fraction` 条件依赖于聊天模型的[模型配置数据](/oss/langchain/models#model-profiles)。如果数据不可用，请使用其他条件或手动指定：
...
<ParamField body="model" type="string | BaseChatModel" required>
    用于生成摘要的模型。可以是一个模型标识符字符串（例如 `'openai:gpt-4o-mini'`）或一个 `BaseChatModel` 实例。有关更多信息，请参阅 @[`init_chat_model`][init_chat_model(model)]。
</ParamField>

<ParamField body="trigger" type="ContextSize | list[ContextSize] | None">
    触发摘要的条件。可以是：

    - 单个 @[`ContextSize`] 字典（所有属性必须满足 - AND 逻辑）
    - @[`ContextSize`] 字典列表（任何条件满足即可 - OR 逻辑）
...
<ParamField body="summary_prompt" type="string">
    用于摘要的自定义提示模板。如果未指定，则使用内置模板。模板应包含 `{messages}` 占位符，对话历史将插入到此处。
</ParamField>

<ParamField body="trim_tokens_to_summarize" type="number" default="4000">
    生成摘要时包含的最大 Token 数。在进行摘要之前，消息将被修剪以适应此限制。
</ParamField>

<ParamField body="summary_prefix" type="string">
    添加到摘要消息中的前缀。如果未提供，则使用默认前缀。
</ParamField>
...
:::js
<Tip>
    如果使用 `langchain@1.1.0`，`trigger` 和 `keep` 的 `fraction` 条件依赖于聊天模型的[模型配置数据](/oss/langchain/models#model-profiles)。如果数据不可用，请使用其他条件或手动指定：
    ```typescript
...
<ParamField body="model" type="string | BaseChatModel" required>
    用于生成摘要的模型。可以是一个模型标识符字符串（例如 `'openai:gpt-4o-mini'`）或一个 `BaseChatModel` 实例。
</ParamField>
...
摘要中间件会监控消息 Token 计数，并在达到阈值时自动总结较早的消息，同时保留最近的消息并压缩较旧的上下文。
...
# 单一条件：当 tokens >= 4000 且 messages >= 10 时触发
agent = create_agent(
    model="gpt-4o",
    tools=[your_weather_tool, your_calculator_tool],
    middleware=[
...
在工具调用执行前暂停 Agent 执行，以供人工批准、编辑或拒绝工具调用。[人机协同](/oss/langchain/human-in-the-loop) 在以下情况很有用：

- 需要人工批准的高风险操作（例如：数据库写入、金融交易）。
- 强制要求人工监督的合规工作流程。
- 依赖人工反馈来指导 Agent 的长期对话。
...
<Warning>
    人机协同中间件需要一个[检查点 (checkpointer)](/oss/langgraph/persistence#checkpoints) 来在中断期间维护状态。
</Warning>
...
<Tip>
    有关完整的示例、配置选项和集成模式，请参阅[人机协同文档](/oss/langchain/human-in-the-loop)。
</Tip>

:::python
<Callout icon="circle-play" iconType="solid">
    观看此[视频指南](https://www.youtube.com/watch?v=SpfT6-YAVPk)，了解人机协同中间件的行为。
</Callout>
:::

:::js
<Callout icon="circle-play" iconType="solid">
    观看此[视频指南](https://www.youtube.com/watch?v=tdOeUVERukA)，了解人机协同中间件的行为。
</Callout>
:::

### Model call limit (模型调用限制)

限制模型调用的次数，以防止无限循环或产生过多费用。模型调用限制在以下情况很有用：

- 防止失控的 Agent 进行过多的 API 调用。
- 在生产部署中实施成本控制。
- 在特定调用预算内测试 Agent 的行为。
...
通过限制工具调用的次数来控制 Agent 的执行，可以全局限制或针对特定工具进行限制。工具调用限制在以下情况很有用：
...
<ParamField body="thread_limit" type="number">
    线程中所有运行的总工具调用次数限制（即对话中）。跨具有相同线程 ID 的多次调用持久化。需要检查点才能维护状态。`None` 表示无线程限制。
</ParamField>

<ParamField body="run_limit" type="number">
    单次调用（一次用户消息 → 响应周期）的最大工具调用次数。每次新用户消息都会重置。`None` 表示无运行限制。
...
<ParamField body="exit_behavior" type="string" default="continue">
    ...
    - `'end'` - 立即使用 `ToolMessage` 和 AI 消息停止执行，对应超限的工具调用。仅当限制单个工具时有效；如果其他工具有待处理的调用，则会引发 `NotImplementedError`。
</ParamField>
:::
...
<ParamField body="threadLimit" type="number">
    线程中所有运行的总工具调用次数限制（即对话中）。跨具有相同线程 ID 的多次调用持久化。需要检查点才能维护状态。`undefined` 表示无线程限制。
</ParamField>

<ParamField body="runLimit" type="number">
    单次调用（一次用户消息 → 响应周期）的最大工具调用次数。每次新用户消息都会重置。`undefined` 表示无运行限制。
...
<ParamField body="exitBehavior" type="string" default="continue">
    ...
    - `'end'` - 立即使用 ToolMessage 和 AI 消息停止执行，对应超限的工具调用。仅当限制单个工具时有效；如果其他工具有待处理的调用，则会抛出错误。
</ParamField>
:::
...
退出行为：
- `'continue'`（默认）- 使用错误消息阻止超限调用，Agent 继续执行
- `'error'` - 立即引发异常
...
当主模型失败时，自动回退到备用模型。模型回退在以下情况很有用：
...
使用可配置的策略检测和处理对话中的个人身份信息 (PII)。PII 检测在以下情况很有用：
...
您可以提供 `detector` 参数来创建自定义的 PII 类型。这允许您检测超出内置类型的特定于用例的模式。
...
<Tip>
    ...
    - 使用正则表达式字符串进行简单模式匹配
    - 当你需要标志（例如忽略大小写的匹配）时，使用 RegExp 对象
    - 当你需要超出模式匹配的验证逻辑时，使用自定义函数
    - 自定义函数提供了对检测逻辑的完全控制，可以实现复杂的验证规则
</Tip>
...
<ParamField body="detector" type="function | regex">
    自定义检测函数或正则表达式模式。如果未提供，则使用内置 PII 类型的检测器。
</ParamField>
...
<ParamField body="detector" type="RegExp | string | function">
    ...
    - `function` - 自定义检测函数 `(content: string) => PIIMatch[]`

    如果未提供，则使用内置 PII 类型的检测器。
</ParamField>
...
为 Agent 提供任务规划和跟踪能力，以处理复杂的多步骤任务。待办事项列表在以下情况很有用：
...
:::js
<Callout icon="circle-play" iconType="solid">
    观看此[视频指南](https://www.youtube.com/watch?v=dwvhZ1z_Pas)，了解待办事项列表中间件的行为。
</Callout>
:::
...
使用 LLM 在调用主模型之前智能地选择相关工具。LLM 工具选择器在以下情况很有用：

- 工具数量很多（10 个以上）且大部分工具与查询不相关的 Agent。
...
<ParamField body="model" type="string | BaseChatModel">
    用于工具选择的模型。可以是一个模型标识符字符串（例如 `'openai:gpt-4o-mini'`）或一个 `BaseChatModel` 实例。有关更多信息，请参阅 @[`init_chat_model`][init_chat_model(model)]。
...
<ParamField body="max_tools" type="number">
    要选择的最大工具数量。如果模型选择的工具超过此数量，则只使用前 max_tools 个。如果未指定，则无限制。
</ParamField>

<ParamField body="always_include" type="list[string]">
    无论选择如何都始终包含的工具名称。这些不计入 max_tools 的限制。
</ParamField>
:::

:::js
<ParamField body="model" type="string | BaseChatModel">
    用于工具选择的模型。可以是一个模型标识符字符串（例如 `'openai:gpt-4o-mini'`）或一个 `BaseChatModel` 实例。默认为 Agent 的主模型。
</ParamField>
...
<ParamField body="maxTools" type="number">
    要选择的最大工具数量。如果模型选择的工具超过此数量，则只使用前 maxTools 个。如果未指定，则无限制。
</ParamField>

<ParamField body="alwaysInclude" type="string[]">
    无论选择如何都始终包含的工具名称。这些不计入 maxTools 的限制。
</ParamField>
:::
...
使用可配置的指数退避策略自动重试失败的工具调用。工具重试在以下情况很有用：

- 处理外部 API 调用中的瞬态（临时）故障。
- 提高网络依赖型工具的可靠性。
...
<ParamField body="tools" type="list[BaseTool | str]">
    可选列表，指定要应用重试逻辑的工具或工具名称。如果为 `None`，则应用于所有工具。
</ParamField>

<ParamField body="retry_on" type="tuple[type[Exception], ...] | callable" default="(Exception,)">
    要么是应重试的异常类型元组，要么是一个接受异常并返回 `True`（如果应重试）的可调用对象。
</ParamField>
...
<ParamField body="jitter" type="boolean" default="true">
    是否添加随机抖动（`±25%`）延迟以避免“惊群效应”(thundering herd)
</ParamField>
:::
...
<ParamField body="tools" type="(ClientTool | ServerTool | string)[]">
    可选数组，指定要应用重试逻辑的工具或工具名称。可以是 `BaseTool` 实例或工具名称字符串列表。如果为 `undefined`，则应用于所有工具。
</ParamField>

<ParamField body="retryOn" type="((error: Error) => boolean) | (new (...args: any[]) => Error)[]" default="() => true">
    要么是应重试的错误构造函数数组，要么是一个接受错误并返回 `true`（如果应重试）的函数。默认为重试所有错误。
</ParamField>
...
<ParamField body="onFailure" type="'error' | 'continue' | ((error: Error) => string)" default="continue">
    ...
    - 自定义函数 - 接受异常并返回一个字符串作为 `ToolMessage` 内容的函数，允许自定义错误格式

    **已弃用值:** `'raise'` (使用 `'error'` 代替) 和 `'return_message'` (使用 `'continue'` 代替)。这些已弃用的值仍然有效，但会显示警告。
</ParamField>
...
使用可配置的指数退避策略自动重试失败的模型调用。模型重试在以下情况很有用：

- 处理模型 API 调用中的瞬态（临时）故障。
- 提高网络依赖型模型请求的可靠性。
...
<ParamField body="on_failure" type="string | callable" default="continue">
    所有重试耗尽时的行为。选项：
    - `'continue'`（默认）- 返回包含错误详情的 `AIMessage`（允许 Agent 潜在地优雅地处理失败）
    - `'error'` - 重新抛出异常（停止 Agent 执行）
...
<ParamField body="onFailure" type="'error' | 'continue' | ((error: Error) => string)" default="continue">
    ...
    - `'continue'`（默认）- 返回包含错误详情的 `AIMessage`，允许 Agent 潜在地优雅地处理失败
    - `'error'` - 重新抛出异常，停止 Agent 执行
    - 自定义函数 - 接受异常并返回一个字符串作为 `AIMessage` 内容的函数，允许自定义错误格式
</ParamField>
...
retry_continue = ModelRetryMiddleware(
    max_retries=4,
    on_failure="continue",  # 失败时返回 AIMessage 错误详情而不是引发异常
)
...
const retryContinue = modelRetryMiddleware({
  maxRetries: 4,
  onFailure: "continue", // 失败时返回 AIMessage 错误详情而不是抛出
});
...
使用 LLM 模拟工具执行，而不是实际执行工具调用，从而用于测试目的。LLM 工具模拟器在以下情况很有用：

- 在不执行真实工具的情况下测试 Agent 的行为。
- 在外部工具不可用或昂贵时开发 Agent。
- 在实现实际工具之前原型设计 Agent 工作流程。
...
<ParamField body="tools" type="list[str | BaseTool]">
    要模拟的工具名称（str）或 BaseTool 实例列表。如果为 `None`（默认），将模拟**所有**工具。如果为空列表 `[]`，则不模拟任何工具。如果包含工具名称/实例的数组，则仅模拟这些工具。
</ParamField>

<ParamField body="model" type="string | BaseChatModel">
    用于生成模拟工具响应的模型。可以是模型标识符字符串（例如 `'anthropic:claude-sonnet-4-5-20250929'`）或 `BaseChatModel` 实例。如果未指定，则默认为 Agent 的模型。有关更多信息，请参阅 @[`init_chat_model`][init_chat_model(model)]。
</ParamField>
:::

:::js
<ParamField body="tools" type="(string | ClientTool | ServerTool)[]">
    要模拟的工具名称（string）或工具实例列表。如果为 `undefined`（默认），将模拟**所有**工具。如果为空数组 `[]`，则不模拟任何工具。如果包含工具名称/实例的数组，则仅模拟这些工具。
</ParamField>

<ParamField body="model" type="string | BaseChatModel">
    用于生成模拟工具响应的模型。可以是模型标识符字符串（例如 `'anthropic:claude-sonnet-4-5-20250929'`）或 `BaseChatModel` 实例。如果未指定，则默认为 Agent 的模型。
</ParamField>
:::
...
该中间件使用 LLM 为工具调用生成合理（Plausible）的响应，而不是执行实际工具。
...
当达到 Token 限制时，通过清除较早的工具调用输出来管理对话上下文，同时保留最近的结果。这有助于在包含大量工具调用的长期对话中保持上下文窗口的可管理性。上下文编辑在以下情况很有用：

- 包含大量工具调用的长期对话，超过 Token 限制
...
<ParamField body="clear_at_least" type="number" default="0">
    运行编辑时要回收的最小 Token 数。如果设置为 0，则清除所需的一切。
</ParamField>
...
<ParamField body="clear_tool_inputs" type="boolean" default="False">
    是否清除 AI 消息上原始的工具调用参数。如果为 `True`，则工具调用参数将被替换为空对象。
</ParamField>
...
<ParamField body="placeholder" type="string" default="[cleared]">
    插入到已清除工具输出中的占位符文本。它会替换原始工具消息内容。
</ParamField>
:::
...
<ParamField body="clearToolInputs" type="boolean" default="false">
    是否清除 AI 消息上原始的工具调用参数。如果为 `true`，工具调用参数将被替换为空对象。
</ParamField>
...
向 Agent 暴露一个持久化的 Shell 会话，用于命令执行。Shell 工具中间件在以下情况很有用：
...
<Warning>
    **安全考虑**: 使用适当的执行策略（`HostExecutionPolicy`, `DockerExecutionPolicy`, 或 `CodexSandboxExecutionPolicy`）以匹配部署环境的安全要求。
</Warning>

<Note>
    **限制**: 持久化 Shell 会话目前不支持中断（人机协同）。我们预计未来会增加对该功能的支持。
</Note>
...
<ParamField body="workspace_root" type="str | Path | None">
    Shell 会话的基础目录。如果省略，Agent 启动时会创建一个临时目录，并在 Agent 结束时删除。
</ParamField>
...
<ParamField body="execution_policy" type="BaseExecutionPolicy | None">
    ...
    - `HostExecutionPolicy` - 完整主机访问（默认）；最适用于 Agent 已在容器或 VM 内部运行的可信环境
    - `DockerExecutionPolicy` - 为每次 Agent 运行启动一个单独的 Docker 容器，提供更强的隔离
...
<ParamField body="redaction_rules" type="tuple[RedactionRule, ...] | list[RedactionRule] | None">
    可选的清理规则，用于在将命令输出返回给模型前对其进行消毒
</ParamField>

<ParamField body="tool_description" type="str | None">
    注册的 Shell 工具描述的可选覆盖值
</ParamField>
...
<ParamField body="env" type="Mapping[str, Any] | None">
    可选的环境变量，用于提供给 Shell 会话。值在命令执行前被强制转换为字符串。
</ParamField>
...
该中间件为 Agent 提供了一个单一的持久化 Shell 会话，可用于按顺序执行命令。
...
提供对文件系统的 Glob 和 Grep 搜索工具。文件搜索中间件在以下情况很有用：
...
- 三种输出模式：`files_with_matches`（带匹配的文件）、`content`（内容） 或 `count`（计数）
...
## Provider-specific middleware (特定于提供商的中间件)
...
:::python
    ...
    <Card title="OpenAI" href="/oss/integrations/middleware/openai" icon="openai" arrow>
        用于 OpenAI 模型的（OpenAI models）内容审核中间件。
    </Card>
:::
