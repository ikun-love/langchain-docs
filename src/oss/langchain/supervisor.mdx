...
**主管模式**（Supervisor Pattern）是一种多代理（[multi-agent](/oss/langchain/multi-agent)）架构，其中一个中心化的主管代理协调专门的工人代理。当任务需要不同类型的专业知识时，这种方法表现出色。与其构建一个必须在所有领域管理工具选择的单一代理，不如创建聚焦的专家，由一个理解整体工作流程的主管来协调。
...
我们还将整合[人工干预审核](/oss/langchain/human-in-the-loop)机制，允许用户根据需要批准、编辑和拒绝操作（例如传出的电子邮件）。

### 为什么要使用主管？

多代理架构允许您将[工具](/oss/langchain/tools)划分给不同的工作代理，每个代理都有自己独立的提示词或指令。想象一个可以直接访问所有日历和电子邮件 API 的代理：它必须从许多相似的工具中进行选择，理解每个 API 的确切格式，并同时处理多个领域。如果性能下降，将相关工具和关联的提示词分离到逻辑组中可能是有益的（部分是为了管理迭代改进）。
...
- [人工干预审核](/oss/langchain/human-in-the-loop)
...
:::js
<Tip>
有关完整的工作示例，演示了包含人工干预审核和高级信息流控制的主管模式，请查看 LangChain.js 示例中的 [`supervisor_complete.ts`](https://github.com/langchain-ai/langchainjs/blob/main/examples/src/createAgent/supervisor_complete.ts)。
</Tip>
:::
...
对于只有少量工具的简单情况，请使用单个代理。当代理需要与用户进行对话时，请使用[交接（handoffs）](/oss/langchain/multi-agent#handoffs)代替。对于代理之间的点对点协作，请考虑其他多代理模式。
</Tip>

## 后续步骤

了解[交接（handoffs）](/oss/langchain/multi-agent#handoffs)以实现代理间的对话，探索[上下文工程](/oss/langchain/context-engineering)以微调信息流，阅读[多代理概述](/oss/langchain/multi-agent)以比较不同模式，并使用 [LangSmith](https://smith.langchain.com) 来调试和监控您的多代理系统。