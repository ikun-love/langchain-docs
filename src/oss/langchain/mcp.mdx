...
:::python
[模型上下文协议 (Model Context Protocol, MCP)](https://modelcontextprotocol.io/introduction) 是一个开放协议，用于标准化应用程序如何向大型语言模型 (LLM) 提供工具和上下文。LangChain 代理可以使用 [`langchain-mcp-adapters`](https://github.com/langchain-ai/langchain-mcp-adapters) 库中在 MCP 服务器上定义的工具。
:::
:::js
[模型上下文协议 (Model Context Protocol, MCP)](https://modelcontextprotocol.io/introduction) 是一个开放协议，用于标准化应用程序如何向大型语言模型 (LLM) 提供工具和上下文。LangChain 代理可以使用 [`@langchain/mcp-adapters`](https://github.com/langchain-ai/langchainjs/tree/main/libs/langchain-mcp-adapters) 库中在 MCP 服务器上定义的工具。
:::
...
`langchain-mcp-adapters` 使得代理能够使用在一个或多个 MCP 服务器上定义的工具。

<Note>
    `MultiServerMCPClient` 默认是**无状态的**。每一次工具调用都会创建一个新的 MCP `ClientSession`，执行工具，然后清理。有关更多详细信息，请参阅[有状态会话](#stateful-sessions)部分。
</Note>
...
async def get_weather(location: str) -> str:
    """获取指定地点的天气。"""
    return "It's always sunny in New York" # 纽约总是晴天
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
    return {
        ...
        {
            name: "add",
            description: "Add two numbers", // 加两个数
            inputSchema: {
                type: "object",
                properties: {
                    a: {
                        type: "number",
                        description: "First number", // 第一个数
                    },
                    b: {
                        type: "number",
                        description: "Second number", // 第二个数
                    },
                },
                required: ["a", "b"],
            },
        ...
        {
            name: "multiply",
            description: "Multiply two numbers", // 乘两个数
            inputSchema: {
                type: "object",
                properties: {
                    a: {
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
    return {
        ...
        {
            name: "get_weather",
            description: "Get weather for location", // 获取地点的天气
            inputSchema: {
            type: "object",
            properties: {
                location: {
                type: "string",
                description: "Location to get weather for", // 获取天气的地点
                },
            },
            required: ["location"],
            },
        ...
        case "get_weather": {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `It's always sunny in ${location}`, // 无论在哪个地点，纽约的天气总是晴朗的
                    },
                ],
            };
        }
...
MCP 支持不同的传输机制用于客户端和服务器之间的通信。

### HTTP

`http` 传输（也称为 `streamable-http`）使用 HTTP 请求进行客户端与服务器之间的通信。有关更多详细信息，请参阅 [MCP HTTP 传输规范](https://modelcontextprotocol.io/specification/2025-03-26/basic/transports#streamable-http)。
...
#### 传递请求头 (Passing headers)

:::python
当通过 HTTP 连接到 MCP 服务器时，您可以使用连接配置中的 `headers` 字段包含自定义请求头（例如，用于身份验证或跟踪）。这支持 `sse`（MCP 规范已弃用）和 `streamable_http` 传输。
...
:::python
`langchain-mcp-adapters` 库在底层使用官方 [MCP SDK](https://github.com/modelcontextprotocol/python-sdk)，允许您通过实现 `httpx.Auth` 接口来提供自定义的身份验证机制。
...
客户端启动服务器作为子进程，并通过标准输入/输出进行通信。最适合本地工具和简单设置。

<Note>
    与 HTTP 传输不同，`stdio` 连接具有内在的**有状态性**——子进程在客户端连接的生命周期内持续存在。但是，当使用 `MultiServerMCPClient` 而不进行显式会话管理时，每次工具调用仍然会创建一个新的会话。请参阅[有状态会话](#stateful-sessions)部分以管理持久连接。
</Note>
...
如果您需要控制 MCP 会话的 [生命周期](https://modelcontextprotocol.io/specification/2025-03-26/basic/lifecycle)（例如，当处理一个跨工具调用维护上下文的有状态服务器时），您可以使用 `client.session()` 创建一个持久的 `ClientSession`。
...
[工具](https://modelcontextprotocol.io/docs/concepts/tools) 允许 MCP 服务器公开可执行函数，LLM 可以调用这些函数来执行操作——例如查询数据库、调用 API 或与外部系统交互。LangChain 将 MCP 工具转换为 LangChain [工具](/oss/langchain/tools)，使其可以直接用于任何 LangChain 代理或工作流中。

#### 加载工具

使用 `client.get_tools()` 从 MCP 服务器检索工具并将它们传递给您的代理：
...
MCP 工具可以返回与人类可读文本响应相伴的 [结构化内容](https://modelcontextprotocol.io/specification/2025-03-26/server/tools#structured-content)。当工具需要返回除文本之外的机器可解析数据（如 JSON）时，这非常有用。

当 MCP 工具返回 `structuredContent` 时，适配器会将其封装在 [`MCPToolArtifact`](/docs/reference/langchain-mcp-adapters#MCPToolArtifact) 中，并作为工具的工件返回。您可以通过 `ToolMessage` 上的 `artifact` 字段进行访问。您还可以使用 [拦截器](#tool-interceptors) 来自动处理或转换结构化内容。

**从工件中提取结构化内容**

调用代理后，您可以从工具消息的响应中访问结构化内容：
...
如果您希望结构化内容在对话历史中可见（供模型查看），您可以使用 [拦截器](#tool-interceptors) 自动将结构化内容追加到工具结果中：
...
MCP 工具可以在其响应中返回 [多模态内容](https://modelcontextprotocol.io/specification/2025-03-26/server/tools#tool-result)（图像、文本等）。当 MCP 服务器返回包含多个部分（例如文本和图像）的内容时，适配器会将其转换为 LangChain 的 [标准内容块](/oss/langchain/messages#standard-content-blocks)。您可以通过 `ToolMessage` 上的 `content_blocks` 属性访问标准化表示：
...
这使得您可以以提供者无关的方式处理多模态工具响应，无论底层 MCP 服务器如何格式化其内容。

### 资源 (Resources)

[资源](https://modelcontextprotocol.io/docs/concepts/resources) 允许 MCP 服务器公开可被客户端读取的数据——例如文件、数据库记录或 API 响应。LangChain 将 MCP 资源转换为 [Blob](/docs/reference/langchain-core/documents#Blob) 对象，为处理文本和二进制内容提供统一的接口。
...
您也可以直接将 [`load_mcp_resources`](/docs/reference/langchain-mcp-adapters#load_mcp_resources) 与会话一起使用，以获得更多控制权：
...
[提示](https://modelcontextprotocol.io/docs/concepts/prompts) 允许 MCP 服务器公开可被客户端检索和使用的可重用提示模板。LangChain 将 MCP 提示转换为 [消息](/docs/concepts/messages)，使其易于集成到基于聊天的工作流中。
...
您也可以直接将 [`load_mcp_prompt`](/docs/reference/langchain-mcp-adapters#load_mcp_prompt) 与会话一起使用，以获得更多控制权：
...
MCP 服务器作为单独的进程运行——它们无法访问 LangGraph 的运行时信息，如 [存储区 (store)](/oss/langgraph/persistence#memory-store)、[上下文 (context)](/oss/langchain/context-engineering) 或代理状态。**拦截器弥补了这一差距**，在 MCP 工具执行期间为您提供对该运行时上下文的访问权限。

拦截器还提供类似中间件的控制，用于工具调用：您可以修改请求、实现重试、动态添加请求头或完全中断执行。

| 部分 | 描述 |
|------|--------------|
| [访问运行时上下文](#accessing-runtime-context) | 读取用户 ID、API 密钥、存储数据和代理状态 |
| [状态更新和命令](#state-updates-and-commands) | 使用 `Command` 更新代理状态或控制图流程 |
| [编写拦截器](#writing-interceptors) | 用于修改请求、组合拦截器和错误处理的模式 |

#### 访问运行时上下文

当 MCP 工具通过 `create_agent` 在 LangChain 代理中使用时，拦截器会收到对 `ToolRuntime` 上下文的访问权限。这提供了对工具调用 ID、状态、配置和存储区的访问——从而支持强大的模式，用于访问用户数据、持久化信息和控制代理行为。

<Tabs>
  <Tab title="运行时上下文">
    访问在调用时传递的用户特定配置，如用户 ID、API 密钥或权限：

    ```python 将用户上下文注入到 MCP 工具调用中
  ...
  <Tab title="状态 (State)">
    ...
    async def require_authentication(
        ...
        if request.name in sensitive_tools and not is_authenticated:
            # 返回错误而不是调用工具
            return ToolMessage(
                content="Authentication required. Please log in first.", # 需要身份验证。请先登录。
  ...
  <Tab title="工具调用 ID (Tool call ID)">
    ...
    async def rate_limit_interceptor(
        ...
        if is_rate_limited(request.name):
            return ToolMessage(
                content="Rate limit exceeded. Please try again later.", # 超过速率限制。请稍后再试。
    ...
    client = MultiServerMCPClient(
        {...},
        tool_interceptors=[rate_limit_interceptor],  # [!code highlight]
    )
    ```
  </Tab>
</Tabs>
...
使用 `Command(goto="__end__")` 可以提前结束执行：
...
拦截器是包装工具执行的异步函数，支持请求/响应修改、重试逻辑和其他横切关注点。它们遵循“洋葱”模式，列表中第一个拦截器是最外层。

**基本模式**

拦截器是一个异步函数，接收请求和处理程序 (handler)。您可以在调用处理程序**之前**修改请求，**之后**修改响应，或者完全跳过处理程序。
...
使用 `request.override()` 创建一个修改后的请求。这遵循不可变模式，原始请求保持不变。
...
async def fallback_interceptor(
    ...
    try:
        ...
        return f"Tool {request.name} timed out. Please try again later." # 工具 {request.name} 超时。请稍后再试。
    except ConnectionError:
        return f"Could not connect to {request.name} service. Using cached data." # 无法连接到 {request.name} 服务。使用缓存数据。
```
...
MCP 协议支持 [日志记录](https://modelcontextprotocol.io/specification/2025-03-26/server/utilities/logging#log-levels) 通知（来自服务器）。使用 `Callbacks` 类来订阅这些事件。
