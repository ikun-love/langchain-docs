...
您可以进行以下选择：
- `.split_text` — 返回纯字符串块。
- `.create_documents` — 返回 LangChain @[Document] 对象，当需要为下游任务保留元数据时非常有用。
:::
:::js
您可以进行以下选择：
- `.splitText` — 返回纯字符串块。
- `.createDocuments` — 返回 LangChain @[Document] 对象，当需要为下游任务保留元数据时非常有用。
:::
...
:::
:::js
使用 `.createDocuments` 将与每个文档关联的元数据传播到输出的块中：
