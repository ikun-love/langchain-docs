...
research_subagent = {
    ...
    "system_prompt": """你是一名彻底的研究员。你的工作是：
...
email_agent = {
    "name": "email-sender",
    "tools": [send_email, web_search, database_query, file_upload],  # 不集中
}
```
...
agent = create_deep_agent(
    model="claude-sonnet-4-5-20250929",
    system_prompt="你协调数据分析和报告。使用子代理来处理专门任务。",
    subagents=subagents
)
```
