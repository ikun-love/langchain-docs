...
在 v1.0 之前，我们推荐使用 [`langgraph.prebuilt.create_react_agent`](https://reference.langchain.com/python/langgraph/agents/#langgraph.prebuilt.chat_agent_executor.create_react_agent) 来构建代理。现在，我们推荐使用 [`langchain.agents.create_agent`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent) 来构建代理。

下表概述了从 [`create_react_agent`](https://reference.langchain.com/python/langgraph/agents/#langgraph.prebuilt.chat_agent_executor.create_react_agent) 到 [`create_agent`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent) 的功能变化：
...
有关更多信息，请参阅 [代理](/oss/python/langchain/agents)。
...
如果系统提示词中使用了 [`SystemMessage`](https://reference.langchain.com/python/langchain/messages/#langchain.messages.SystemMessage) 对象，请提取其字符串内容：
...
动态提示词是一种核心的上下文工程模式——它们根据当前的对话状态来调整您告诉模型的指令。要实现此功能，请使用 [`@dynamic_prompt`](https://reference.langchain.com/python/langchain/middleware/#langchain.agents.middleware.dynamic_prompt) 装饰器：
...
常见用例包括：
* [人工干预回路](/oss/python/langchain/human-in-the-loop)
* 输出护栏
...
1. **通过 [`create_agent`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent) 上的 [`state_schema`](https://reference.langchain.com/python/langchain/middleware/#langchain.agents.middleware.AgentMiddleware.state_schema)** - 最适用于工具需要访问的状态
2. **通过中间件** - 最适用于由特定中间件钩子和附加到所述中间件的工具来管理的状态

<Note>
通过中间件定义自定义状态优于在 [`create_agent`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent) 上定义它的原因在于，它允许您将状态扩展在概念上限定于相关的中间件和工具。
...
有关通过中间件定义自定义状态的更多详细信息，请参阅 [中间件操作文档](/oss/python/langchain/middleware#custom-state-schema)。

#### 状态类型限制

[`create_agent`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent) 只支持 `TypedDict` 作为状态模式（schema）。Pydantic 模型和数据类不再受支持。
...
动态模型选择允许您根据运行时上下文（例如，任务复杂性、成本限制或用户偏好）选择不同的模型。[`create_react_agent`](https://reference.langchain.com/python/langgraph/agents/#langgraph.prebuilt.chat_agent_executor.create_react_agent) 在 [`langgraph-prebuilt`](https://pypi.org/project/langgraph-prebuilt) 的 v0.6 版本中，通过传递给 `model` 参数的可调用对象（callable）支持动态模型和工具选择。
...
[`create_agent`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent) 的 [`tools`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent(tools)) 参数接受以下内容的列表：
...
此参数将不再接受 [`ToolNode`](https://reference.langchain.com/python/langgraph/agents/#langgraph.prebuilt.tool_node.ToolNode) 实例。
...
在 v1 中，消息获得了与提供商无关的标准内容块。通过 [`message.content_blocks`](https://reference.langchain.com/python/langchain_core/language_models/#langchain_core.messages.BaseMessage.content_blocks) 访问它们，以获得跨提供商的一致、类型化的视图。现有的 [`message.content`](https://reference.langchain.com/python/langchain_core/language_models/#langchain_core.messages.BaseMessage.content_blocks) 字段对于字符串或提供商原生结构保持不变。

### 发生了什么变化

- 消息上新增了 [`content_blocks`](https://reference.langchain.com/python/langchain_core/language_models/#langchain_core.messages.BaseMessage.content_blocks) 属性，用于规范化内容
- 标准化了块形状，记录在 [消息](/oss/python/langchain/messages#standard-content-blocks) 中
...
当与 Responses API 交互时，`langchain-openai` 现在默认将响应项存储在消息的 `content` 中。要恢复先前的行为，请将环境变量 `LC_OUTPUT_VERSION` 设置为 `v0`，或在实例化 [`ChatOpenAI`](https://reference.langchain.com/python/integrations/langchain_openai/ChatOpenAI) 时指定 `output_version="v0"`。
...
`example` 参数已从 [`AIMessage`](https://reference.langchain.com/python/langchain/messages/#langchain.messages.AIMessage) 对象中移除。我们建议迁移到使用 `additional_kwargs` 来根据需要传递额外的元数据。
