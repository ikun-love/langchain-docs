...
<Tip>
刚接触上下文工程？请从[概念概述](/oss/python/concepts/context)开始，了解不同类型的上下文以及何时使用它们。
</Tip>
...
LangChain 的[中间件](/oss/python/langchain/middleware) 是底层机制，它使得上下文工程对于使用 LangChain 的开发人员来说是可行的。
...
对于**持久**的更新（例如[生命周期上下文](#life-cycle-context)中的摘要示例），它会修改状态（如摘要），请使用生命周期钩子，例如 `before_model` 或 `after_model` 来永久更新对话历史。有关更多详细信息，请参阅[中间件文档](/oss/python/langchain/middleware)。
</Note>
...
请参阅[动态选择工具](/oss/python/langchain/middleware#dynamically-selecting-tools)以获取更多示例。
...
请参阅[动态模型](/oss/python/langchain/agents#dynamic-model)以获取更多示例。
...
请参阅[工具](/oss/python/langchain/tools)以获取有关在工具中访问状态、存储和运行时上下文的全面示例。

## 生命周期上下文

控制智能体步骤**之间**发生的事情——拦截数据流以实现摘要、护栏和日志记录等跨领域关注点。

正如你在[模型上下文](#model-context)和[工具上下文](#tool-context)中所看到的，[中间件](/oss/python/langchain/middleware)是使上下文工程可行的机制。中间件允许你挂接到智能体生命周期的任何步骤，并进行：
...
<Note>
有关内置中间件的完整列表、可用钩子以及如何创建自定义中间件的说明，请参阅[中间件文档](/oss/python/langchain/middleware)。
</Note>
