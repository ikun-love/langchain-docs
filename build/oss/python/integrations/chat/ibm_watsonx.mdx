...
本示例旨在展示如何使用 `LangChain` 的 LLMs API 与 `watsonx.ai` 模型进行通信。

## 概览 (Overview)

### 集成细节 (Integration details)

| 类 (Class) | 包 (Package) | 本地 (Local) | 可序列化 (Serializable) | [JS 支持](https://js.langchain.com/docs/integrations/chat/ibm/) | 下载量 (Downloads) | 版本 (Version) |
| :--- | :--- | :---: | :---: |  :---: | :---: | :---: |
...
| [工具调用](/oss/python/langchain/tools/) | [结构化输出](/oss/python/langchain/structured-output) | JSON 模式 | 图像输入 | 音频输入 | 视频输入 | [Token 级流式传输](/oss/python/langchain/streaming/) | 原生异步 (Native async) | [Token 使用量](/oss/python/langchain/models#token-usage) | [Logprobs](/oss/python/langchain/models#log-probabilities) |
| :---: | :---: | :---: | :---: |  :---: | :---: | :---: | :---: | :---: | :---: |
| ✅ | ✅ | ✅ | ✅ | ❌ | ❌ | ✅ | ❌ | ✅ | ✅ |
...
下面的代码块定义了与 watsonx 基础模型推理工作所需的凭证。
...
您可能需要根据不同的模型或任务调整模型的 `parameters`（参数）。有关详细信息，请参阅 [可用 TextChatParameters](https://ibm.github.io/watsonx-ai-python-sdk/fm_schema.html#ibm_watsonx_ai.foundation_models.schema.TextChatParameters)。
...
- 要为 API 调用提供上下文，您必须传递 `project_id` 或 `space_id`。要获取项目或空间 ID，请打开您的项目或空间，转到 **管理 (Manage)** 选项卡，然后单击 **常规 (General)**。更多信息请参见：[项目文档](https://www.ibm.com/docs/en/watsonx-as-a-service?topic=projects) 或 [部署空间文档](https://www.ibm.com/docs/en/watsonx/saas?topic=spaces-creating-deployment)。
- 根据您实例的服务区域，使用 [watsonx.ai API 认证](https://ibm.github.io/watsonx-ai-python-sdk/setup_cloud.html#authentication) 中列出的某个 URL。

在此示例中，我们将使用 `project_id` 和 Dallas URL。

您需要指定用于推理的 `model_id`。您可以在 [支持的聊天模型](https://ibm.github.io/watsonx-ai-python-sdk/fm_helpers.html#ibm_watsonx_ai.foundation_models_manager.FoundationModelsManager.get_chat_model_specs) 列表中找到所有可用模型的列表。
...
备选方案是使用 Cloud Pak for Data 凭证。详情请参阅 [watsonx.ai 软件设置](https://ibm.github.io/watsonx-ai-python-sdk/setup_cpd.html)。
...
您可以用先前部署模型所引用的 `deployment_id` 替换 `model_id`，该模型需引用一个 Prompt Template（提示模板）：[部署模型参考](https://cloud.ibm.com/apidocs/watsonx-ai#deployments-text-chat)。
...
对于某些特定需求，可以选择将 IBM 的 [`APIClient`](https://ibm.github.io/watsonx-ai-python-sdk/base.html#apiclient) 对象传递给 `ChatWatsonx` 类。
...
要获取补全内容，您可以直接使用字符串提示（prompt）调用模型。
...
```python
# 多轮对话调用 (Invocation multiple chat)
...
创建 `ChatPromptTemplate` 对象，这些对象负责生成随机问题。
...
请注意，`AIMessage` 有一个 `tool_calls` 属性。它以标准化的 `ToolCall` 格式包含信息，这种格式与模型提供商无关。
...
有关 `ChatWatsonx` 所有功能和配置的详细文档，请前往 [API 参考](https://reference.langchain.com/python/integrations/langchain_ibm/ChatWatsonx/) 页面。
