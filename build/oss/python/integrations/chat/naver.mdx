...
本指南提供了通过 CLOVA Studio 使用 Naver 的 HyperCLOVA X [聊天模型](https://python.langchain.com/docs/concepts/chat_models)的快速入门概览。有关所有 ChatClovaX 功能和配置的详细文档，请参阅[API 参考](https://guide.ncloud-docs.com/docs/clovastudio-dev-langchain)。

[CLOVA Studio](http://clovastudio.ncloud.com/) 包含多个聊天模型。您可以在 CLOVA Studio 指南的[文档](https://guide.ncloud-docs.com/docs/clovastudio-model)中找到有关最新模型的信息，包括它们的成本、上下文窗口和支持的输入类型。
...
3. 创建一个 CLOVA Studio 测试应用或您要使用的模型的服务应用（参见[此处](https://guide.ncloud-docs.com/docs/clovastudio-playground-testapp).)
4. 获取一个测试或服务 API 密钥（参见[此处](https://api.ncloud-docs.com/docs/ai-naver-clovastudio-summary#API%ED%82%A4).)

### 凭证 (Credentials)

使用您的 API 密钥设置 `CLOVASTUDIO_API_KEY` 环境变量。

您可以像下面这样将其添加到您的环境变量中：
...
除了下面的 `invoke` 方法外，`ChatClovaX` 还支持批量处理、流式处理及其异步功能。
...
CLOVA Studio 支持工具调用（也称为“[函数调用](https://api.ncloud-docs.com/docs/clovastudio-chatcompletionsv3-fc)”），它允许您描述工具及其参数，并让模型返回一个包含要调用的工具及其输入的 JSON 对象。这对于构建使用工具的链和代理，以及更普遍地从模型中获取结构化输**出非常有用。

**注意**: 要在 CLOVA Studio 中使用工具调用功能，您应将 `max_tokens` 设置得大于 1024。

### ChatClovaX.bind_tools()

使用 `ChatClovaX.bind_tools`，我们可以轻松地将 Pydantic 类、字典模式 (dict schemas)、LangChain 工具甚至函数作为工具传递给模型。在底层，这些会被转换为 OpenAI 兼容的工具模式（Schemas），如下所示：
...
请注意，`AIMessage` 具有 `tool_calls` 属性。它包含一个标准化的 `ToolCall` 格式，与模型提供商无关。
...
对于受支持的模型，您可以使用[结构化输出](https://api.ncloud-docs.com/docs/clovastudio-chatcompletionsv3-so)功能，强制模型以特定结构（如 Pydantic 模型、TypedDict 或 JSON）生成响应。

**注意**: 结构化输出要求禁用“思考”模式。将 `thinking.effort` 设置为 `none`。
...
对于受支持的模型，当启用[思考](https://api.ncloud-docs.com/docs/clovastudio-chatcompletionsv3-thinking)功能时（默认启用），它将输出导致最终答案的分步推理过程。
...
当“思考”模式启用时，您可以通过 `AIMessage.additional_kwargs` 中的 `thinking_content` 属性访问思考过程。
...
您可以通过将 `task_id` 传递给 `model` 参数来调用微调模型，格式如下：`ft:{task_id}`。

您可以在相应的测试应用或服务应用的详细信息中查看 `task_id`。
