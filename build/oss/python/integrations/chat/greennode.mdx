...
>[GreenNode](https://greennode.ai/) 是一家全球性的 AI 解决方案提供商，同时也是 **NVIDIA 优先合作伙伴 (Preferred Partner)**。我们为美国、中东和北非（MENA）以及亚太地区的企业提供全栈式 AI 能力——从基础设施到应用程序。GreenNode 运营在 **世界一流的基础设施**之上（符合 LEED Gold、TIA-942、Uptime Tier III 标准），为企业、初创公司和研究人员提供全面的 AI 服务套件。

本页面将帮助您开始使用 GreenNode Serverless AI [聊天模型](/oss/python/langchain/models)。有关 ChatGreenNode 所有功能和配置的详细文档，请访问 [API 参考](https://python.langchain.com/api_reference/greennode/chat_models/langchain_greennode.chat_models.ChatGreenNode.html)。

[GreenNode AI](https://greennode.ai/) 提供了一个 API，用于查询 [20 多个领先的开源模型](https://aiplatform.console.greennode.ai/models)。
...
| [工具调用](/oss/python/langchain/tools) (Tool calling) | [结构化输出](/oss/python/langchain/structured-output) (Structured output) | JSON 模式 (JSON mode) | [图像输入](/oss/python/langchain/messages#multimodal) (Image input) | 音频输入 (Audio input) | 视频输入 (Video input) | [Token 级流式传输](/oss/python/langchain/streaming#llm-tokens) (Token-level streaming) | 原生异步 (Native async) | [Token 使用量](/oss/python/langchain/models#token-usage) (Token usage) | [对数概率](/oss/python/langchain/models#log-probabilities) (Logprobs) |
| :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| ✅ | ✅ | ✅ | ✅ | ❌ | ❌ | ✅ | ✅ | ✅ | ❌ |
...
请访问 [此页面](https://aiplatform.console.greennode.ai/api-keys) 注册 GreenNode AI 平台并生成 API 密钥。完成此操作后，请设置 `GREENNODE_API_KEY` 环境变量：
...
如果您想对模型的调用进行自动化跟踪，可以通过取消注释下面的代码来设置您的 [LangSmith](https://docs.smith.langchain.com/) API 密钥：
...
您可以在 LangChain 链 (Chains) 和代理 (Agents) 中使用 `ChatGreenNode`：
...
支持的模型完整列表请参见 [GreenNode Serverless AI 模型](https://greennode.ai/product/model-as-a-service)。
