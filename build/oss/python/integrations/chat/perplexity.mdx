...
本页面将帮助您开始使用 Perplexity 的 [chat 模型](/oss/python/langchain/models)。如需了解所有 `ChatPerplexity` 功能和配置的详细文档，请参阅 [API 参考文档](https://python.langchain.com/api_reference/perplexity/chat_models/langchain_perplexity.chat_models.ChatPerplexity.html)。

## 概述

### 集成详情

| 类名 | 包 | 本地 | 可序列化 | [JS 支持](https://js.langchain.com/docs/integrations/chat/xai) | 下载量 | 版本 |
| :--- | :--- | :---: | :---: |  :---: | :---: | :---: |
...
| [工具调用](/oss/python/langchain/tools) | [结构化输出](/oss/python/langchain/structured-output) | JSON 模式 | [图像输入](/oss/python/langchain/messages#multimodal) | 音频输入 | 视频输入 | [Token 级流式传输](/oss/python/langchain/streaming#llm-tokens) | 原生异步 | [Token 使用量](/oss/python/langchain/models#token-usage) | [对数概率](/oss/python/langchain/models#log-probabilities) |
| :---: | :---: | :---: | :---: |  :---: | :---: | :---: | :---: | :---: | :---: |
| ❌ | ✅ | ✅ | ❌ | ❌ | ❌ | ✅ | ❌ | ✅ | ❌ |

## 设置

要访问 Perplexity 模型，您需要创建一个 Perplexity 账户，获取一个 API 密钥，并安装 `langchain-perplexity` 集成包。

### 凭证

请访问 [此页面](https://www.perplexity.ai/) 注册 Perplexity 并生成 API 密钥。完成此操作后，设置 `PPLX_API_KEY` 环境变量：
...
提供的代码假设您的 PPLX_API_KEY 已在您的环境中设置。如果您想手动指定 API 密钥并选择不同的模型，可以使用以下代码：

```python
chat = ChatPerplexity(temperature=0, pplx_api_key="YOUR_API_KEY", model="sonar")
```

您可以在 [此处](https://docs.perplexity.ai/docs/model-cards) 查看可用模型的列表。为了保证结果的可复现性，我们可以通过接收笔记本中的输入来动态设置 API 密钥。
...
您可以像通常那样格式化和构造提示。在下面的示例中，我们要求模型讲一个关于猫的笑话。
...
## 通过 `ChatPerplexity` 使用特定于 Perplexity 的参数

您还可以通过 `ChatPerplexity` 类使用特定于 Perplexity 的参数。例如，像 `search_domain_filter`、`return_images`、`return_related_questions` 或 `search_recency_filter` 这样的参数可以使用 `extra_body` 参数来实现，如下所示：
...
Perplexity 通常会提供它所参考的网页列表（“search\_results”）。
您无需传递任何特殊的参数——该列表位于 `response.additional_kwargs["search_results"]` 中。
