...
这将帮助您开始使用 `langchain_huggingface` [聊天模型](/oss/python/langchain/models)。有关 `ChatHuggingFace` 所有功能和配置的详细文档，请参阅 [API 参考](https://python.langchain.com/api_reference/huggingface/chat_models/langchain_huggingface.chat_models.huggingface.ChatHuggingFace.html)。有关 Hugging Face 支持的模型列表，请查看 [此页面](https://huggingface.co/models)。
...
现在让我们利用 [Inference Providers](https://huggingface.co/docs/inference-providers) 在特定的第三方提供商上运行模型
...
### 使用量化 (Quantization) 进行实例化

要运行模型的量化版本，您可以按如下方式指定一个 `BitsAndBytesConfig`：
...
然后将其作为 `model_kwargs` 的一部分传递给 `HuggingFacePipeline`：
