...
聊天提示由一个 [`SystemMessage`](https://reference.langchain.com/python/langchain/messages/#langchain.messages.SystemMessage) 以及成对的 `HumanMessage`/`AIMessage` 组成，这些消息包含了作为问题/SQL 对的示例。您可以向此列表追加样本对，但这并非旨在促进典型的自然语言对话。
