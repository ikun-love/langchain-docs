---
title: ChatOpenAI
description: 开始在 LangChain 中使用 OpenAI 的 [聊天模型](/oss/python/langchain/models)。
---
...
<Tip>
    **API 参考**
    ...
    有关所有功能和配置选项的详细文档，请查阅 [`ChatOpenAI`](https://reference.langchain.com/python/integrations/langchain_openai/ChatOpenAI) API 参考。
</Tip>

<Note>
    **Chat Completions API 兼容性**

    `ChatOpenAI` 完全兼容 OpenAI 的 [Chat Completions API](https://platform.openai.com/docs/api-reference/chat)。如果您希望连接到支持 Chat Completions API 的其他模型提供商，可以这样做——请参阅[说明](/oss/python/integrations/chat#chat-completions-api)。
</Note>
...
| 类 | 包 | <Tooltip tip="Can run on local hardware" cta="Learn more" href="/oss/python/langchain/models#local-models">本地</Tooltip> | 可序列化 | JS/TS 支持 | 下载量 | 最新版本 |
| :--- | :--- | :---: | :---: |  :---: | :---: | :---: |
...
| [工具调用](/oss/python/langchain/tools) | [结构化输出](/oss/python/langchain/structured-output) | 图像输入 | 音频输入 | 视频输入 | [Token 级别流式传输](/oss/python/langchain/streaming/) | 原生异步 | [Token 使用量](/oss/python/langchain/models#token-usage) | [Logprobs](/oss/python/langchain/models#log-probabilities) |
| :---: | :---: | :---: |  :---: | :---: | :---: | :---: | :---: | :---: |
| ✅ | ✅ | ✅ | ✅ | ❌ | ✅ | ✅ | ✅ | ✅ |
...
有关所有可用模型参数的完整信息，请参阅 [`ChatOpenAI`](https://reference.langchain.com/python/integrations/langchain_openai/ChatOpenAI) API 参考。
...
要在使用 [`ChatOpenAI`](https://reference.langchain.com/python/integrations/langchain_openai/ChatOpenAI) 或 `AzureChatOpenAI` 进行流式传输时恢复 Token 计数，请将 `stream_usage=True` 设置为初始化参数或调用参数：
...
<Info>
    **Azure OpenAI v1 API 支持**
    ...
    对于传统的特定于 Azure 的实现，请继续使用 [`AzureChatOpenAI`](/oss/python/integrations/chat/azure_chat_openai/)。
</Info>
...
有关绑定工具和工具调用输出的更多信息，请查阅[工具调用](/oss/python/langchain/tools)文档。
...
OpenAI 支持面向构建[代理式](/oss/python/langchain/agents)应用的[响应](https://platform.openai.com/docs/guides/responses-vs-chat-completions) API。它包括一套[内置工具](https://platform.openai.com/docs/guides/tools?api-mode=responses)，如网络和文件搜索。它还支持[对话状态](https://platform.openai.com/docs/guides/conversation-state?api-mode=responses)的管理，允许您在不显式传入先前消息的情况下继续对话，以及[推理过程](https://platform.openai.com/docs/guides/reasoning?api-mode=responses)的输出。

当使用这些功能时，[`ChatOpenAI`](https://reference.langchain.com/python/integrations/langchain_openai/ChatOpenAI) 将路由到 Responses API。您也可以在实例化 [`ChatOpenAI`](https://reference.langchain.com/python/integrations/langchain_openai/ChatOpenAI) 时指定 `use_responses_api=True`。
...
请注意，响应包括结构化的[内容块](/oss/python/langchain/messages/#message-content)，其中包含响应文本和 OpenAI [注释](https://platform.openai.com/docs/guides/tools-web-search?api-mode=responses#output-and-citations)（引用其来源）。输出消息还将包含来自任何工具调用的信息：
...
[`ChatOpenAI`](https://reference.langchain.com/python/integrations/langchain_openai/ChatOpenAI) 支持 `"computer-use-preview"` 模型，这是一种专门用于内置计算机使用工具的模型。要启用它，请像传递其他工具一样传递[计算机使用工具](https://platform.openai.com/docs/guides/tools-computer-use)。

目前，计算机使用的工具输出存在于消息 `content` 字段中。要回复计算机使用工具调用，请使用 `{"type": "computer_call_output"}` 构造一个带有 `additional_kwargs` 的 [`ToolMessage`](https://reference.langchain.com/python/langchain/messages/#langchain.messages.ToolMessage)。消息的内容将是屏幕截图。下面，我们演示一个简单的示例。
...
接下来，我们使用这些属性构造一个 [`ToolMessage`](https://reference.langchain.com/python/langchain/messages/#langchain.messages.ToolMessage):
...
您可以手动管理状态，或使用 [LangGraph](/oss/python/langgraph/quickstart)，就像使用其他聊天模型一样：
...
<Tip>
**您可以使用 [LangGraph](https://langchain-ai.github.io/langgraph/) 在内存或 Postgres 等各种后端为您管理对话线程。请参阅[本教程](/oss/python/langgraph/quickstart)开始。**
...
OpenAI 可以返回模型推理的摘要（尽管它不暴露原始推理 Token）。要配置 [`ChatOpenAI`](https://reference.langchain.com/python/integrations/langchain_openai/ChatOpenAI) 返回此摘要，请指定 `reasoning` 参数。如果设置了此参数，[`ChatOpenAI`](https://reference.langchain.com/python/integrations/langchain_openai/ChatOpenAI) 将自动路由到 Responses API。
...
OpenAI 具有支持多模态输入的模型。您可以向这些模型传入图像、PDF 或音频。有关如何在 LangChain 中执行此操作的更多信息，请查阅[多模态输入](/oss/python/langchain/messages#multimodal)文档。

您可以在 [OpenAI 文档](https://platform.openai.com/docs/models)中查看支持不同模态的模型列表。

对于所有模态，LangChain 同时支持其跨提供商的标准格式和 OpenAI 的原生内容块格式。

要将多模态数据传入 [`ChatOpenAI`](https://reference.langchain.com/python/integrations/langchain_openai/ChatOpenAI)，请创建一个[内容块](/oss/python/langchain/messages/)，其中包含数据并将其纳入消息中，例如如下所示：
...
请参阅[操作指南](/oss/python/langchain/messages#multimodal)中“如何操作”部分的示例。
...
有关更多信息，请参阅[操作指南](/oss/python/langchain/messages#multimodal#example-openai-file-names)中“如何操作”部分的示例，以及[此处](/oss/python/langchain/messages#multimodal#documents-pdf)。
...
有关更多信息，请参阅[操作指南](/oss/python/langchain/messages#multimodal#audio)的示例。
