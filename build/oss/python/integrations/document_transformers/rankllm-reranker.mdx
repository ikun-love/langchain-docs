...
**[RankLLM](https://github.com/castorini/rank_llm)** 是一个**灵活的重排框架**，支持**列表式（listwise）、成对（pairwise）和点式（pointwise）排序模型**。它包含了 **RankVicuna、RankZephyr、MonoT5、DuoT5、LiT5 和 FirstMistral**，并集成了 **FastChat、vLLM、SGLang 和 TensorRT-LLM** 以实现高效推理。RankLLM 针对**检索和排序任务**进行了优化，同时利用了**开源 LLM** 和 **RankGPT、RankGemini** 等专有重排器。它支持**批处理推理、首次令牌重排以及通过 BM25 和 SPLADE 进行检索**。

> **注意：** 如果使用内置检索器，RankLLM 需要 **Pyserini、JDK 21、PyTorch 和 Faiss** 来实现检索功能。
...
我们首先初始化一个简单的向量存储检索器，并将 2023 年国情咨文（State of the Union）演讲内容（分块后）存储进去。我们可以将检索器设置为检索较多的文档数量（20 个）。
...
RankZephyr 执行列表式重排以提高检索质量，但需要至少 24GB 的 VRAM 才能高效运行。
...
```output
{'query': 'What was done to Russia?',
 'result': '俄罗斯遭受了强有力的经济制裁，包括将其最大的银行排除在国际金融体系之外，阻止其中央银行保卫俄罗斯卢布，并切断其获取技术的渠道。此外，美国领空已对所有俄罗斯航班关闭，进一步孤立了俄罗斯并加剧了其经济压力。这些行动导致卢布大幅贬值、俄罗斯股市暴跌以及俄罗斯经济陷入全面动荡。'}
```

# 检索 + RankLLM 重排（RankGPT）
...
```output
{'query': 'What did the president say about Ketanji Brown Jackson',
 'result': "总统提到，Ketanji Brown Jackson 是美国最顶尖的法律人才之一，曾是私人执业的顶级诉讼律师，联邦公设辩护人，并且出身于公立学校教育工作者和警察的家庭。他还强调她是一位共识建立者，自被提名以来获得了广泛的支持。"}
```
