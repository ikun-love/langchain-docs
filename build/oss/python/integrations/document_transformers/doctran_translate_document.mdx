...
通过嵌入（embeddings）进行文档比较的一个好处是它可以跨多种语言工作。"Harrison says hello" 和 "Harrison dice hola" 因为具有相同的语义，在向量空间中会占据相似的位置。

然而，使用 LLM **将文档翻译成其他语言**后再进行向量化仍然非常有用。当预期用户将使用不同语言查询知识库，或者对于特定语言尚未提供最先进的嵌入模型时，这一点尤其有帮助。

我们可以使用 [Doctran](https://github.com/psychic-api/doctran) 库来实现这一点，该库利用 OpenAI 的函数调用（function calling）功能在不同语言之间进行文档翻译。
...
## 使用同步版本 (Sync version) 输出

在翻译文档后，结果将作为一个新的 `Document` 对象返回，其 `page_content` 已被翻译成目标语言。
...
## 使用异步版本 (Async version) 输出

在翻译文档后，结果将作为一个新的 `Document` 对象返回，其 `page_content` 已被翻译成目标语言。异步版本在文档被分块处理时可以提高性能。它还将确保以正确的顺序返回输出。
