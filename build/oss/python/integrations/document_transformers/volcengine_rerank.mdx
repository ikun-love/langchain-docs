...
本 Notebook 展示了如何使用 Volcengine Reranker 进行文档压缩和检索。Volcengine 是字节跳动（TikTok 的母公司）开发的云服务平台，您可以通过 [Volcengine](https://www.volcengine.com/) 访问。

Volcengine 的 Rerank 服务支持对最多 50 个文档进行重排序，每个文档最大长度为 4000 个 Token。如需了解更多信息，请访问 [此处](https://www.volcengine.com/docs/84313/1254474) 和 [此处](https://www.volcengine.com/docs/84313/1254605)。
...
```python
# 获取 ak/sk 的方式：https://www.volcengine.com/docs/84313/1254488
...
我们首先初始化一个简单的向量存储检索器，并将 2023 年“国情咨文”演讲稿（分块后）存储其中。我们可以将检索器设置为检索较多的文档数量（例如 20 个）。
...
## 使用 VolcengineRerank 进行重排序 (Reranking with VolcengineRerank)

现在，我们将基础检索器包装成一个 `ContextualCompressionRetriever`。我们将使用 `VolcengineRerank` 来对返回的结果进行重排序。
