...
存储在向量存储知识库中的文档通常以叙事或对话格式存储。然而，大多数用户查询都是以问题形式出现的。如果在向量化之前**将文档转换为问答（Q&A）格式**，我们可以提高检索相关文档的可能性，并降低检索不相关文档的可能性。

我们可以使用 [Doctran](https://github.com/psychic-api/doctran) 库来实现这一点，该库利用 OpenAI 的函数调用功能来“询问”（interrogate）文档。

有关基于原始文档与询问后文档的各种查询的向量相似性得分的基准测试，请参阅[此 Notebook](https://github.com/psychic-api/doctran/blob/main/benchmark.ipynb)。
...
询问文档后，结果将作为包含元数据中提供的问题和答案的新文档返回。
