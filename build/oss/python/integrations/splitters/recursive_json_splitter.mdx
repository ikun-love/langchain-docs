...
这个 JSON 分割器 [提供了一个工具](/oss/python/integrations/splitters/)，用于分割 JSON 数据，同时可以控制块（chunk）的大小。它以深度优先的方式遍历 JSON 数据，并构建更小的 JSON 块。它会尝试保持嵌套的 JSON 对象完整，但如果需要，也会将其分割，以确保块的大小介于 `min_chunk_size` 和 `max_chunk_size` 之间。
