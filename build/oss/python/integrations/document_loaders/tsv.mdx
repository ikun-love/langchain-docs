...
> [选项卡分隔值 (TSV)](https://en.wikipedia.org/wiki/Tab-separated_values) 文件是一种简单的、基于文本的文件格式，用于存储表格数据。[3] 记录之间用换行符分隔，记录内的值之间用制表符（tab 字符）分隔。

## `UnstructuredTSVLoader`

您也可以使用 `UnstructuredTSVLoader` 来加载表格。使用 `UnstructuredTSVLoader` 的一个优点是：如果您在 `"elements"` 模式下使用它，表的 HTML 表示形式将可在元数据中获取。
