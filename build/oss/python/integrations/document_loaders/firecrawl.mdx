...
[FireCrawl](https://firecrawl.dev/?ref=langchain) 可以抓取并转换任何网站内容为 LLM（大语言模型）可用的数据。它会抓取所有可访问的子页面，并为每个页面提供干净的 Markdown 和元数据。无需提供站点地图（sitemap）。

FireCrawl 能够处理复杂的任务，例如反向代理、缓存、速率限制以及由 JavaScript 阻止的内容。由 [mendable.ai](https://mendable.ai) 团队构建。

## 概述

### 集成详情

| 类（Class） | 包（Package） | 本地（Local） | 可序列化（Serializable） | [JS 支持](https://js.langchain.com/docs/integrations/document_loaders/web_loaders/firecrawl/)|
| :--- | :--- | :---: | :---: |  :---: |
...
您需要获取自己的 API 密钥。请参阅 [firecrawl.dev](https://firecrawl.dev)
...
- `scrape`: 抓取单个 URL 并返回 Markdown 内容。
- `crawl`: 抓取 URL 及其所有可访问的子页面，并为每个页面返回 Markdown 内容。
- `map`: 对目标 URL 进行映射，返回一组语义相关的页面列表。
...
您还可以向加载器传递 `params` 参数。这是一个需要传递给爬虫的选项字典。有关更多信息，请参阅 [FireCrawl API 文档](https://github.com/mendableai/firecrawl-py)。
