...
> [Bigtable](https://cloud.google.com/bigtable) 是一个键值存储和宽列存储数据库，非常适合快速访问结构化、半结构化或非结构化数据。通过利用 Bigtable 的 LangChain 集成，您可以扩展数据库应用程序以构建由 AI 驱动的体验。

本指南（Notebook）将介绍如何使用 [Bigtable](https://cloud.google.com/bigtable) 通过 `BigtableLoader` 和 `BigtableSaver` 来 [保存、加载和删除 LangChain 文档](/oss/python/integrations/document_loaders)。

您可以在 [GitHub](https://github.com/googleapis/langchain-google-bigtable-python/) 上了解有关此包的更多信息。
...
在当前 Notebook 的运行时环境中确认可以访问数据库后，请填写以下值并运行单元格，然后再运行示例脚本。
...
1. `instance_id` - Bigtable 实例的 ID。
1. `table_id` - Bigtable 中用于存储 LangChain 文档的表名称。
...
1. 使用 [过滤器 (filter)](https://cloud.google.com/python/docs/reference/bigtable/latest/row-filters)
2. 使用 [行集 (row_set)](https://cloud.google.com/python/docs/reference/bigtable/latest/row-set#google.cloud.bigtable.row_set.RowSet)
...
默认创建的客户端是默认客户端，只使用 `admin=True` 选项。要使用非默认客户端，可以将 [自定义客户端](https://cloud.google.com/python/docs/reference/bigtable/latest/client#class-googlecloudbigtableclientclientprojectnone-credentialsnone-readonlyfalse-adminfalse-clientinfonone-clientoptionsnone-adminclientoptionsnone-channelnone) 传递给构造函数。
...
`BigtableLoader` 假定存在一个名为 `langchain` 的列族，其中包含一个名为 `content` 的列，该列中的值已使用 UTF-8 编码。默认值可以如下更改：
...
默认情况下，[`Document`](https://reference.langchain.com/python/langchain_core/documents/#langchain_core.documents.base.Document) 对象的 `metadata` 映射将包含一个键：`rowkey`，其值为行的 rowkey 值。要向该映射添加更多项，请使用 `metadata_mapping`。
...
### 元数据为 JSON (Metadata as JSON)

如果 Bigtable 中的某个列包含一个 JSON 字符串，并且您希望将其添加到输出文档的元数据中，可以将以下参数添加到 `BigtableLoader`。请注意，`metadata_as_json_encoding` 的默认值是 UTF-8。
...
`BigtableSaver` 也可以像 `BigtableLoader` 一样进行定制。
