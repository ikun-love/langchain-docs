...
>
> - `Brave Search` 使用自己的网络索引。截至 2022 年 5 月，它覆盖了超过 100 亿个页面，并且 92% 的搜索结果是在不依赖任何第三方的情况下提供的。剩余的结果是通过服务器端从 Bing API 检索（或在用户选择加入（opt-in）的基础上，通过客户端从 Google 检索）。根据 Brave 的说法，该索引被“有意地做得比 Google 或 Bing 的小”，以帮助避免垃圾邮件和其他低质量内容，缺点是“Brave Search 在恢复长尾查询方面还不如 Google”。
>- `Brave Search 付费版 (Premium)`: 截至 2023 年 4 月，Brave Search 是一个无广告的网站，但它最终将切换到一个包含广告的新模式，届时付费用户将获得无广告体验。默认情况下，IP 地址等用户数据不会被收集。需要购买付费账户才能选择加入（opt-in）数据收集。
