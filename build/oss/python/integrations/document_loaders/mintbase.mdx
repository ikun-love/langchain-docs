---
title: Near 区块链
---

本笔记的目的是为 LangChain Near 区块链文档加载器提供一个测试其功能的途径。

初始阶段，该加载器支持以下功能：

* 从 NFT 智能合约（NEP-171 和 NEP-177）加载 NFT 作为文档（Documents）
* 支持 Near 主网（Mainnet）和 Near 测试网（Testnet）（默认是主网）
* 支持 Mintbase 的 Graph API

如果社区认为此加载器有价值，它可以进行扩展。具体来说：

* 可以添加额外的 API（例如，与交易相关的 API）

此文档加载器需要：

* 一个免费的 [Mintbase API 密钥](https://docs.mintbase.xyz/dev/mintbase-graph/)

输出格式如下：

* `pageContent` = 单个 NFT
* `metadata` = `{'source': 'nft.yearofchef.near', 'blockchain': 'mainnet', 'tokenId': '1846'}`

## 将 NFT 加载到文档加载器中

```python
# 从 https://docs.mintbase.xyz/dev/mintbase-graph/ 获取 MINTBASE_API_KEY

mintbaseApiKey = "..."
```

### 选项 1：以太坊主网（默认的 `BlockchainType`）

```python
from MintbaseLoader import MintbaseDocumentLoader

contractAddress = "nft.yearofchef.near"  # Year of chef 合约地址


blockchainLoader = MintbaseDocumentLoader(
    contract_address=contractAddress, blockchain_type="mainnet", api_key="omni-site"
)

nfts = blockchainLoader.load()

print(nfts[:1])

for doc in blockchainLoader.lazy_load():
    print()
    print(type(doc))
    print(doc)
```

---

<Callout icon="pen-to-square" iconType="regular">
    [Edit the source of this page on GitHub.](https://github.com/langchain-ai/docs/edit/main/src/oss\python\integrations\document_loaders\mintbase.mdx)
</Callout>
<Tip icon="terminal" iconType="regular">
    [Connect these docs programmatically](/use-these-docs) to Claude, VSCode, and more via MCP for real-time answers.
</Tip>
