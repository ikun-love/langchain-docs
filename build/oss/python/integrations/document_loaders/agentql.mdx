...
`AgentQLLoader` 当前一次只加载一个 [`Document`](https://reference.langchain.com/python/langchain_core/documents/#langchain_core.documents.base.Document)。因此，`load()` 和 `lazy_load()` 的行为是相同的：
