...
您可以通过两种模式运行加载器："single"（单个）和 "elements"（元素）。如果您使用 "single" 模式，文档将以单个 [`Document`](https://reference.langchain.com/python/langchain_core/documents/#langchain_core.documents.base.Document) 对象的形式返回。如果您使用 "elements" 模式，`unstructured` 库将把文档拆分成如 `Title` 和 `NarrativeText` 等元素。您可以在 `mode` 之后传入额外的 `unstructured` kwargs，以应用不同的 `unstructured` 设置。
