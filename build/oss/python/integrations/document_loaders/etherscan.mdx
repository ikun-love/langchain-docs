...
>[Etherscan](https://docs.etherscan.io/) 是领先的以太坊区块链浏览器、搜索、API 和分析平台，以太坊是一个去中心化的智能合约平台。

## 概述

`Etherscan` 加载器使用 `Etherscan API` 来加载指定账户在 `Ethereum Mainnet`（以太坊主网）上的交易历史记录。

你需要一个 `Etherscan api key` 才能继续操作。免费的 API 密钥每秒有 5 次调用的配额。

该加载器支持以下六种功能：

* 检索指定账户在以太坊主网（Ethereum Mainnet）上的普通交易
* 检索指定账户在以太坊主网（Ethereum Mainnet）上的内部交易
* 检索指定账户在以太坊主网（Ethereum Mainnet）上的 ERC20 交易
* 检索指定账户在以太坊主网（Ethereum Mainnet）上的 ERC721 交易
* 检索指定账户在以太坊主网（Ethereum Mainnet）上的 ERC1155 交易
* 检索指定账户在以太坊主网（Ethereum Mainnet）上 `wei` 形式的以太坊余额

如果账户没有对应的交易，加载器将返回一个包含一个文档的列表。该文档的内容是 `''`。

你可以向加载器传递不同的 `filter` 来访问我们上面提到的不同功能：
...
* `"erc721_transaction"`
* `"erc1155_transaction"`
`filter` 的默认值是 `normal_transaction`

如果你有任何疑问，可以访问 [Etherscan API 文档](https://etherscan.io/tx/0x0ffa32c787b1398f44303f731cb06678e086e4f82ce07cebf75e99bb7c079c77) 或通过 [i@inevitable.tech](mailto:i@inevitable.tech) 与我联系。

所有与交易历史相关的函数都限制在最多 1000 条历史记录，这是受 Etherscan 限制的影响。你可以使用以下参数来查找所需的交易历史：

* `offset`: 默认为 20。一次显示 20 笔交易。
* `page`: 默认为 1。这控制着分页。
* `start_block`: 默认为 0。交易历史从第 0 个区块开始。
* `end_block`: 默认为 99999999。交易历史从第 99999999 个区块开始。
* `sort`: `"desc"`（降序）或 `"asc"`（升序）。默认设置为 `"desc"` 以获取最新的交易。
