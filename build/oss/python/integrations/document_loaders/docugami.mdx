...
本指南介绍如何从 `Docugami` 加载文档。它阐述了与替代数据加载器相比，使用 Docugami 系统的优势。

## 前提条件 (Prerequisites)

1. 安装必要的 Python 包。
2. 获取您的工作区（workspace）的访问令牌（access token），并确保它已设置为 `DOCUGAMI_API_KEY` 环境变量。
3. 获取您已处理文档的文档集（docset）ID 和文档 ID，具体说明请参见：[help.docugami.com/home/docugami-api](https://help.docugami.com/home/docugami-api)

```python
# 如果您没有使用 poetry，请直接运行 pip install，而不是 "poetry run"
...
2. 添加您的文档（PDF, DOCX 或 DOC），并允许 Docugami 将它们摄取（ingest）并聚类成相似文档集，例如保密协议（NDAs）、租赁协议（Lease Agreements）和服务协议（Service Agreements）。系统支持的文档类型没有固定集合，创建的集群取决于您的具体文档，后续您可以自行 [更改文档集分配](https://help.docugami.com/home/working-with-the-doc-sets-view)。
3. 通过工作区的 Developer Playground 创建一个访问令牌。 [详细说明](https://help.docugami.com/home/docugami-api)
4. 探索 [Docugami API](https://api-docs.docugami.com) 以获取已处理的文档集 ID 列表，或单个文档集的文档 ID 列表。
5. 如下文所述，使用 `DocugamiLoader` 来获取文档的丰富语义块。
6. （可选）构建并发布一个或多个 [报告或摘要](https://help.docugami.com/home/reports)。这有助于 Docugami 根据您的偏好，使用更好的标签来改进语义 XML，这些信息随后会作为元数据添加到 `DocugamiLoader` 的输出中。使用自查询检索器（self-querying retriever）等技术可以实现高精度的文档问答（Document QA）。

## 与其他分块技术的优势对比

对文档进行适当的分块对于从文档中检索信息至关重要。存在许多分块技术，包括依赖于空格和基于字符长度的递归拆分等简单方法。Docugami 提供了一种不同的方法：

1. **智能分块 (Intelligent Chunking):** Docugami 会将每个文档分解成一个**分层的语义 XML 树**，块的大小不一，从单个单词或数值到整个部分。这些块遵循文档的语义轮廓，提供了比任意长度或简单的基于空格的分块更有意义的表示。
2. **语义注释 (Semantic Annotations):** 块会使用在整个文档集中保持一致的语义标签进行注释，从而便于在多个文档之间进行一致的分层查询，即使它们的编写和格式化方式不同。例如，在一组租赁协议中，您可以轻松识别出“出租人 (Landlord)”、“承租人 (Tenant)”或“续约日期 (Renewal Date)”等关键条款，以及更复杂的信息，例如任何转租条款的措辞，或者特定管辖区在“终止条款 (Termination Clause)”中是否包含“例外部分 (exception section)”。
3. **结构化表示 (Structured Representation):** 此外，XML 树通过使用表示标题、段落、列表、表格和其他常见元素的**属性**，指示了每个文档的结构轮廓，并且在所有支持的文档格式（如扫描的 PDF 或 DOCX 文件）中保持一致。它能妥善处理长篇文档的特征，例如页眉/页脚或多栏流，以实现干净的文本提取。
4. **附加元数据 (Additional Metadata):** 如果用户使用了 Docugami 的报告功能，块还会带有额外的（用户定义的）元数据注释。这些额外的元数据可用于高精度的文档问答，且不受上下文窗口的限制。下面有详细的代码演练。
...
如果设置了 `DOCUGAMI_API_KEY` 环境变量，则无需明确将其传递给加载器；否则，您可以将其作为 `access_token` 参数传入。
...
每个 [`Document`](https://reference.langchain.com/python/langchain_core/documents/#langchain_core.documents.base.Document)（实际上是来自实际 PDF、DOC 或 DOCX 的一个块）的 `metadata` 包含一些有用的附加信息：

1. **id 和 source:** 在 Docugami 内部，块源自的文件的 ID 和名称（PDF, DOC 或 DOCX）。
2. **xpath:** 块在文档 XML 表示中的 XPath。这对于直接引用文档 XML 中实际块的来源非常有用。
3. **structure:** 块的结构属性，例如 `h1`, `h2`, `div`, `table`, `td` 等。如果调用者需要，这可用于过滤掉某些类型的块。
4. **tag:** 使用各种生成和抽取技术为块分配的语义标签。更多详情请参见：[github.com/docugami/DFM-benchmarks](https://github.com/docugami/DFM-benchmarks)

您可以通过在 `DocugamiLoader` 实例上设置以下属性来控制分块行为：

1. 您可以设置最小和最大块大小（min and max chunk size），系统会尽量遵守这些限制（最小截断）。您可以通过设置 `loader.min_text_length` 和 `loader.max_text_length` 来控制这些。
2. 默认情况下，只返回块的文本内容。然而，Docugami 的 XML 知识图谱包含额外的丰富信息，包括块内实体的语义标签。如果您希望在返回的块中包含额外的 XML 元数据，请设置 `loader.include_xml_tags = True`。
3. 此外，如果您希望 Docugami 返回块的父块，可以设置 `loader.parent_hierarchy_levels`。子块通过 `loader.parent_id_key` 的值指向父块。这在与 MultiVector Retriever 结合使用时非常有用，可实现 [“小到大” (small-to-big)](https://www.youtube.com/watch?v=ihSiRrOUwmg) 检索。详见本笔记本后面的示例。
...
## 基本用法：面向文档问答的 Docugami Loader

您可以像使用标准加载器一样使用 `Docugami Loader` 来对多个文档进行问答，只不过它提供的是遵循文档自然轮廓的更优质的块。有很多关于如何执行此操作的精彩教程，例如 [这个教程](https://www.youtube.com/watch?v=3yPBVii7Ct0)。我们可以使用相同的代码，但使用 `DocugamiLoader` 进行更优的块划分，而不是直接使用带有基本拆分技术的文本文件或 PDF 文件。
...
```python
# 对于这个示例，我们已经有了一个处理后的文档集，用于存放一组租赁文件
loader = DocugamiLoader(docset_id="zo954yqy53wp")
chunks = loader.load()

# 故意剥离语义元数据，以测试在没有语义元数据的情况下各项功能的运行情况
for chunk in chunks:
...
加载器返回的文档已经分块，因此我们无需使用文本分块器（text splitter）。作为可选操作，我们可以利用每个文档中的元数据（例如 `structure` 或 `tag` 属性）来进行任何我们需要的后处理。

我们将直接使用 `DocugamiLoader` 的输出，以通常的方式设置检索问答链（retrieval QA chain）。
...
大型文档的一个问题是，问题的正确答案可能依赖于文档中相距较远的块。典型的分块技术，即使有重叠，也很难为 LLM 提供足够的上下文来回答此类问题。随着未来上下文容量非常大的 LLM 的出现，可能可以将大量 Token，甚至整个文档塞入上下文，但这对于非常长的文档或大量文档来说，最终仍会达到限制。

例如，如果我们提出一个更复杂的问题，要求 LLM 综合文档不同部分的块信息，即使是 OpenAI 强大的 LLM 也无法正确回答。

```python
chain_response = qa_chain("What is rentable area for the property owned by DHA Group?")
chain_response["result"]  # 正确答案应为 13,500 平方英尺
```
...
乍一看，答案似乎合理，但实际上是错误的。如果您仔细查看此答案的源块，您会发现文档分块没有将“Landlord 名称”和“可租用面积”放在同一个上下文中，从而产生了不相关的块，因此答案是错误的（正确答案应该是 **13,500 平方英尺**）。

Docugami 可以提供帮助。如果用户[使用了 Docugami 报告](https://help.docugami.com/home/reports)，块将使用不同技术创建的附加元数据进行注释。更专业的处理方法将在稍后添加。

具体来说，让我们要求 Docugami 在其输出中返回 XML 标签以及附加的元数据：

```python
loader = DocugamiLoader(docset_id="zo954yqy53wp")
loader.include_xml_tags = (
    True  # 获取来自 Docugami 知识图谱的附加语义信息
)
chunks = loader.load()
...
我们可以使用**自查询检索器 (self-querying retriever)** 来提高查询准确性，方法是利用这些附加元数据：
...
我们再次运行相同的问题。它会返回正确的结果，因为自查询检索器在元数据上创建了一个过滤器，正确地过滤出了专门针对 `DHA Group` 这个房东的文档。返回的源块都与该房东相关，这提高了答案的准确性，即使房东信息在包含正确答案的特定块中没有直接提及。
...
这次答案是正确的，因为自查询检索器根据元数据中的 `Landlord` 属性进行了过滤，从而正确地筛选出了特定于 DHA Group 的文档。结果返回的源块都与该房东相关，这提高了答案的准确性，即使房东信息没有直接出现在包含正确答案的特定块中。

# 高级主题：使用文档知识图进行“小到大”检索 (Small-to-Big Retrieval with Document Knowledge Graph Hierarchy)

文档本质上是半结构化的，`DocugamiLoader` 能够导航文档的语义和结构轮廓，并在返回的块上提供父块引用（parent chunk references）。这在与 MultiVector Retriever 结合使用时非常有用，可实现 [“小到大” (small-to-big)](https://www.youtube.com/watch?v=ihSiRrOUwmg) 检索。

要获取父块引用，您可以设置 `loader.parent_hierarchy_levels` 为非零值。
...
loader.max_text_length = (
    1024 * 8
)  # 8K 字符大约是 2K Token（参考：https://help.openai.com/en/articles/4936856-what-are-tokens-and-how-to-count-them）
loader.include_project_metadata_in_doc_metadata = (
    False  # 不在向量中过滤项目元数据，因此移除以减轻向量负载
)
...
retriever = MultiVectorRetriever(
    ...
    docstore=store,
    search_type=SearchType.mmr,  # 使用最大边际相关性搜索 (max marginal relevance search)
    search_kwargs={"k": 2},
)
...
```python
# 查询检索器，应返回父块（因为上面设置了 MMR 作为 search_type）
