...
我们创建一个 GeoPandas DataFrame，以 [`Open City Data`](/oss/python/integrations/document_loaders/open_city_data) 作为示例输入。
...
将 GeoPandas DataFrame 加载为 [`Document`](https://reference.langchain.com/python/langchain_core/documents/#langchain_core.documents.base.Document) 以进行下游处理（嵌入、聊天等）。
