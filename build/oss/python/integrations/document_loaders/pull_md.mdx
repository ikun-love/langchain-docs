...
这个包为 Web 内容实现了一个 [文档加载器](/oss/python/integrations/document_loaders/)。与传统的网页抓取工具不同，`PullMdLoader` 能够处理使用 React、Angular 或 Vue.js 等动态 JavaScript 框架构建的网页，并在无需本地渲染的情况下将其转换为 Markdown 格式。
