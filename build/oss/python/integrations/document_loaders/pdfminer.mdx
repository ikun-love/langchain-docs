...
本指南快速概述了如何开始使用 `PDFMiner` [文档加载器](https://python.langchain.com/docs/concepts/document_loaders)。有关所有 `__ModuleName__Loader` 功能和配置的详细文档，请访问 [API 参考](https://python.langchain.com/api_reference/community/document_loaders/langchain_community.document_loaders.pdf.PDFMinerLoader.html)。
...
如果您想获取模型调用的自动化最佳分级追踪，可以通过取消注释以下内容来设置您的 [LangSmith](https://docs.smith.langchain.com/) API 密钥：
...
其他元数据特定于每个解析器。
这些信息对（例如，对 PDF 进行分类）可能很有帮助。

## 分割模式和自定义页面分隔符

加载 PDF 文件时，您可以以两种不同的方式进行分割：

- 按页分割
- 作为单个文本流

默认情况下，PDFMinerLoader 将 PDF 按页分割。

### 按页提取 PDF。每页被提取为一个 langchain Document 对象
...
在此模式下，PDF 按页分割，生成的 Documents 元数据包含页码。但在某些情况下，我们可能希望将 PDF 作为单个文本流来处理（这样就不会将某些段落截断）。在这种情况下，您可以使用 *single* 模式：
...
逻辑上，在此模式下，'page\_number' 元数据会消失。以下是如何在文本流中清晰地标识页面结束位置的方法：

### 在 *single* 模式中添加自定义 *pages\_delimiter* 以识别页面结束位置
...
这可以简单地是 `\n`，或 `\f` 以清晰地指示页面更改，或 `<!-- PAGE BREAK -->` 用于在不产生视觉效果的情况下无缝注入 Markdown 查看器中。
...
您可以调整这些函数以选择提取图像的输出格式（*html*、*markdown* 或 *text*）

结果将插入到页面文本的倒数第二个和最后一个段落之间。
...
许多文档加载器都涉及解析文件。这些加载器之间的区别主要在于解析文件的方式，而不是加载文件的方式。例如，您可以使用 `open` 读取 PDF 或 Markdown 文件的二进制内容，但需要不同的解析逻辑才能将该二进制数据转换为文本。

因此，将解析逻辑与加载逻辑解耦可能很有帮助，这使得可以重用给定的解析器，而无需考虑数据是如何加载的。
您可以使用此策略，使用相同的解析参数来分析不同的文件。
...
这在通过 `BeautifulSoup` 解析输出的 HTML 内容以获取有关字体大小、页码、PDF 页眉/页脚等更结构化和丰富的信息时，有助于按语义将文本分块。
...
for c in content:
    ...
    if fs == cur_fs:
        ...
        cur_text = c.text
snippets.append((cur_text, cur_fs))
# 注意：上述逻辑非常直接。我们也可以添加更多策略，例如删除重复片段（因为
# PDF 中的页眉/页脚会出现在多页上，所以如果我们发现重复项，可以假设它是多余的信息）
```
...
for s in snippets:
    ...
    # 如果当前片段的字体大小 <= 前一个部分的标题 => 内容属于同一部分（如果需要，也可以创建子部分的树状结构，但这可能需要更多的思考，并且可能依赖于数据）
    if (
    ...
    # 如果当前片段的字体大小 > 前一个部分的标题，但小于前一个部分的标题，则也创建一个新
    # 部分（例如，PDF 标题将具有最大的字体大小，但我们不希望它覆盖所有部分）
    metadata = {"heading": s[0], "content_font": 0, "heading_font": s[1]}
    metadata.update(docs[0].metadata)
    semantic_snippets.append(Document(page_content="", metadata=metadata))
    cur_idx += 1
...
有关所有 `PDFMinerLoader` 功能和配置的详细文档，请访问 API 参考：[python.langchain.com/api_reference/community/document_loaders/langchain_community.document_loaders.pdf.PDFMinerLoader.html](https://python.langchain.com/api_reference/community/document_loaders/langchain_community.document_loaders.pdf.PDFMinerLoader.html)
...
这在处理文档加载器时很有用，因为输出的 HTML 内容可以通过 `BeautifulSoup` 进行解析，以获取有关字体大小、页码、PDF 页眉/页脚等的更结构化和丰富的信息。
