...
请参阅[此处](/oss/python/integrations/chat/azureml_chat_endpoint)的文档，了解如何访问托管在 [Azure Machine Learning](https://azure.microsoft.com/en-us/products/machine-learning/) 上的聊天模型。
...
由于 Azure Database for PostgreSQL 是开源的 Postgres，您可以使用 [LangChain 对 Postgres 的支持](/oss/python/integrations/vectorstores/pgvector/) 连接到 Azure Database for PostgreSQL。
...
为开发人员提供基础设施、API 和工具，用于大规模进行向量、关键字和混合查询的信息检索。有关使用示例，请参阅[此处](/oss/python/integrations/vectorstores/azuresearch)。
...
您需要在数据库中[启用 pgvector 扩展](https://learn.microsoft.com/en-us/azure/postgresql/flexible-server/how-to-use-pgvector)，才能将 Postgres 用作向量存储。启用扩展后，您可以使用 [LangChain 中的 PGVector](/oss/python/integrations/vectorstores/pgvector/) 来连接到 Azure Database for PostgreSQL。

查看 [使用示例](/oss/python/integrations/vectorstores/pgvector/)。只需使用来自 Azure 门户的[连接字符串](https://learn.microsoft.com/en-us/azure/postgresql/flexible-server/connect-python?tabs=cmd%2Cpassword#add-authentication-code)。
...
有关设置，请参阅[此处](/oss/python/integrations/tools/azure_dynamic_sessions/#setup)的说明。
...
请遵循[此处](/oss/python/integrations/tools/bing_search)的文档，以获取此工具的详细解释和说明。
