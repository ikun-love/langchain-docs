...
> [CrateDB] 是一个分布式、可扩展的 SQL 数据库，用于实时存储和分析海量数据，即使是复杂的查询也能轻松应对。它兼容 PostgreSQL，基于 Lucene，并继承了 Elasticsearch 的特性。

## 安装和设置

### 设置 CrateDB
有两种快速开始 CrateDB 的方法。或者，您可以选择其他的 [CrateDB 安装选项]。

#### 在本地机器上启动 CrateDB
示例：使用 Docker 或 Podman 运行一个禁用安全功能的单节点 CrateDB 实例。**不推荐在生产环境中使用此方法**。
...
[CrateDB Cloud] 是一项托管式 CrateDB 服务。请注册以获取 [免费试用][CrateDB Cloud Console]。
...
## 文档
有关 CrateDB 包装器的更详细演练，请参阅 [在 CrateDB 中使用 LangChain]。另请参阅 [CrateDB 的所有功能]，了解 CrateDB 提供的其他功能。


## 功能
LangChain 的 CrateDB 适配器提供了 API，可以将 CrateDB 用作向量存储、文档加载器和聊天消息存储。

### 向量存储
围绕 `FLOAT_VECTOR` 和 `KNN_MATCH` 使用 CrateDB 向量存储功能，用于相似性搜索和其他目的。另请参阅 [CrateDBVectorStore 教程]。

请确保已配置有效的 OpenAI API 密钥。
```bash
...
### 聊天消息历史记录
将 CrateDB 用作聊天消息的存储。另请参阅 [CrateDBChatMessageHistory 教程]。
...
### 完整缓存
标准的/完整缓存会避免在遇到的提示与之前遇到的提示完全相同时调用 LLM。另请参阅 [CrateDBCache 示例]。
