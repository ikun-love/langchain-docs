---
title: Google 生成式 AI 嵌入模型 (AI Studio 和 Gemini API)
...
要使用 Google 生成式 AI 模型，您必须拥有一个 API 密钥。您可以在 Google AI Studio 中创建一个。有关说明，请参阅[Google 文档](https://ai.google.dev/gemini-api/docs/api-key)。
...
嵌入模型通常用于检索增强生成 (RAG) 流程中，既可以作为数据索引的一部分，也可以在后续检索中使用。有关更详细的说明，请参阅我们的 [RAG 教程](/oss/python/langchain/rag)。

下面展示了如何使用上面初始化的 `embeddings` 对象来索引和检索数据。在这个例子中，我们将使用 `InMemoryVectorStore` 索引和检索示例文档。
...
`GoogleGenerativeAIEmbeddings` 可选地支持 `task_type` 参数，该参数目前必须是以下值之一：

- `SEMANTIC_SIMILARITY`: 用于生成优化用于评估文本相似性的嵌入。
- `CLASSIFICATION`: 用于生成优化用于根据预设标签对文本进行分类的嵌入。
- `CLUSTERING`: 用于生成优化用于根据相似性对文本进行聚类的嵌入。
- `RETRIEVAL_DOCUMENT`, `RETRIEVAL_QUERY`, `QUESTION_ANSWERING`, 和 `FACT_VERIFICATION`: 用于生成优化用于文档搜索或信息检索的嵌入。
- `CODE_RETRIEVAL_QUERY`: 用于根据自然语言查询（例如排序数组或反转链表）检索代码块。代码块的嵌入是使用 `RETRIEVAL_DOCUMENT` 计算的。

默认情况下，我们在 `embed_documents` 方法中使用 `RETRIEVAL_DOCUMENT`，在 `embed_query` 方法中使用 `RETRIEVAL_QUERY`。如果您提供了 `task_type`，我们将对所有方法使用该类型。
