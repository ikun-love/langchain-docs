...
> 借助 Zep，您可以赋予 AI 助手回忆过去对话的能力，无论这些对话多么久远，
> 同时还能减少幻觉、延迟和成本。
...
在下面的示例中，我们使用了 Zep 的自动嵌入（auto-embedding）功能，该功能使用低延迟嵌入模型在 Zep 服务器上自动嵌入文档。

## 注意

- 这些示例使用 Zep 的异步（async）接口。请通过移除方法名前缀的 `a` 来调用同步（sync）接口。
- 如果您传入一个 [`Embeddings`](https://reference.langchain.com/python/langchain_core/embeddings/#langchain_core.embeddings.embeddings.Embeddings) 实例，Zep 将使用它来嵌入文档，而不是自动嵌入。
您还必须将您的文档集合设置为 `isAutoEmbedded === false`。
- 如果您将集合设置为 `isAutoEmbedded === false`，则必须传入一个 [`Embeddings`](https://reference.langchain.com/python/langchain_core/embeddings/#langchain_core.embeddings.embeddings.Embeddings) 实例。
...
ZEP_API_URL = "http://localhost:8000"  # 您的 Zep 实例的 API url
ZEP_API_KEY = "<optional_key>"  # 您的 Zep 实例的可选 API Key
collection_name = f"babbage{uuid4().hex}"  # 一个唯一的集合名称。仅限字母数字字符

# 如果我们正在创建一个新的 Zep 集合，则需要集合配置
config = CollectionConfig(
    name=collection_name,
    description="<optional description>",
    metadata={"optional_metadata": "associated with the collection"},
    is_auto_embedded=True,  # 我们将让 Zep 使用其低延迟嵌入器来嵌入我们的文档
    embedding_dimensions=1536,  # 这应与您配置 Zep 使用的模型相匹配。
)
...
# 实例化向量存储。由于集合尚未存在于 Zep 中，它将被创建并填充我们传入的文档。
vs = ZepVectorStore.from_documents(
    ...
    api_url=ZEP_API_URL,
    api_key=ZEP_API_KEY,
    embedding=None,  # 我们将让 Zep 使用其低延迟嵌入器来嵌入我们的文档
)
```
...
Zep 提供原生的、硬件加速的搜索结果 MMR（最大边际相关性）重新排序功能。
...
使用元数据过滤器来缩小搜索结果范围。首先，加载另一本书：《福尔摩斯历险记》（Adventures of Sherlock Holmes）
