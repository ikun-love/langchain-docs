...
>[LanceDB](https://lancedb.com/) 是一个开源的向量搜索数据库，它构建在持久化存储之上，极大地简化了向量嵌入的检索、过滤和管理。完全开源。

本 Notebook 展示了如何使用基于 Lance 数据格式的 `LanceDB` 向量数据库相关的功能。
...
我们希望使用 [`OpenAIEmbeddings`](https://reference.langchain.com/python/integrations/langchain_openai/OpenAIEmbeddings) 模型，因此需要获取 OpenAI API 密钥。
...
##### 对于 LanceDB Cloud，您可以按如下方式调用向量存储：

```python
db_url = "db://lang_test" # 您创建的 db 的 url
