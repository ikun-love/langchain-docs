...
[Azure AI 搜索](https://learn.microsoft.com/azure/search/search-what-is-azure-search)（以前称为 `Azure Search` 和 `Azure Cognitive Search`）是一项云搜索服务，它为开发人员提供了基础设施、API 和工具，用于大规模地执行向量、关键字和混合查询的信息检索。

您需要使用 `pip install -qU langchain-community` 安装 `langchain-community` 才能使用此集成。

## 安装 Azure AI 搜索 SDK

请使用 `azure-search-documents` 包的 11.4.0 或更高版本。
...
我们假设您使用的是 [`OpenAIEmbeddings`](https://reference.langchain.com/python/integrations/langchain_openai/OpenAIEmbeddings)，但如果您使用的是 Azure OpenAI，请改用导入 [`AzureOpenAIEmbeddings`](https://reference.langchain.com/python/integrations/langchain_openai/AzureOpenAIEmbeddings)。
...
为您的 OpenAI 提供商设置变量。您需要拥有一个 [OpenAI 账户](https://platform.openai.com/docs/quickstart?context=python) 或一个 [Azure OpenAI 账户](https://learn.microsoft.com/en-us/azure/ai-services/openai/how-to/create-resource) 才能生成嵌入向量（Embeddings）。
...
```python
# 选项 2：使用部署了嵌入模型的 Azure OpenAI 账户
...
要使用此向量存储集成，您需要一个 [Azure 订阅](https://azure.microsoft.com/en-us/free/search) 和一个 [Azure AI 搜索服务](https://learn.microsoft.com/azure/search/search-create-service-portal)。对于小型和有限的工作负载，提供了免费版本（No-cost versions）。

设置您的 Azure AI 搜索 URL 和管理员 API 密钥的变量。您可以从 [Azure 门户](https://portal.azure.com/#blade/HubsExtension/BrowseResourceBlade/resourceType/Microsoft.Search%2FsearchServices) 获取这些变量。
...
创建 [`OpenAIEmbeddings`](https://reference.langchain.com/python/integrations/langchain_openai/OpenAIEmbeddings) 和 AzureSearch 类的实例。完成此步骤后，您的 Azure AI 搜索资源上应该有一个空的搜索索引。集成模块提供了一个默认的模式（schema）。

```python
# 选项 1：将 OpenAIEmbeddings 与 OpenAI 账户一起使用
...
```python
# 选项 2：将 AzureOpenAIEmbeddings 与 Azure 账户一起使用
...
```python
# 指定 Azure 客户端的其他属性，例如以下链接中提到的配置：https://github.com/Azure/azure-sdk-for-python/blob/main/sdk/core/azure-core/README.md#configurations
vector_store: AzureSearch = AzureSearch(
    azure_search_endpoint=vector_store_address,
    azure_search_key=vector_store_password,
    index_name=index_name,
...
此步骤加载、分块（chunk）和向量化示例文档，然后将内容索引到 Azure AI 搜索上的搜索索引中。
...
['M2U1OGM4YzAtYjMxYS00Nzk5LTlhNDgtZTc3MGVkNTg1Mjc0',
 ...
 'YTllYTc5OTgtMzJiNC00ZjZjLWJiMzUtNWVhYzFjYzhmMTI1',
...
使用 `similarity_search_with_relevance_scores()` 方法执行纯向量相似度搜索。不满足阈值要求的查询将被排除。
...
使用 `search_type` 参数或 `hybrid_search()` 方法执行混合搜索。向量字段和非向量文本字段并行查询，合并结果，并返回统一结果集中的前 N 个匹配项。
...
此模式展示了字段定义。它是默认模式，加上几个被标记为可筛选（filterable）的新字段。由于它使用了默认的向量配置，您不会在这里看到向量配置或向量配置文件的覆盖设置。默认向量配置文件的名称是 "myHnswProfile"，它使用 Hierarchical Navigable Small World (HNSW) 向量配置来进行索引，并对 `content_vector` 字段执行查询。

此步骤中没有该模式的数据。当您执行该单元 (cell) 时，您应该在 Azure AI 搜索上获得一个空索引。
...
此示例根据自定义模式向向量存储添加数据。它将文本加载到 `title` 和 `source` 字段中。`source` 字段是可筛选的。本节中的示例查询根据 `source` 字段中的内容筛选结果。

```python
# 包含在 metadata 字典中且在索引中具有对应字段的数据将被添加到索引中。
# 在此示例中，metadata 字典包含 title、source 和一个 random 字段。
# title 和 source 作为单独的字段添加到索引中，但 random 值被忽略，因为它未在模式中定义。
...
这是另一个自定义模式，其中包含评分配置文件（Scoring Profile）的定义。评分配置文件用于对非向量内容进行相关性调整，这在混合搜索场景中非常有用。
...
# 添加一个带有新鲜度函数（freshness function）的自定义评分配置文件
