...
[Vectara](https://vectara.com/) 是一个值得信赖的 AI 助手和代理平台，专注于为任务关键型应用提供企业级就绪能力。
Vectara 的无服务器 RAG-as-a-Service（检索增强生成即服务）通过一个易于使用的 API 提供了 RAG 的所有组件，包括：

1. 一种从文件（PDF、PPT、DOCX 等）中提取文本的方法。
2. 提供最先进性能的基于 ML 的分块（chunking）技术。
...
5. 一个查询服务，它自动将查询编码为嵌入向量，并检索最相关的文本片段，同时支持 [混合搜索](https://docs.vectara.com/docs/api-reference/search-apis/lexical-matching) 以及多种重排（reranking）选项，例如 [多语言相关性重排器](https://www.vectara.com/blog/deep-dive-into-vectara-multilingual-reranker-v1-state-of-the-art-reranker-across-100-languages)、[MMR](https://vectara.com/get-diverse-results-and-comprehensive-summaries-with-vectaras-mmr-reranker/) 和 [UDF 重排器](https://www.vectara.com/blog/rag-with-user-defined-functions-based-reranking)。
6. 一个 LLM（大型语言模型），用于根据检索到的文档（上下文）创建带有引用的 [生成式摘要](https://docs.vectara.com/docs/learn/grounded-generation/grounded-generation-overview)。
...
本教程展示了如何在仅将 Vectara 用作向量存储（不进行摘要）时使用其基本检索功能，包括：`similarity_search` 和 `similarity_search_with_score`，以及如何使用 LangChain 的 `as_retriever` 功能。

## 设置

要使用 `VectaraVectorStore`，您首先需要安装相应的合作伙伴软件包。
...
1. 如果您还没有，请注册 [免费的 Vectara 试用](https://www.vectara.com/integrations/langchain)。
2. 在您的帐户中，您可以创建一个或多个“语料库”（corpus）。每个语料库代表一个存储区域，用于在摄取输入文档时存储文本数据。要创建语料库，请使用 **“Create Corpus”** 按钮。然后为您创建的语料库提供一个名称以及描述。您还可以选择性地定义过滤属性并应用一些高级选项。如果您点击已创建的语料库，您可以在顶部看到其名称和 `corpus ID`（语料库 ID）。
3. 接下来，您需要创建 API 密钥来访问该语料库。点击语料库视图中的 **“Access Control”** 选项卡，然后点击 **“Create API Key”** 按钮。为您的密钥命名，并选择您的密钥是仅查询（query-only）还是查询+索引（query+index）。点击“创建”，您现在就拥有了一个有效的 API 密钥。请妥善保管此密钥。
...
请注意，我们使用的是 `add_files` 接口，它**不需要任何本地处理或分块**——Vectara 接收文件内容并执行所有必要的文件预处理、分块和嵌入操作，将其存入其知识库中。

在本例中，它使用的是 `.txt` 文件，但对于许多其他 [文件类型](https://docs.vectara.com/docs/api-reference/indexing-apis/file-upload/file-upload-filetypes) 也是相同的。
...
- 我们启用摘要功能，并指定我们希望 LLM 选择最相关的 7 个分块并以英文进行响应。

使用此配置，我们创建一个 LangChain `Runnable` 对象，该对象封装了完整的 Vectara RAG 流程，通过使用 `as_rag` 方法实现：
...
## 幻觉检测和事实一致性得分（Factual Consistency Score）

Vectara 创建了 [HHEM](https://huggingface.co/vectara/hallucination_evaluation_model)——一个可用于评估 RAG 响应事实一致性的开源模型。

作为 Vectara RAG 的一部分，FCS（事实一致性得分），它是开源 HHEM 的改进版本，可通过 API 获得。它会自动包含在 RAG 流程的输出中。
...
## Vectara 作为 LangChain 检索器（Retriever）

Vectara 组件也可以仅用作检索器。

在这种情况下，它的行为与任何其他 LangChain 检索器一样。此模式的主要用途是进行语义搜索，在这种情况下，我们禁用摘要功能：
...
为了向后兼容，您也可以启用带有检索器的摘要功能，在这种情况下，摘要将作为额外的 Document 对象添加到检索结果中：
...
[Document(metadata={'X-TIKA:Parsed-By': 'org.apache.tika.parser.csv.TextAndCSVParser', 'Content-Encoding': 'UTF-8', 'X-TIKA:detectedEncoding': 'UTF-8', 'X-TIKA:encodingDetector': 'UniversalEncodingDetector', 'Content-Type': 'text/plain; charset=UTF-8', 'source': 'text_file', 'framework': 'langchain'}, page_content='We won’t be able to compete for the jobs of the 21st Century if we don’t fix that. That’s why it was so important to pass the Bipartisan Infrastructure Law—the most sweeping investment to rebuild America in history. This was a bipartisan effort, and I want to thank the members of both parties who worked to make it happen. We’re done talking about infrastructure weeks. We’re going to have an infrastructure decade.'),
 ...
 Document(metadata={'summary': True, 'fcs': (0.54785156,)}, page_content='President Biden spoke about several key issues. He emphasized the importance of the Bipartisan Infrastructure Law, which he describes as the most significant investment to rebuild America in history. He highlights the bipartisan effort involved in passing this law and expresses gratitude to members of both parties for their collaboration. Biden also announced measures against Russian oligarchs, including assembling a task force to seize their assets and closing American airspace to Russian flights, further isolating Russia economically [2]. Additionally, he expressed a commitment to investigating the health impacts of burn pits on military personnel, referencing his son, Major Beau Biden, who suffered from brain cancer [3].')]
```

## 使用 Vectara 进行高级 LangChain 查询预处理

Vectara 的“RAG 即服务”在创建问答链或聊天机器人链方面承担了大部分繁重的工作。与 LangChain 的集成提供了使用附加功能（如查询预处理，例如 `SelfQueryRetriever` 或 `MultiQueryRetriever`）的选项。让我们看一下使用 [MultiQueryRetriever](https://python.langchain.com/docs/modules/data_connection/retrievers/MultiQueryRetriever) 的示例。

由于 MQR 使用 LLM，我们必须对其进行设置——在这里我们选择 [`ChatOpenAI`](https://reference.langchain.com/python/integrations/langchain_openai/ChatOpenAI)：
