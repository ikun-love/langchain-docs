...
> Azure SQL 提供了一个专用的 [Vector 数据类型](https:\learn.microsoft.com\sql\t-sql\data-types\vector-data-type?view=azuresqldb-current&viewFallbackFrom=sql-server-ver16&tabs=csharp-sample)，它简化了向量嵌入的创建、存储和查询，可以直接在关系数据库中进行。这消除了对单独向量数据库和相关集成的需求，从而提高了解决方案的安全性，同时降低了整体复杂性。

Azure SQL 是一项强大的服务，它集成了可扩展性、安全性和高可用性，提供了现代数据库解决方案的所有优势。它利用复杂的查询优化器和企业级功能，可以在执行传统 SQL 查询的同时执行向量相似性搜索，从而增强数据分析和决策制定能力。

阅读更多关于使用 Azure SQL 数据库构建[智能应用](https://learn.microsoft.com/azure/azure-sql/database/ai-artificial-intelligence-intelligent-applications?view=azuresql)的内容。

此 Notebook 演示了如何利用这个集成的 SQL [向量数据库](https://devblogs.microsoft.com/azure-sql/exciting-announcement-public-preview-of-native-vector-support-in-azure-sql-database/)来存储文档，并使用余弦距离 (Cosine)、L2 (欧几里得距离) 和 IP (内积) 进行向量搜索查询，以定位与查询向量接近的文档。

## 设置 (Setup)

安装 `langchain-sqlserver` Python 包。

代码位于一个名为 `langchain-sqlserver` 的集成包中: [langchain-sqlserver](https:\github.com\langchain-ai\langchain-azure\tree\main\libs\sqlserver)。
...
如果您想对模型的调用进行一流的自动化跟踪，可以取消注释以下内容并设置您的 [LangSmith](https://docs.smith.langchain.com/) API 密钥：
...
请在 Azure 门户中您的数据库设置下查找您的 Azure SQL DB 连接字符串。

更多信息请参阅：[连接到 Azure SQL DB - Python](https:\learn.microsoft.com\en-us\azure\azure-sql\database\connect-query-python?view=azuresql)
...
# 连接字符串可能有所不同：
# "mssql+pyodbc://<username>:<password><servername>/<dbname>?driver=ODBC+Driver+18+for+SQL+Server" -> 指定了用户名和密码
...
在此示例中，我们使用 Azure OpenAI 来生成嵌入（embeddings），但您也可以使用 LangChain 中提供的不同嵌入模型。

您可以按照此[指南](https:\learn.microsoft.com\en-us\azure\ai-services\openai\how-to\create-resource?pivots=web-portal)在 Azure 门户上部署 Azure OpenAI 实例版本。实例运行后，请确保您拥有其实例名称和密钥。您可以在 Azure 门户中，在实例的“密钥和终结点”部分找到密钥。
...
# 使用 AzureChatOpenAI 进行聊天完成 (chat completions)
llm = AzureChatOpenAI(
...
# 使用 AzureOpenAIEmbeddings 进行嵌入 (embeddings)
...
vector_store = SQLServer_VectorStore(
    ...
    embedding_function=embeddings,  # 您可以使用 LangChain 中提供的不同嵌入模型
...
一旦向量存储创建完成并且相关的文档已添加，您很可能希望在链或代理运行时查询它。

执行简单的相似性搜索可以按如下方式完成：

```python
# 在查询嵌入和文档嵌入之间执行相似性搜索 (Perform a similarity search between the embedding of the query and the embeddings of the documents)
...
向量存储支持一组可以应用于文档元数据字段的筛选器。此功能使开发人员和数据分析师能够精炼其查询，确保搜索结果与他们的需求准确对齐。通过根据特定元数据属性应用筛选器，用户可以限制搜索范围，仅关注最相关的数据子集。

```python
# 混合搜索 -> 筛选条件为 id 不等于 1。 (hybrid search -> filter for cases where id not equal to 1.)
...
对于 Azure SQL 向量存储可以执行的不同搜索列表的完整文档，请参阅 [API 参考](https://python.langchain.com/api_reference/sqlserver/index.html)。

### 当您已有要搜索的嵌入时进行相似性搜索

```python
# 如果您已经有要搜索的嵌入 (if you already have embeddings you want to search on)
...
```python
# 如果您已经有要搜索的嵌入，则执行带分数的相似性搜索 (Similarity Search with Score if you already have embeddings you want to search on)
...
### 释放向量存储 (Drop Vector Store)

```python
# 释放向量存储 (drop vectorstore)
vector_store.drop()
```

## 从 Azure Blob 存储加载文档 (Load a Document from Azure Blob Storage)

以下是如何在将文档拆分为块后，将其从 Azure Blob 存储容器加载到 SQL Vector 存储中的示例。[Azure 博客存储](https://learn.microsoft.com/en-us/azure/storage/blobs/storage-blobs-introduction)是 Microsoft 在云中对象存储的解决方案。Blob 存储针对存储海量非结构化数据进行了优化。
...
#### 用例 1：基于故事书的问答系统 (Use Case 1: Q&A System based on the Story Book)

问答功能使用户能够询问有关故事、角色和事件的具体问题，并获得简洁、信息丰富的答案。这不仅增强了他们对书籍的理解，还让他们感觉自己是魔法世界的一部分。

## 通过转换为检索器进行查询 (Query by turning into retriever)

LangChain 向量存储通过启用基于用户查询的高效相似性搜索来查找前 10 个相关文档，从而简化了复杂的问答系统的构建。**检索器** (retriever) 是从 **vector\_store** 创建的，问答链是使用 **create\_stuff\_documents\_chain** 函数构建的。使用 **ChatPromptTemplate** 类创建提示模板，确保响应结构良好且信息丰富。在问答应用中，向用户展示用于生成答案的来源通常很重要。LangChain 内置的 **create\_retrieval\_chain** 会将检索到的源文档通过 “context” 键传播到输出：

阅读更多关于 LangChain RAG 教程和上述术语的介绍 [此处](/oss/python/langchain/rag)
