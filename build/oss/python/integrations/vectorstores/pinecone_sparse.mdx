...
本 Notebook 演示了如何使用与 `Pinecone` 向量数据库相关的各项功能。

## 设置

要使用 `PineconeSparseVectorStore`，您首先需要安装配套包以及本 Notebook 中使用的其他包。
...
请创建一个新的 Pinecone 账户，或登录您现有的账户，并生成一个 API Key 以便在本 Notebook 中使用。
...
在初始化我们的向量存储之前，我们先连接到一个 Pinecone 索引。如果名为 `index_name` 的索引不存在，它将被创建。

```python
from pinecone import AwsRegion, CloudProvider, Metric, ServerlessSpec

index_name = "langchain-sparse-vector-search"  # 如果需要，请更改
...
print(f"索引 `{index_name}` 的主机地址: {index.config.host}")
```

```output
索引 `langchain-sparse-vector-search` 的主机地址: https://langchain-sparse-vector-search-yrrgefy.svc.aped-4627-b74a.pinecone.io
```

对于我们的稀疏嵌入模型，我们使用 [`pinecone-sparse-english-v0`](https://docs.pinecone.io/models/pinecone-sparse-english-v0)，初始化方法如下：
...
现在我们的 Pinecone 索引和嵌入模型都已准备就绪，我们可以初始化 LangChain 中的稀疏向量存储：
...
我们可以使用 `delete` 方法从向量存储中删除记录，只需向其中提供一个要删除的文档 ID 列表即可。
...
将文档加载到向量存储中后，我们很可能就可以开始查询了。LangChain 提供了多种执行此操作的方法。
...
我们还可以向查询中添加 [元数据过滤](https://docs.pinecone.io/guides/data/understanding-metadata#metadata-query-language)，以根据各种标准限制搜索范围。我们尝试一个简单的过滤条件，将搜索限制为仅包含 `source=="social"` 的记录：
...
比较这些结果时，我们可以看到第一个查询返回了来自 `"website"` 源的不同记录。而在后面的过滤查询中，情况不再如此。

### 相似度搜索和分数

我们还可以搜索并返回包含相似度分数的结果，结果形式为 `(document, score)` 元组列表。其中 `document` 是一个 LangChain [`Document`](https://reference.langchain.com/python/langchain_core/documents/#langchain_core.documents.base.Document) 对象，包含我们的文本内容和元数据。
...
在我们的链 (chains) 和代理 (agents) 中，我们通常会将向量存储用作 `VectorStoreRetriever`。要创建它，我们使用 `as_retriever` 方法：
