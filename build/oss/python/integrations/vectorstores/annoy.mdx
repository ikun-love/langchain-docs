...
> [Annoy](https://github.com/spotify/annoy)（全称为 `Approximate Nearest Neighbors Oh Yeah`）是一个 C++ 库，带有 Python 绑定，用于在给定查询点时搜索空间中相近的点。它还可以创建大型的、基于文件的只读数据结构，这些结构被映射到内存中，以便多个进程可以共享相同的数据。

您需要使用 `pip install -qU langchain-community` 安装 `langchain-community` 才能使用此集成。

本 Notebook 演示了如何使用与 `Annoy` 向量数据库相关的各种功能。

```{note}
NOTE: Annoy 是只读的——一旦索引构建完成，就不能再添加新的嵌入向量了！
如果您希望逐步添加新的条目到您的 VectorStore 中，最好选择其他替代方案！
```
