...
[ManticoreSearch](https://manticoresearch.com/) 是一款开源搜索引擎，提供快速、可扩展且用户友好的功能。它起源于 [Sphinx Search](http://sphinxsearch.com/) 的一个分支（Fork），并已发展出许多现代搜索引擎的特性和改进。ManticoreSearch 以其强大的性能和易于集成到各种应用的能力而著称。

ManticoreSearch 最近引入了 [向量搜索功能](https://manual.manticoresearch.com/dev/Searching/KNN)，该功能在搜索引擎 **6.2 版本**中首次提供，并且**仅在安装了** [manticore-columnar-lib](https://github.com/manticoresoftware/columnar) **包时才可用**。这是一个重要的进步，因为它允许基于向量相似性执行搜索。

截至目前，向量搜索功能仅在搜索引擎的开发（dev）版本中可用。因此，要有效地利用此功能，**必须**使用开发中的 [manticoresearch-dev](https://pypi.org/project/manticoresearch-dev/) Python 客户端。

## 设置环境

使用 ManticoreSearch 启动 Docker 容器并安装 manticore-columnar-lib 包（可选）
...
# 以 root 用户安装 manticore-columnar-lib 包
