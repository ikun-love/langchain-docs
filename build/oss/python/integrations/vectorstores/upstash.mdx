...
> [Upstash Vector](https://upstash.com/docs/vector/overall/whatisvector) 是一个无服务器（serverless）的向量数据库，专为处理向量嵌入（vector embeddings）而设计。
>
> LangChain 的 `vector` 集成是对 [upstash-vector](https://github.com/upstash/vector-py) Python 包的封装。
>
> 该 Python 包在后台使用了 [vector rest api](https://upstash.com/docs/vector/api/get-started)。

## 安装 (Installation)

从 [upstash console](https://console.upstash.com/vector) 创建一个免费的向量数据库实例，并配置所需的维度（dimensions）和距离度量（distance metric）。

接下来，您可以通过以下几种方式创建 `UpstashVectorStore` 实例：
...
此外，还需要一个 [`Embeddings`](https://reference.langchain.com/python/langchain_core/embeddings/#langchain_core.embeddings.embeddings.Embeddings) 实例来将给定的文本转换为向量嵌入。这里我们以 `OpenAIEmbeddings` 为例：
...
创建 `UpstashVectorStore` 的另一种方式是 [通过选择一个模型来创建 Upstash Vector 索引](https://upstash.com/docs/vector/features/embeddingmodels#using-a-model)，并将 `embedding=True` 传递给构造函数。在这种配置下，文档或查询将以文本形式发送到 Upstash，并在那里进行嵌入处理。

```python
store = UpstashVectorStore(embedding=True)
```

如果您有兴趣尝试这种方法，可以像上面那样更新 `store` 的初始化代码，然后运行本教程的其余部分。

## 加载文档 (Load documents)

加载一个示例文本文件，并将其分割成可以转换为向量嵌入的块（chunks）。
...
## 插入文档 (Inserting documents)

向量存储使用嵌入对象来嵌入文本块，并将它们批量插入到数据库中。此操作会返回已插入向量的 ID 数组。
...
## 带分数查询 (Querying with score)

查询结果可以包含每个结果的相似度分数（score）。

> 查询返回的分数是一个在 0 到 1 之间的归一化值，其中 1 表示最高相似度，0 表示最低相似度，这与所使用的相似性函数无关。有关更多信息，请参阅 [文档](https://upstash.com/docs/vector/overall/features#vector-similarity-functions)。
...
命名空间（Namespaces）可用于分隔不同类型的文档。这可以通过减少搜索空间来提高查询效率。如果未提供命名空间，则使用默认命名空间。
...
可以在查询结果中通过元数据（Metadata）进行过滤。您可以在 [文档](https://upstash.com/docs/vector/features/filtering) 中查看更复杂的过滤方法。
...
## 获取向量数据库信息 (Getting info about vector database)

您可以使用 `info` 函数获取有关数据库的信息，例如距离度量和维度。

> 当发生插入操作时，数据库会进行索引。在此期间，新的向量无法被查询。`pendingVectorCount` 表示当前正在索引中的向量数量。
...
## 清空向量数据库 (Clearing the vector database)
