...
本指南将帮助您开始使用 IMAP [检索器](/oss/python/integrations/retrievers)。`ImapRetriever` 能够将来自 IMAP 服务器的电子邮件搜索和检索结果转换为 LangChain 的 `Document` 对象。
...
对于使用 docling 进行完整的文档处理（DOCX, PPTX 等，尚未测试）：
...
为了测试目的，您可以使用 GreenMail 在本地设置一个 IMAP 服务器：
...
要使用 `ImapRetriever`，您需要使用 `ImapConfig` 为其配置 IMAP 服务器详细信息：
...
config = ImapConfig(
    ...
    password="your-app-password",  # Gmail 请使用应用密码
    ssl_mode="ssl",
)
...
- **auth_method**: 认证方法（默认值："login"）
- **ssl_mode**: SSL 模式 - "ssl" (默认值), "starttls", 或 "plain"
- **verify_cert**: 对于自签名证书设置为 `False`（生产环境不推荐）
- **k**: 要检索的文档数量
...
- `"full_content"`：使用 docling 从 Office 文档中进行完整提取（需要 `[docling]` 额外依赖项）
...
与其它检索器一样，`ImapRetriever` 可以通过链集成到 LLM 应用中。这是一个完整的示例，它使用 LLM 来生成 IMAP 查询并根据电子邮件内容回答问题：
...
- 'SENTSINCE "01-Oct-2024"' - 自特定日期起的邮件
- 'BODY "meeting"' - 正文中包含特定词语的邮件
- 'FROM "boss@company.com" SUBJECT "urgent"' - 组合条件

重要提示：输出中只包含有效的 IMAP 命令。
重要提示：输出中不要包含任何其他文本。
...
如果您正在使用 GreenMail 测试容器，测试完成后请清理它：
