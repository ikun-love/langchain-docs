...
本示例笔记将介绍如何开始使用 `Cohere RAG` 检索器。它允许您利用搜索各种连接器中**文档**的能力，或通过提供自己的文档来实现。
...
请注意，连接器（connectors）和文档（documents）不能同时使用。如果您在 `invoke` 方法中提供了文档，它们将优先使用，并且对于该特定请求将不使用任何连接器，如上一个代码片段所示！
