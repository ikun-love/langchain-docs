...
## Zep 的检索器示例 (Retriever Example for [Zep](https://docs.getzep.com/))

> 召回、理解并从聊天历史中提取数据。为个性化 AI 体验提供支持。

> [Zep](https://www.getzep.com) 是 AI 助手应用的长期记忆服务。通过 Zep，您可以赋予 AI 助手回忆过去对话的能力，无论这些对话发生多久以前，同时还能减少“幻觉”、延迟和成本。
...
> Zep 开源项目地址：[https://github.com/getzep/zep](https://github.com/getzep/zep)
> Zep 开源文档：[https://docs.getzep.com/](https://docs.getzep.com/)

## 检索器示例 (Retriever Example)

本 Notebook 演示了如何使用 [Zep 长期记忆存储 (Zep Long-term Memory Store)](https://getzep.github.io/) 搜索历史聊天消息记录。
...
2.  对对话历史进行向量搜索：
    1.  对聊天消息进行相似性搜索
    2.  使用最大边际相关性 (MMR) 对聊天消息搜索结果进行重新排序
    3.  使用元数据过滤器过滤搜索结果
    4.  对聊天消息的摘要进行相似性搜索
    5.  使用最大边际相关性 (MMR) 对摘要搜索结果进行重新排序
...
### 初始化 Zep 聊天消息历史记录类，并将聊天消息历史记录添加到内存存储中

**注意：** 与其他检索器不同，Zep 检索器返回的内容是针对特定会话/用户的。实例化检索器时，必须提供 `session_id`。

```python
# 提供您的 Zep API 密钥。注意，这是可选的。请参阅 https://docs.getzep.com/deployment/auth
AUTHENTICATE = False
...
```python
# 预加载一些消息到内存中。默认的消息窗口大小是 12 条消息。我们希望超出这个限制，以演示自动摘要功能。
test_history = [
...
Zep 为历史对话内存提供了原生的向量搜索功能。嵌入（Embedding）是自动完成的。

注意：消息的嵌入是异步发生的，因此第一次查询可能不会返回结果。后续查询会在生成嵌入后返回结果。
...
Zep 原生支持使用 MMR 对结果进行重新排序，并经过 SIMD 加速。这对于消除结果中的冗余信息非常有用。
...
Zep 会自动生成聊天消息的摘要。您可以使用 Zep 检索器对这些摘要进行搜索。由于摘要是对对话的提炼，它们更有可能匹配您的搜索查询，并为 LLM 提供丰富、简洁的上下文。

连续生成的摘要可能包含相似的内容，Zep 的相似性搜索会返回匹配度最高的但多样性较差的结果。
MMR 会对结果进行重新排序，以确保您填充到 Prompt 中的摘要既相关，又为 LLM 提供了额外信息。
