...
[Dappier](https://dappier.com) 能够将任何大型语言模型（LLM）或您的智能体 AI 连接到来自可信来源的实时、经过授权的专有数据，从而使您的 AI 成为任何领域的专家。我们提供的专业模型包括实时网络搜索、新闻、体育、金融股票市场数据、加密货币数据，以及来自高级出版商的独家内容。欢迎访问 [marketplace.dappier.com](https://marketplace.dappier.com) 探索我们市场中提供的各种数据模型。

[Dappier](https://dappier.com) 提供经过丰富处理、可直接用于提示（prompt-ready）且与上下文相关的**数据字符串**，这些数据经过优化，可与 LangChain 无缝集成。无论您是构建对话式 AI、推荐引擎还是智能搜索，Dappier 与 LLM 无关的 RAG 模型都能确保您的 AI 能够访问经过验证的最新数据，而无需您自行构建和管理检索管道的复杂性。

# DappierRetriever

这将帮助您开始使用 Dappier 的 [检索器](https://python.langchain.com/docs/concepts/retrievers/)（retriever）。有关 `DappierRetriever` 所有功能的详细文档和配置，请前往 [API 参考](https://python.langchain.com/en/latest/retrievers/langchain_dappier.retrievers.Dappier.DappierRetriever.html)。
...
您可以通过访问 [Dappier 市场](https://platform.dappier.com/marketplace) 来查找支持的数据模型。

如果您想从单个查询中获取自动化的追踪（tracing）记录，您还可以通过取消注释以下内容来设置您的 [LangSmith](https://docs.smith.langchain.com/) API 密钥：
...
- data\_model\_id: str
    数据模型 ID，以 `dm_` 开头。
    您可以在 [Dappier 市场](https://platform.dappier.com/marketplace) 上找到可用的数据模型 ID。
- k: int
    要返回的文档数量。
- ref: Optional[str]
    显示 AI 推荐内容的网站域名。
- num\_articles\_ref: int
    从指定的 `ref` 域获取的最小文章数量。
    其余部分将来自 RAG 模型中的其他站点。
- search\_algorithm: Literal[
    "most\_recent",
    "most\_recent\_semantic",
    "semantic",
    "trending"
]
    用于检索文章的搜索算法。
...
有关 `DappierRetriever` 所有功能的详细文档和配置，请前往 [API 参考](https://python.langchain.com/en/latest/retrievers/langchain_dappier.retrievers.Dappier.DappierRetriever.html)。
