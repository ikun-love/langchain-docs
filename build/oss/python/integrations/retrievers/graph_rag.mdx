...
[检索器](/oss/python/langchain/retrieval/)，它将基于矢量的**非结构化**相似性搜索与**结构化**的元数据属性遍历相结合。这使得能够对**现有**向量存储执行基于图的检索。
...
<Tabs groupId="vector-store" queryString>
    <Tab title="AstraDB">
      ...
      [AstraDB 向量存储指南](/oss/python/integrations/vectorstores/astradb)。
