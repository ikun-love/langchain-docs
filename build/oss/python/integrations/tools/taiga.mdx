...
本指南提供了 Taiga 工具（位于 `langchain_taiga` 中）的快速入门概览（[链接到文档](https://github.com/Shikenso-Analytics/langchain-taiga/blob/main/docs/tools.ipynb)）。有关每个工具和配置的更多详细信息，请参阅您的代码库中的文档字符串或相关的文档页面。
...
- **`create_entity_tool`**: 在 Taiga 中创建用户故事（user stories）、任务（tasks）和问题（issues）。
- **`search_entities_tool`**: 在 Taiga 中搜索用户故事、任务和问题。
- **`get_entity_by_ref_tool`**: 通过引用（reference）获取用户故事、任务或问题。
...
此集成要求您将 `TAIGA_URL`、`TAIGA_API_URL`、`TAIGA_USERNAME`、`TAIGA_PASSWORD` 和 `OPENAI_API_KEY` 设置为环境变量，以便向 Taiga 进行身份验证。
...
设置 [LangSmith](https://smith.langchain.com/) 也有助于实现一流的可观测性（但这不是必需的）：
...
下面是一个示例，展示了如何在 `langchain_taiga` 中实例化 Taiga 工具。请根据您的具体用途进行调整。
...
下面是一个简单的示例，展示了如何使用字典中的关键字参数调用工具。
...
如果您有一个模型生成的 `ToolCall`，请以如下格式将其传递给 `tool.invoke()`。
...
下面是一个更完整的示例，展示了如何在链或 Agent 中集成 `create_entity_tool` 和 `search_entities_tool` 以及 LLM。此示例假定您有一个函数（如 [`create_agent`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent)）可以设置一个能够根据需要调用工具的 LangChain 风格的 Agent。

```python
# 示例：在 Agent 中使用 Taiga 工具
