...
本指南简要介绍了如何开始使用 Jina [工具](/oss/python/integrations/tools/)。有关 Jina 所有功能和配置的详细文档，请参阅[API 参考文档](https://python.langchain.com/api_reference/community/tools/langchain_community.tools.jina_search.tool.JinaSearch.html)。
...
| [返回产物](/oss/python/langchain/tools) | 原生异步 (Native async) | 返回数据 | 定价 (Pricing) |
| :---: | :---: | :---: | :---: |
| ❌ | ❌ | URL, 摘要 (Snippet), 标题 (Title), 页面内容 (Page Content) | 100 万响应 token 免费 |

## 设置

该集成存在于 `langchain-community` 包中，并添加于版本 `0.2.16`：
...
if not os.environ.get("JINA_API_KEY"):
    os.environ["JINA_API_KEY"] = getpass.getpass("Jina API 密钥:\n")
```

设置 [LangSmith](https://smith.langchain.com/) 也很有帮助（但不是必需的），以获得一流的可观测性：
