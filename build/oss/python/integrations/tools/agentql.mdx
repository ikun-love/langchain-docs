...
[AgentQL](https://www.agentql.com/) 工具提供使用 AgentQL 查询或自然语言提示与任何网页进行交互，并从中提取结构化数据的功能。AgentQL 可以在多种语言和网页上使用，且不易因时间或页面变化而失效。

## 概述

AgentQL 提供了以下三种工具：

- **`ExtractWebDataTool`**：根据提供的 URL，使用 [AgentQL 查询](https://docs.agentql.com/agentql-query/query-intro) 或对所需数据的自然语言描述，从网页中提取结构化数据（JSON 格式）。

以下两种工具已捆绑为 `AgentQLBrowserToolkit`，必须与 `Playwright` 浏览器或通过 Chrome 开发者工具协议 (CDP) 连接的远程浏览器实例配合使用：

- **`ExtractWebDataBrowserTool`**：使用 [AgentQL 查询](https://docs.agentql.com/agentql-query/query-intro) 或对所需数据的自然语言描述，从浏览器中活动网页提取结构化数据（JSON 格式）。

- **`GetWebElementBrowserTool`**：使用自然语言描述在浏览器中查找活动网页上的一个网页元素，并返回其 CSS 选择器，以便进行进一步交互。

### 集成详情

| 类名 | 包名 | 可序列化 | [JS 支持](https://js.langchain.com/docs/integrations/tools/langchain_agentql) | 版本 |
| :--- | :--- | :---: | :---: | :---: |
| AgentQL | langchain-agentql | ❌ | ❌ | 1.0.0 |

### 工具特性

| 工具 | 网页数据提取 | 网页元素提取 | 可与本地浏览器一起使用 |
| :--- | :---: | :---: | :---: |
| ExtractWebDataTool | ✅ | ❌ | ❌
...
要运行此 Notebook，需要安装 `Playwright` 浏览器并配置 Jupyter Notebook 的 `asyncio` 事件循环。

```python
!playwright install

# 仅在 Jupyter Notebooks 中需要此导入，因为它们有自己的事件循环
import nest_asyncio
...
要使用 AgentQL 工具，您需要从 [AgentQL 开发者门户](https://dev.agentql.com/) 获取自己的 API 密钥，并设置 AgentQL 环境变量。
...
- `timeout`: 等待请求超时的时间（秒）。如果数据提取超时，请增加此值。**默认为 `900`**。
- `is_stealth_mode_enabled`: 是否启用实验性的反机器人规避策略。此功能可能并非在所有网站上始终有效。启用此模式可能会延长数据提取的完成时间。**默认为 `False`**。
- `wait_for`: 在提取数据之前等待页面加载的时间（秒）。**默认为 `0`**。
- `is_scroll_to_bottom_enabled`: 在提取数据之前是否滚动到页面底部。**默认为 `False`**。
- `mode`: `"standard"` 使用深度数据分析，而 `"fast"` 则以速度换取部分分析深度，适用于大多数用例。[请在此指南中了解关于模式的更多信息](https://docs.agentql.com/accuracy/standard-mode)。**默认为 `"fast"`**。
- `is_screenshot_enabled`: 在提取数据前是否截取屏幕截图。截图将作为 Base64 字符串返回在 'metadata' 字段中。**默认为 `False`**。

`ExtractWebDataTool` 是使用 AgentQL 的 REST API 实现的，您可以在 [API 参考文档](https://docs.agentql.com/rest-api/api-reference) 中查看有关参数的更多详细信息。
...
- `timeout`: 等待请求超时的时间（秒）。如果数据提取超时，请增加此值。**默认为 `900`**。
- `wait_for_network_idle`: 在执行前是否等待网络完全空闲。**默认为 `True`**。
- `include_hidden`: 在提取时是否考虑视觉上隐藏的元素。**默认为 `True`**。
- `mode`: `"standard"` 使用深度数据分析，而 `"fast"` 则以速度换取部分分析深度，适用于大多数用例。[请在此指南中了解关于模式的更多信息](https://docs.agentql.com/accuracy/standard-mode)。**默认为 `"fast"`**。

`ExtractWebDataBrowserTool` 是使用 AgentQL 的 SDK 实现的。您可以在 AgentQL 的 [API 参考](https://docs.agentql.com/python-sdk/api-references/agentql-page#querydata) 中找到有关参数和函数的更多详细信息。
...
- `timeout`: 等待请求超时的时间（秒）。如果数据提取超时，请增加此值。**默认为 `900`**。
- `wait_for_network_idle`: 在执行前是否等待网络完全空闲。**默认为 `True`**。
- `include_hidden`: 在提取时是否考虑视觉上隐藏的元素。**默认为 `False`**。
- `mode`: `"standard"` 使用深度数据分析，而 `"fast"` 则以速度换取部分分析深度，适用于大多数用例。[请在此指南中了解关于模式的更多信息](https://docs.agentql.com/accuracy/standard-mode)。**默认为 `"fast"`**。

`GetWebElementBrowserTool` 是使用 AgentQL 的 SDK 实现的。您可以在 AgentQL 的 [API 参考](https://docs.agentql.com/python-sdk/api-references/agentql-page#queryelements) 中找到有关参数和函数的更多详细信息。
...
此工具在底层使用 AgentQL 的 REST API，将公开网页的 URL 发送到 AgentQL 端点。这不适用于私有页面或已登录的会话。对于此类用例，请使用 `ExtractWebDataBrowserTool`。

- `url`: 要从中提取数据的网页的 URL。
- `query`: 要执行的 AgentQL 查询。如果您想提取精确的结构化数据，请使用 AgentQL 查询。了解有关[如何在文档中编写 AgentQL 查询](https://docs.agentql.com/agentql-query)或在 [AgentQL Playground](https://dev.agentql.com/playground) 中测试查询的更多信息。
- `prompt`: 对要从页面中提取的数据的自然语言描述。AgentQL 将从您的提示中推断出数据的结构。如果您想提取由自由文本描述的数据而无需定义特定结构，请使用 `prompt`。

**注意：** 使用 AgentQL 时，您必须定义 `query` 或 `prompt` 之一。

```python
# 您可以使用 query 或 prompt 来调用此工具
...
# 禁用 'visible_only' 将允许我们点击页面上不可见的元素
...
您可以通过首先将 AgentQL 工具绑定到 [工具调用模型](/oss/python/langchain/tools/)，然后调用它，在链中（chain）使用 AgentQL 工具：
...
您可以使用 `AgentQLBrowserToolkit` 将 AgentQL 工具与 AI 代理（Agent）一起使用。此工具包包含 `ExtractDataBrowserTool` 和 `GetWebElementBrowserTool`。下面是一个将 AgentQL 工具包与 Playwright 工具相结合的代理浏览器操作示例。
...
### 提及 “AI” 的新闻标题 (News Titles Mentioning "AI"):
1. 第 1 页：**AI is blurring the line between PMs and engineers?**
2. 第 2 页：
