...
本 Notebook 展示了如何使用 [Infobip](https://www.infobip.com/) API 封装库来发送短信和电子邮件。

Infobip 提供了许多服务，但本 Notebook 将重点介绍 SMS（短信）和 Email（电子邮件）服务。您可以 [此处](https://www.infobip.com/docs/api) 找到有关 API 和其他渠道的更多信息。

## 设置 (Setup)

要使用此工具，您需要拥有一个 Infobip 账户。您可以创建一个 [免费试用账户](https://www.infobip.com/docs/essentials/free-trial)。

`InfobipAPIWrapper` 使用命名参数来提供凭证信息：

- `infobip_api_key` - [API 密钥](https://www.infobip.com/docs/essentials/api-authentication#api-key-header)，您可以在 [开发者工具](https://portal.infobip.com/dev/api-keys) 中找到它。
- `infobip_base_url` - Infobip API 的 [基础 URL](https://www.infobip.com/docs/essentials/base-url)。您可以使用默认值 `https://api.infobip.com/`。

您也可以将 `infobip_api_key` 和 `infobip_base_url` 作为环境变量 `INFOBIP_API_KEY` 和 `INFOBIP_BASE_URL` 来提供。
