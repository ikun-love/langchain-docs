...
本教程将介绍如何使用 `Google Serper` 组件来搜索网络。首先，您需要在 [serper.dev](https://serper.dev) 注册一个免费账户并获取您的 API 密钥。
...
## 作为 Self Ask With Search Agent 的一部分

为了创建一个使用 Google Serper 工具的智能体（Agent），您需要安装 LangGraph：

```python
pip install -qU langgraph langchain-openai
```

然后使用 `[`create_agent`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent)` 功能来初始化一个 ReAct 智能体。您还需要设置您的 `OPENAI_API_KEY`（访问 [platform.openai.com](https://platform.openai.com)）以便访问 OpenAI 的聊天模型。
...
如果您希望以结构化的方式（包括元数据）获取搜索结果，我们将使用包装器（wrapper）的 `results` 方法。
 ...
 'news': [{'title': 'Tesla stock soars after DoT unveils new self-driving car '
          ...
          {'title': 'Tesla, Inc. (TSLA) Faces Weak Q1 Auto Revenues but '
           ...
           'snippet': 'We recently compiled a list of the 12 AI Stocks '
                      ...
                      'we are going to take a look at where Tesla, Inc.',
           'date': '1 day ago',
           'source': 'Yahoo Finance',
...
要查看所有支持的过滤器，请访问 [Google Search](https://google.com)，搜索一些内容，点击“工具 (Tools)”，添加您的日期过滤器，然后查看 URL 中的 `tbs=` 参数即可。

## 搜索 Google 热门地点 (Places)

我们也可以使用此包装器查询 Google 热门地点。例如：
