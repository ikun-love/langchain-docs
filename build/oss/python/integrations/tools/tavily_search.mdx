...
[Tavily 搜索引擎 API](https://tavily.com) 专为 AI 智能体（LLM）构建，能够快速提供实时、准确且基于事实的搜索结果。

## 概述

### 集成详情

| 类 (Class) | 包 (Package) | 可序列化 (Serializable) | [JS 支持](https://js.langchain.com/docs/integrations/tools/tavily_search) | 版本 (Version) |
|:---|:---|:---:|:---:|:---|
...
| [返回工件](/oss/python/langchain/tools) | 原生异步 (Native async) | 返回数据 (Return data) | 定价 (Pricing) |
|:---:|:---:|:---|:---|
| ❌ | ✅ | title（标题）, URL, content snippet（内容片段）, raw\_content（原始内容）, answer（答案）, images（图片） | 每月 1,000 次免费搜索 |
...
我们还需要设置 Tavily API 密钥。您可以通过访问 [此站点](https://app.tavily.com/sign-in) 创建账户并获取 API 密钥。
...
下面演示了如何实例化一个 Tavily 搜索工具实例。该工具接受各种参数来自定义搜索行为。实例化后，我们使用一个简单的查询来调用该工具。此工具允许您使用 Tavily 的 Search API 端点来完成搜索查询。

实例化
工具在实例化期间接受各种参数：

- `max_results`（可选, int）：返回的最大搜索结果数量。默认为 5。
- `topic`（可选, str）：搜索的类别。可以是 "general"（通用）、"news"（新闻）或 "finance"（金融）。默认为 "general"。
- `include_answer`（可选, bool）：在结果中包含对原始查询的回答。默认为 False。
- `include_raw_content`（可选, bool）：包含每个搜索结果的清理和解析后的 HTML 内容。默认为 False。
- `include_images`（可选, bool）：在响应中包含与查询相关的图片列表。默认为 False。
- `include_image_descriptions`（可选, bool）：包含每张图片的描述性文本。默认为 False。
- `search_depth`（可选, str）：搜索的深度，可以是 "basic"（基础）或 "advanced"（高级）。默认为 "basic"。
- `time_range`（可选, str）：从当前日期回溯以过滤结果的时间范围 —— "day"（天）、"week"（周）、"month"（月）或 "year"（年）。默认为 None。
- `include_domains`（可选, List[str]）：要明确包含的域名列表。默认为 None。
- `exclude_domains`（可选, List[str]）：要明确排除的域名列表。默认为 None。

有关可用参数的全面概述，请参阅 **Tavily Search API 文档**：[docs.tavily.com/documentation/api-reference/endpoint/search](https://docs.tavily.com/documentation/api-reference/endpoint/search)
...
- `query`（必需）：自然语言搜索查询。
- 在调用时也可以设置以下参数：`include_images`、`search_depth`、`time_range`、`include_domains`、`exclude_domains`、`include_images`。
- 出于可靠性和性能考虑，影响响应大小的某些参数在调用期间**不能修改**：`include_answer` 和 `include_raw_content`。这些限制旨在防止意外的上下文窗口问题，并确保结果一致性。

注意：可选参数允许智能体动态设置。如果您在实例化时设置了某个参数，然后在调用工具时传入了不同的值，工具将使用您在调用时传入的值。
...
我们可以通过将工具绑定到智能体来与智能体执行器直接使用我们的工具。这使得智能体能够动态设置 Tavily 搜索工具的可用参数。

在以下示例中，当我们要求智能体查找“2024 年欧洲杯的主办国是哪个？仅包含维基百科来源。”时，智能体将动态设置参数并调用 Tavily 搜索工具：在调用 `tavily_search` 时，它会传入 `{'query': 'Euro 2024 host nation', 'include_domains': ['wikipedia.org']}`。
...
2024 年欧洲杯的主办国是 德国。您可以在 [UEFA Euro 2024 的维基百科页面](https://en.wikipedia.org/wiki/UEFA_Euro_2024) 上找到更多信息。
```
...
有关所有 Tavily 搜索 API 功能和配置的详细文档，请访问 API 参考：[docs.tavily.com/documentation/api-reference/endpoint/search](https://docs.tavily.com/documentation/api-reference/endpoint/search)
