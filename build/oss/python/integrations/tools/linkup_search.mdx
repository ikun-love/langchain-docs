...
> [Linkup](https://www.linkup.so/) 提供了一个 API，用于连接大型语言模型（LLM）到互联网以及 Linkup 高级合作伙伴资源。

本指南快速概述了如何开始使用 `LinkupSearchTool` [工具](/oss/python/langchain/tools)。要获取所有 `LinkupSearchTool` 功能和配置的详细文档，请访问 [API 参考](https://python.langchain.com/api_reference/linkup/tools/linkup_langchain.search_tool.LinkupSearchTool.html)。

## 概述

### 集成详情

| 类 (Class) | 包 (Package) | 可序列化 (Serializable) | [JS 支持](https://js.langchain.com/docs/integrations/tools/linkup_search) | 版本 (Version) |
| :--- | :--- | :---: | :---: | :---: |
...
要使用 Linkup 提供者，您需要一个有效的 API 密钥，您可以通过 [此处](https://app.linkup.so/sign-up) 注册找到。要运行以下示例，您还需要一个 OpenAI API 密钥。
...
设置 [LangSmith](https://smith.langchain.com/) 也有帮助（但非必需），以便获得一流的可观测性：
...
这里我们展示了如何实例化一个 `LinkupSearchTool` 工具实例，并进行如下配置：
...
要获取所有 `LinkupSearchTool` 功能和配置的详细文档，请访问 [API 参考](https://python.langchain.com/api_reference/linkup/tools/linkup_langchain.search_tool.LinkupSearchTool.html)。
