...
本笔记将介绍如何使用自托管的 `SearxNG` 搜索 API 来进行网络搜索。

你可以[点击此链接](https://docs.searxng.org/dev/search_api.html)，查看关于 `Searx API` 参数的更多信息。
...
对于某些搜索引擎，如果直接存在一个 `answer`（答案），包装器（wrapper）会打印该答案而不是完整的搜索结果列表。如果你想获取所有结果，可以使用包装器的 `results` 方法。
...
SearxNG 支持 [135 个搜索引擎](https://docs.searxng.org/user/configured_engines.html)。你还可以使用任意命名的参数来自定义 Searx 包装器，这些参数将被传递给 Searx 搜索 API。在下面的示例中，我们将更深入地使用来自 searx 搜索 API 的自定义搜索参数。

在这个示例中，我们将使用 `engines` 参数来查询维基百科（wikipedia）。
...
## 获取带元数据的结果 (Obtaining results with metadata)

在这个示例中，我们将使用 `categories` 参数查找科学论文，并限制结果在特定的 `time_range`（时间范围）（并非所有引擎都支持时间范围选项）。
...
在此示例中，我们查询类别为 `it` (信息技术) 的“大语言模型”（large language models），并筛选出结果来源为 `github` 的结果。
...
我们也可以直接查询来自 `github` 和其他源码托管平台的结果。
...
<Callout icon="pen-to-square" iconType="regular">
    [Edit the source of this page on GitHub.](https://github.com/langchain-ai/docs/edit/main/src/oss\python\integrations\tools\searx_search.mdx)
</Callout>
