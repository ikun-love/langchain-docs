...
[Vectara](https://vectara.com/) 是一个值得信赖的 AI 助手和智能体平台，专注于为关键任务型应用程序提供企业级就绪能力。更多[详情](../providers/vectara.ipynb)。

[Vectara](https://vectara.com/) 提供了几个可与 LangChain 配合使用的工具。

- **VectaraSearch**: 用于对您的语料库进行语义搜索。
- **VectaraRAG**: 用于使用 RAG（检索增强生成）生成摘要。
- **VectaraIngest**: 用于将文档摄取（Ingest）到您的语料库中。
- **VectaraAddFiles**: 用于上传文件。

## 设置 (Setup)

要使用 `Vectara Tools`，您首先需要安装配套包。
...
1. 如果您还没有账户，请[注册](https://www.vectara.com/integrations/langchain)您的免费 Vectara 试用版。
2. 在您的账户中，您可以创建一个或多个语料库（corpora）。每个语料库代表一个区域，用于在从输入文档摄取数据时存储文本数据。要创建一个语料库，请使用 **"Create Corpus"**（创建语料库）按钮。然后您需要为您的语料库提供一个名称以及一个描述。您可以选择性地定义过滤属性并应用一些高级选项。如果点击您创建的语料库，您可以在顶部看到其名称和 `corpus ID`（语料库 ID）。
3. 接下来，您需要创建 API 密钥（API keys）来访问该语料库。点击语料库视图中的 **"Access Control"**（访问控制）选项卡，然后点击 **"Create API Key"**（创建 API 密钥）按钮。给您的密钥命名，并选择您希望该密钥拥有查询（query-only）权限还是查询+索引（query+index）权限。点击 "Create"，您现在就拥有了一个活动的 API 密钥。请妥善保管此密钥。

要将 LangChain 与 Vectara 结合使用，您需要拥有这两个值：`corpus_key` 和 `api_key`。
您可以通过两种方式为 LangChain 提供 `VECTARA_API_KEY`：
...
2. 将它们添加到 `Vectara` 向量存储（vectorstore）的构造函数中：
...
请注意，我们使用了 `VectaraAddFiles` 工具，该工具**不需要**任何本地处理或分块（chunking）——Vectara 接收文件内容，并执行所有必要的事前处理、分块和嵌入操作，将其存入其知识库中。

在这种情况下，使用的是 `.txt` 文件，但对于许多其他[文件类型](https://docs.vectara.com/docs/api-reference/indexing-apis/file-upload/file-upload-filetypes)，操作是相同的。
...
我们现在创建一个 `VectaraQueryConfig` 对象来控制检索和总结选项：
- 我们启用总结功能，指定我们希望 LLM 选择最相关的 7 个匹配块（chunks）并以英文回复。

使用此配置，让我们创建一个 LangChain 工具 `VectaraRAG` 对象，它封装了完整的 Vectara RAG 流程：
...
## 将 Vectara 作为 LangChain 检索器 (Vectara as a langchain retriever)

`VectaraSearch` 工具可以仅用作检索器（retriever）。

在这种模式下，它的行为与任何其他 LangChain 检索器一样。此模式的主要用途是进行语义搜索，在这种情况下，我们禁用总结功能：
...
## 使用 LangChain 检索器链接 (Chaining with Vectara tools)

您可以将 Vectara 工具与其他 LangChain 组件链接（Chain）起来。该示例展示了如何：

- 设置 `ChatOpenAI` 模型以进行额外的处理。
- 创建自定义的提示模板（Prompt Template）以满足特定的总结需求。
- 使用 LangChain 的 Runnable 接口链接多个组件。
- 处理和格式化来自 Vectara 的 JSON 响应。
...
以下代码演示了如何使用 Vectara 工具和 LangChain 来创建一个智能体（agent）。
...
`VectaraIngest` 工具允许您直接将文本内容摄取（ingest）到您的 Vectara 语料库中。当您拥有需要添加到语料库中的文本内容，而无需先创建文件时，此功能非常有用。
...
如需了解详细信息，请查看 Vectara [工具](https://github.com/vectara/langchain-vectara/blob/main/libs/vectara/langchain_vectara/tools.py)的实现。

---

<Callout icon="pen-to-square" iconType="regular">
    [Edit the source of this page on GitHub.](https://github.com/langchain-ai/docs/edit/main/src/oss\python\integrations\tools\vectara.mdx)
</Callout>
