...
本 Notebook 将引导您完成 LangChain 连接 `Amadeus` 旅游 API 的过程。

这个 `Amadeus` 工具包使代理在处理旅行决策时能够做出选择，特别是关于搜索和预订涉及航班的行程。

要使用此工具包，您需要准备好 Amadeus API 密钥，相关信息请参阅 [Amadeus 自助服务 API 入门指南](https://developers.amadeus.com/get-started/get-started-with-self-service-apis-335)。一旦您获得了 `AMADEUS_CLIENT_ID` 和 `AMADEUS_CLIENT_SECRET`，就可以在下面将其作为环境变量输入。

注意：Amadeus 自助服务 API 提供了一个测试环境，其中包含[有限的免费数据](https://amadeus4dev.github.io/developer-guides/test-data/)。这使得开发人员可以在部署到生产环境之前构建和测试其应用程序。要访问实时数据，您需要[迁移到生产环境](https://amadeus4dev.github.io/developer-guides/API-Keys/moving-to-production/)。
...
该工具包将读取 `AMADEUS_CLIENT_ID` 和 `AMADEUS_CLIENT_SECRET` 环境变量来对用户进行身份验证，因此您需要在**此处**设置它们。
...
默认情况下，`AmadeusToolkit` 使用 [`ChatOpenAI`](https://reference.langchain.com/python/integrations/langchain_openai/ChatOpenAI) 来识别离给定位置最近的机场。要使用它，只需设置 `OPENAI_API_KEY` 即可。
