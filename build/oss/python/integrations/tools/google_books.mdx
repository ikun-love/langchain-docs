...
支持 ReAct 模式的 Google Books 工具，允许您搜索 Google Books API。Google Books 是全球最大的 API 之一，以精选的方式跟踪图书信息。它拥有超过 4000 万条记录，可以为用户提供海量数据。
...
您需要一个来自 Google Books 的 API 密钥。您可以通过访问 [https://developers.google.com/books/docs/v1/using#APIKey](https://developers.google.com/books/docs/v1/using#APIKey) 并遵循步骤来获取。

然后，您需要将环境变量 `GOOGLE_BOOKS_API_KEY` 设置为您 Google Books API 密钥的值。

## 实例化

要实例化此工具，请导入 Google Books 工具并设置您的凭证。
...
### [使用工具调用（ToolCall）](/oss/python/langchain/tools)
...
Google Books API 可以在此处找到：[https://developers.google.com/books](https://developers.google.com/books)
```

---

<Callout icon="pen-to-square" iconType="regular">
    [Edit the source of this page on GitHub.](https://github.com/langchain-ai/docs/edit/main/src/oss\python\integrations\tools\google_books.mdx)
</Callout>
