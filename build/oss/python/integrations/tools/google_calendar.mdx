...
> [Google Calendar](https://workspace.google.com/intl/en-419/products/calendar/) 是 Google Workspace 的一个产品，它允许用户组织他们的日程和事件。它是一个基于云的日历，用户可以在其中创建、编辑和删除事件。它还允许用户与其他
人共享他们的日历。

## 概述

本 Notebook 将帮助您开始使用 Google 日历工具包 (Google Calendar Toolkit)。此工具包与 Google 日历 API 交互，以对日历执行各种操作。它允许您：
...
1. 拥有可以访问 Google 日历的 Google 帐户。
2. 按照 [Google 日历 API 文档](https://developers.google.com/calendar/api/quickstart/python#authorize_credentials_for_a_desktop_application) 中所述设置您的凭据。下载 `credentials.json` 文件后，您就可以开始使用 Google 日历 API 了。
...
此工具包位于 [langchain-google](https://github.com/langchain-ai/langchain-google) 仓库的 `langchain-google-community` 包中。我们需要 `calendar` 额外依赖：
...
默认情况下，该工具包会读取本地的 `credentials.json` 文件。您也可以手动提供一个 `Credentials` 对象。
...
在底层，会使用以下方法创建一个 `googleapi` 资源。您可以手动构建一个 `googleapi` 资源以获得更多的身份验证控制。
...
# 可以在此处查看作用域 (scopes)：https://developers.google.com/calendar/api/auth
...
您可以通过以字典格式传递所需参数来直接调用该工具。下面是使用 `CalendarCreateEvent` 工具创建新事件的示例。
...
下面展示了如何将工具包集成到 [Agent](/oss/python/langchain/agents) 中。
...
```python
example_query = "为今天下午创建一个绿色的事件，内容是跑步 30 分钟。"
...
- 有关更多详细信息，请参阅 [Google 日历 API 概览](https://developers.google.com/calendar/api/guides/overview)。
- 有关所有 Google 日历工具包功能的详细文档和配置，请访问 [日历文档](https://python.langchain.com/api_reference/google_community/calendar.html)。
