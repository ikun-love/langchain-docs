...
如需获取更多附加信息（例如链接、来源），请使用 `DuckDuckGoSearchResults()`
...
默认情况下，结果将作为原始搜索结果的逗号分隔的键值对字符串返回。您也可以通过设置 `output_format="list"` 将搜索结果作为列表返回，或通过设置 `output_format="json"` 作为 JSON 字符串返回。
...
您也可以仅搜索新闻文章。使用关键词 `backend="news"`
...
您也可以直接向 `DuckDuckGoSearchResults` 传递一个自定义的 `DuckDuckGoSearchAPIWrapper`，以便对搜索结果进行更精细的控制。
