...
此示例展示了如何使用 `langchain-ibm` 中的 `watsonx.ai` SQL 数据库工具包（Toolkit），该工具包使用了 Flight 服务。

<Warning>
构建 SQL 数据库的问答系统需要执行模型生成的 SQL 查询，这带有固有的安全风险。请确保您的数据库连接权限始终根据您 Agent 的需求进行尽可能严格的范围限制。这将有助于减轻（但不能完全消除）构建模型驱动的系统的风险。
</Warning>
...
**操作:** 提供 IBM Cloud 用户 API 密钥。详情请参见
...
要设置 SQL 数据库工具包，您必须首先实例化 `WatsonxSQLDatabase` 类。该类通过 Flight SQL 客户端从 IBM watsonx.ai 数据库连接资产中检索必要的信息。有关以编程方式创建数据库连接资产的更多详细信息，请参阅 watsonx.ai Python SDK 的 [文档](https://ibm.github.io/watsonx-ai-python-sdk/v1.4.7/autoai_working_with_dataconnection.html#connection-asset-databaselocation)。
...
另外，您也可以使用 Cloud Pak for Data 凭证。详情请参阅 [watsonx.ai 软件设置](https://ibm.github.io/watsonx-ai-python-sdk/setup_cpd.html)。

对于某些特定需求，可以选择将 IBM 的 [`APIClient`](https://ibm.github.io/watsonx-ai-python-sdk/base.html#apiclient) 对象传递给 `WatsonxSQLDatabase` 类。
...
通过运行 `.get_tools` 方法，可以获取一个包含所有可用工具的列表。
...
# 从 langchain hub 中拉取一个提示模板
from langchain import hub

prompt_template = hub.pull("langchain-ai/sql-agent-system-prompt")
# 格式化系统消息，指定方言和返回前 K 个结果
...
# 假设 agent 变量已定义，这里使用 agent 变量进行 streaming
...
有关 `WatsonxSQLDatabaseToolkit` 所有功能和配置的详细文档，请查阅 [API 参考](https://reference.langchain.com/python/integrations/langchain_ibm/WatsonxSQLDatabaseToolkit/)。

---

<Callout icon="pen-to-square" iconType="regular">
    [Edit the source of this page on GitHub.](https://github.com/langchain-ai/docs/edit/main/src/oss\python\integrations\tools\ibm_watsonx_sql.mdx)
</Callout>
