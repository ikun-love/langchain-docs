...
[**Scrapeless**](https://www.scrapeless.com/) 提供灵活且功能丰富的**数据采集服务**，支持广泛的参数自定义和多格式导出功能。这些能力使 LangChain 能够更有效地集成和利用外部数据。其核心功能模块包括：

**DeepSerp**

- **Google 搜索 (Google Search)**：支持跨所有搜索结果类型进行全面的 Google SERP（搜索引擎结果页面）数据提取。
  - 支持选择本地化的 Google 域名（例如 `google.com`、`google.ad`），以获取特定区域的搜索结果。
  - 支持分页功能，用于检索首页之外的结果。
  - 支持一个搜索结果过滤开关，用于控制是否排除重复或相似的内容。
- **Google 趋势 (Google Trends)**：从 Google 检索关键词趋势数据，包括随时间变化的流行度、区域兴趣和相关搜索。
  - 支持多关键词比较。
  - 支持多种数据类型：`interest_over_time`（随时间的变化趋势）、`interest_by_region`（按区域的兴趣）、`related_queries`（相关查询）和 `related_topics`（相关主题）。
  - 允许按特定的 Google 属性（网页、YouTube、新闻、购物）进行过滤，以便进行特定来源的趋势分析。

**通用抓取 (Universal Scraping)**

- 专为现代、大量使用 JavaScript 的网站设计，支持动态内容提取。
  - 全局优质代理（Premium Proxy）支持，用于绕过地理限制和提高可靠性。

**爬虫 (Crawler)**

- **爬取 (Crawl)**：递归地爬取一个网站及其链接页面，以提取全站内容。
  - 支持配置爬取深度（crawl depth）和限制 URL 范围。
- **抓取 (Scrape)**：从单个网页中高精度地提取内容。
  - 支持“仅主体内容 (main content only)”提取，以排除广告、页脚和其他非必要元素。
  - 支持对多个独立 URL 进行批量抓取。
...
| 类 (Class) | 包 (Package) | 可序列化 (Serializable) | JS 支持 | 版本 (Version) |
| :--- | :--- | :---: | :---: | :---: |
...
| 原生异步 (Native async) | 返回 Artifact | 返回数据格式 |
| :-: | :-: | :-: |
...
要使用此工具，您需要一个 Scrapeless API 密钥。您可以将其设置为环境变量：
...
`ScrapelessCrawlerScrapeTool` 允许您使用 Scrapeless 的 Crawler Scrape API 从一个或多个网站抓取内容。您可以控制提取主体内容、格式、请求头、等待时间以及输出类型。

该工具接受以下参数：

- `urls` (必需, List[str]): 要抓取的**一个或多个**网站的 URL。
- `formats` (可选, List[str]): 定义抓取输出的格式。默认为 `['markdown']`。选项包括：
  - `'markdown'`
  - `'rawHtml'`
  - `'screenshot@fullPage'`（全页面截图）
  - `'json'`
  - `'links'`
  - `'screenshot'`
  - `'html'`
- `only_main_content` (可选, bool): 是否只返回页面的主体内容，排除页眉、导航栏、页脚等。默认为 True。
- `include_tags` (可选, List[str]): 要在输出中包含的 HTML 标签列表（例如 `['h1', 'p']`）。如果设置为 None，则不明确包含任何标签。
- `exclude_tags` (可选, List[str]): 要从输出中排除的 HTML 标签列表。如果设置为 None，则不明确排除任何标签。
- `headers` (可选, Dict[str, str]): 发送请求时使用的自定义请求头（例如用于 Cookie 或 User-Agent）。默认为 None。
- `wait_for` (可选, int): 在抓取前等待的毫秒数。这有助于确保页面完全加载。默认为 `0`。
- `timeout` (可选, int): 请求的超时时间（毫秒）。默认为 `30000`。

### ScrapelessCrawlerCrawlTool

`ScrapelessCrawlerCrawlTool` 允许您使用 Scrapeless 的 Crawler Crawl API，从指定的**基础 URL** 开始爬取网站。它支持对 URL 进行高级过滤、控制爬取深度、内容抓取选项、自定义请求头等功能。

该工具接受以下参数：

- `url` (必需, str): 开始爬取的**基础 URL**。

- `limit` (可选, int): 要爬取的页面最大数量。默认为 `10000`。
- `include_paths` (可选, List[str]): 用于包含匹配 URL 的**路径名正则表达式**。只有匹配这些模式的 URL 才会包含在爬取范围内。例如，设置为 `["blog/.*"]` 将只包含 `/blog/` 路径下的 URL。默认为 None。
- `exclude_paths` (可选, List[str]): 用于排除匹配 URL 的**路径名正则表达式**。例如，设置为 `["blog/.*"]` 将排除 `/blog/` 路径下的 URL。默认为 None。
- `max_depth` (可选, int): 相对于基础 URL 的最大爬取深度，通过 URL 路径中的斜杠数量来衡量。默认为 `10`。
- `max_discovery_depth` (可选, int): 基于发现顺序的最大爬取深度。根 URL 和 sitemap 中的页面深度为 `0`。例如，设置为 `1` 并忽略 sitemap 将只爬取输入的 URL 及其直接链接。默认为 None。
- `ignore_sitemap` (可选, bool): 爬取过程中是否忽略网站的 sitemap 文件。默认为 False。
- `ignore_query_params` (可选, bool): 是否忽略查询参数的不同，以避免重复抓取相似的 URL。默认为 False。
- `deduplicate_similar_urls` (可选, bool): 是否对相似的 URL 进行去重处理。默认为 True。
- `regex_on_full_url` (可选, bool): 正则表达式匹配是应用于完整 URL 还是仅应用于路径部分。默认为 True。
- `allow_backward_links` (可选, bool): 是否允许爬取超出当前 URL 层次结构的**反向链接**。默认为 False。
- `allow_external_links` (可选, bool): 是否允许爬取链接到外部网站的链接。默认为 False。
- `delay` (可选, int): 页面抓取之间的延迟（秒），用于遵守速率限制。默认为 `1`。
- `formats` (可选, List[str]): 抓取内容的格式。默认为 `["markdown"]`。选项包括：
  ...
  - `'screenshot'`
  - `'html'`
- `only_main_content` (可选, bool): 是否只返回主体内容，排除页眉、导航栏、页脚等。默认为 True。
- `include_tags` (可选, List[str]): 要包含在输出中的 HTML 标签列表（例如 `['h1', 'p']`）。默认为 None（无明确包含过滤器）。
- `exclude_tags` (可选, List[str]): 要排除在输出中的 HTML 标签列表。默认为 None（无明确排除过滤器）。
- `headers` (可选, Dict[str, str]): 发送请求时使用的自定义 HTTP 请求头，例如 Cookie 或 User-Agent 字符串。默认为 None。
- `wait_for` (可选, int): 在抓取内容前等待的毫秒数，以允许页面完全加载。默认为 `0`。
...
```python
from langchain_scrapeless import ScrapelessDeepSerpGoogleTrendsTool # 注意：此处可能存在笔误，原文档中应为 ScrapelessCrawlerScrapeTool 或保持原样
# 为保持格式一致性，我们使用原名称
...
- [Scrapeless 文档 (Scrapeless Documentation)](https://docs.scrapeless.com/en/crawl/quickstart/introduction/)
- [Scrapeless API 参考 (Scrapeless API Reference)](https://apidocs.scrapeless.com/api-17509003)
