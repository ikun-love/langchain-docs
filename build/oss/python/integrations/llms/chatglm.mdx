...
[ChatGLM-6B](https://github.com/THUDM/ChatGLM-6B) 是一个基于通用语言模型（GLM）框架的开源双语（中英）语言模型，拥有 62 亿参数。通过量化技术，用户可以在消费级显卡上本地部署（在 INT4 量化级别下仅需要 6GB 的 GPU 显存）。

[ChatGLM2-6B](https://github.com/THUDM/ChatGLM2-6B) 是开源双语（中文-英文）聊天模型 ChatGLM-6B 的第二代版本。它保留了第一代模型的流畅对话和低部署门槛，同时引入了更好的性能、更长的上下文长度和更高效的推理等新特性。

[ChatGLM3](https://github.com/THUDM/ChatGLM3) 是智谱 AI 和清华 KEG 联合发布的新一代预训练对话模型系列。ChatGLM3-6B 是 ChatGLM3 系列中的开源模型。
...
此示例介绍如何使用 LangChain 与 ChatGLM3-6B 推理接口进行交互以实现文本补全（Text completion）。
...
下面的示例展示了如何使用 LangChain 与 ChatGLM2-6B 推理接口进行交互以完成文本补全。
ChatGLM-6B 和 ChatGLM2-6B 具有相同的 API 规范，因此此示例应该可以同时适用于两者。
...
# 仅当您希望 LLM 对象保留对话历史记录并将其作为累积的上下文发送给后端模型 API 时，才开启 with_history（这会使其变为有状态的）。默认是无状态的。
