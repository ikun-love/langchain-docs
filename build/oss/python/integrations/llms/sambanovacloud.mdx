...
<Warning>
**您当前正在查看记录 SambaNovaCloud 模型作为文本补全模型用法的页面。我们推荐您使用 [聊天补全模型](/oss/python/langchain/models)。**

您可能正在寻找 [SambaNova 聊天模型](/oss/python/integrations/chat/sambanova/)。
</Warning>

**[SambaNova](https://sambanova.ai/) 的 [SambaNovaCloud](https://cloud.sambanova.ai/)** 是一个用于使用开源模型执行推理的平台。
...
要访问 ChatSambaNovaCloud 模型，您需要创建一个 [SambaNovaCloud 账户](https://cloud.sambanova.ai/)，获取一个 API 密钥，并将其设置为 `SAMBANOVA_API_KEY` 环境变量：
...
该集成存在于 `langchain-community` 包中。我们还需要安装 `[sseclient-py](https://pypi.org/project/sseclient-py/)` 包，这是运行流式预测所必需的。
...
我们可以将完成模型与提示模板（Prompt Template）链接起来，如下所示：
...
如需了解所有 `SambaNovaCloud` LLM 特性和配置的详细文档，请参阅 API 参考：[https://python.langchain.com/api_reference/community/llms/langchain_community.llms.sambanova.SambaNovaCloud.html](https://python.langchain.com/api_reference/community/llms/langchain_community.llms.sambanova.SambaNovaCloud.html)
```
