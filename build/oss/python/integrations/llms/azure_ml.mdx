...
[Azure ML](https://azure.microsoft.com/en-us/products/machine-learning/) 是一个用于构建、训练和部署机器学习模型的平台。用户可以在模型目录（Model Catalog）中探索要部署的模型类型，该目录提供了来自不同提供商的基础模型和通用模型。

本 Notebook 将介绍如何使用托管在 `Azure ML Online Endpoint` 上的 LLM。
...
* `endpoint_url`: 端点提供的 REST 终结点 URL。
* `endpoint_api_type`:
    * 当将模型部署到 **专用终结点 (Dedicated endpoints)**（托管管理基础架构）时，使用 `endpoint_type='dedicated'`。
    * 当使用 **按使用付费 (Pay-as-you-go)** 方案（模型即服务）部署模型时，使用 `endpoint_type='serverless'`。
* `endpoint_api_key`: 端点提供的 API 密钥。
* `deployment_name`: (可选) 使用该端点的模型的部署名称。

## 内容格式化器 (Content Formatter)

`content_formatter` 参数是一个处理程序类，用于转换 AzureML 端点的请求和响应，以适应所需的 schema。由于模型目录中的模型范围很广，每个模型处理数据的方式可能不同，因此提供了一个 `ContentFormatterBase` 类，允许用户根据自己的需要转换数据。提供的 `ContentFormatter` 类包括：
...
*注意：`OSSContentFormatter` 正在被弃用，并被 `GPT2ContentFormatter` 取代。逻辑是相同的，但 `GPT2ContentFormatter` 是一个更合适的名称。由于更改是向后兼容的，您仍然可以使用 `OSSContentFormatter`。*

## 示例

### 示例：使用实时终结点的 LlaMa 2 补全
