...
>[Replicate](https://replicate.com/blog/machine-learning-needs-better-tools) 在云端运行机器学习模型。我们有一个开源模型库，您只需几行代码即可运行其中的模型。如果您正在构建自己的机器学习模型，Replicate 可以轻松地将它们部署到规模化环境中。

本示例将介绍如何使用 LangChain 与 `Replicate` [模型](https://replicate.com/explore) 进行交互。
...
在 [replicate explore 页面](https://replicate.com/explore) 查找模型，然后按照 `模型名称/版本` 的格式粘贴模型名称。

例如，以下是 [`Meta Llama 3`](https://replicate.com/meta/meta-llama-3-8b-instruct)。
...
再举一个例子，对于这个 [dolly 模型](https://replicate.com/replicate/dolly-v2-12b)，点击 API 选项卡。模型名称/版本将是：`replicate/dolly-v2-12b:ef0e1aefc61f8e096ebe4db6b2bacc297daf2ef6899f0f7e001ec445893500e5`

只有 `model` 参数是必需的，但在初始化时我们可以添加其他模型参数。

例如，如果我们运行 Stable Diffusion 并且想要更改图像尺寸：
...
您可以选择在生成响应时进行流式传输，这有助于向用户展示交互性，特别是对于耗时的生成过程。有关更多信息，请参阅 [流式传输](/oss/python/langchain/streaming) 的详细文档。
...
您还可以指定停止序列。如果您生成的序列有一个明确的停止序列，并且您打算用它来解析，那么最好（更便宜、更快！）在达到一个或多个停止序列时就取消生成，而不是让模型一直生成直到达到指定的 `max_length`。停止序列在流式传输模式下是否启用都有效，Replicate 只对生成到停止序列之前的内容收费。
...
LangChain 的全部意义就在于……链式调用 (chaining)！下面是如何做到这一点的示例。

```python
from langchain.chains import SimpleSequentialChain
```

首先，我们将此模型的 LLM 定义为 flan-5，将 text2image 定义为 stable diffusion 模型。
