...
[Google AI](https://ai.google.dev/) 提供了多种聊天模型，包括强大的 Gemini 系列。有关最新模型、特性、上下文窗口等方面的信息，请参阅 [Google AI 文档](https://ai.google.dev/gemini-api/docs/models/gemini)。

本指南将帮助您开始使用 `ChatGoogleGenerativeAI` [聊天模型](/oss/javascript/langchain/models)。有关 `ChatGoogleGenerativeAI` 所有特性和配置的详细文档，请查阅 [API 参考](https://api.js.langchain.com/classes/langchain_google_genai.ChatGoogleGenerativeAI.html)。

## 概述

### 集成详情

| 类名 | 包名 | 本地 | 可序列化 | [Python 支持](https://python.langchain.com/docs/integrations/chat/google_generative_ai) | 下载量 | 版本 |
| :--- | :--- | :---: | :---: | :---: | :---: | :---: |
...
请查看下方表格标题中的链接，以获取关于如何使用特定功能的指南。
...
您可以通过 `@langchain/google-genai` 集成包中的 `ChatGoogleGenerativeAI` 类来访问 LangChain 中的 Google `gemini` 和 `gemini-vision` 模型，以及其他生成式模型。

<Tip>
您还可以通过 LangChain 的 VertexAI 和 VertexAI-web 集成来访问 Google 的 `gemini` 系列模型。点击 [此处](/oss/javascript/integrations/chat/google_vertex_ai) 阅读文档。
</Tip>

### 凭证 (Credentials)

请在此处获取 API 密钥：[https://ai.google.dev/tutorials/setup](https://ai.google.dev/tutorials/setup)
...
如果您希望对模型调用进行自动追踪（tracing），您也可以通过取消注释以下内容来设置您的 [LangSmith](https://docs.smith.langchain.com/) API 密钥：
...
Gemini 模型具有默认的安全设置，可以被覆盖。如果您从模型收到大量“安全警告”（Safety Warnings），可以尝试调整模型的 `safety_settings` 属性。例如，要关闭对危险内容的安全性阻止，可以从 `@google/generative-ai` 包中导入枚举 (enums)，然后像下面这样构建您的 LLM：
...
Google AI 的工具调用与 [其他模型的工具调用](/oss/javascript/langchain/tools) 大多相同，但在 Schema 方面有一些限制。

Google AI API 不允许工具 Schema 包含具有未知属性的对象。例如，以下 Zod Schema 会抛出错误：
...
### 内置 Google 搜索检索 (Built in Google Search Retrieval)

Google 还提供了一个内置的搜索工具，可用于根据现实世界信息来确定内容生成的依据。以下是如何使用它的示例：
...
Google Generative AI 也支持代码执行。使用内置的 `CodeExecutionTool`，您可以让模型生成代码、执行代码，并将结果用于最终的补全中：
...
上下文缓存允许您首先向模型提供某些内容，缓存输入的 Token，然后对后续请求引用缓存的 Token，以降低成本。您可以使用 `GoogleAICacheManager` 类创建 `CachedContent` 对象，然后使用 `enableCachedContent()` 方法将 `CachedContent` 对象传递给 `ChatGoogleGenerativeAIModel`。
...
- 上下文缓存的最小输入 Token 数为 32,768，最大输入 Token 数与给定模型的最大 Token 数相同。

## Gemini 提示词常见问题 (Gemini prompting FAQs)

在本文撰写之时（2023/12/12），Gemini 对其接受的提示词的类型和结构有一些限制。具体来说：

1. 当提供多模态（图像）输入时，您被限制最多只能有一个 “human”（用户）类型的消息。您不能传递多条消息（尽管单条 human 消息可以包含多个内容条目）。
2. 系统消息 (System messages) 不受原生支持，如果存在，它们将被合并到第一条 human 消息中。
3. 对于常规的对话（chat）场景，消息必须遵循 human/ai/human/ai 交替模式。您不能连续提供两条 AI 或 human 消息。
4. 如果消息违反了 LLM 的安全检查，可能会被阻止。在这种情况下，模型将返回一个空响应。

---

## API 参考

有关 `ChatGoogleGenerativeAI` 所有特性和配置的详细文档，请查阅 [API 参考](https://api.js.langchain.com/classes/langchain_google_genai.ChatGoogleGenerativeAI.html)。
