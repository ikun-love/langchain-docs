...
本指南将帮助您开始使用 Cohere 的 [Chat 模型](/oss/javascript/langchain/models)。如需了解 `ChatCohere` 所有功能和配置的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_cohere.ChatCohere.html)。

## 概述

### 集成详情

| 类名 | 包名 | 本地 | 可序列化 | [Python 支持](https://python.langchain.com/docs/integrations/chat/cohere) | 下载量 | 版本 |
| :--- | :--- | :---: | :---: |  :---: | :---: | :---: |
...
为了使用 LangChain.js Cohere 集成，您需要一个 API 密钥。
您可以在 [此处](https://dashboard.cohere.com/welcome/register) 注册 Cohere 账户并创建 API 密钥。

您首先需要安装 [`@langchain/cohere`](https://www.npmjs.com/package/@langchain/cohere) 包。

### 凭证

请访问 [Cohere 网站](https://dashboard.cohere.com/welcome/register) 注册 Cohere 并生成 API 密钥。完成此操作后，请设置 `COHERE_API_KEY` 环境变量：

```bash
export COHERE_API_KEY="your-api-key"
```

如果您希望对模型调用进行自动追踪，也可以通过取消注释以下内容来设置您的 [LangSmith](https://docs.smith.langchain.com/) API 密钥：
...
API 还允许其他连接方式，这些方式不是静态文档。
例如，`web-search` 连接器允许您传入一个查询，API 将在网络上搜索相关文档。
示例如下：
...
我们可以从 `additional_kwargs` 对象中看到，API 请求执行了以下操作：

- 执行了一次搜索查询，将结果数据存储在 `searchQueries` 和 `searchResults` 字段中。在 `searchQueries` 字段中，我们看到他们为获得更好的结果而重新措辞了我们的查询。
- 从搜索查询生成了三个文档。
- 生成了一份引用列表。
- 根据上述操作和内容生成了最终响应。
...
如需了解 `ChatCohere` 所有功能和配置的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_cohere.ChatCohere.html)。
