...
在摄取 HTML 文档以供后续检索时，我们通常只对网页的实际内容感兴趣，而不是其语义结构。使用 `HtmlToTextTransformer` 从文档中剥离 HTML 标签可以得到内容更丰富的块（chunks），从而使检索效果更有效。
...
```
虽然 Transformer 本身不需要，但下面的用法示例需要 [`cheerio`](https://www.npmjs.com/package/cheerio) 进行网页抓取：
...
你可以向 Transformer 传递 `html-to-text` 包可以接受的任何[参数](https://www.npmjs.com/package/html-to-text)，以自定义其工作方式。
