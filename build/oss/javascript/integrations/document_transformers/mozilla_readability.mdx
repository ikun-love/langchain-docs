...
在摄取 HTML 文档以供后续检索时，我们通常只关注网页的实际内容，而非其语义结构。使用 `MozillaReadabilityTransformer` 从文档中移除 HTML 标签可以产生内容更丰富的块（chunks），从而使检索更有效。
...
```
下面的使用示例需要 [`cheerio`](https://www.npmjs.com/package/cheerio) 进行抓取，但它本身并不是 transformer 所必需的：

```bash npm
npm install cheerio
```
<Tip>
有关安装 LangChain 包的常规说明，请参阅[本章节](/oss/javascript/langchain/install)。
</Tip>
...
下面的示例抓取了一个 Hacker News 帖子，根据 HTML 标签对其进行分割，以便根据标签中的语义信息对块进行分组，然后从各个块中提取内容：
...
您可以向 transformer 传递任何[`@mozilla/readability` 包接受的参数](https://www.npmjs.com/package/@mozilla/readability)，以自定义其工作方式。
