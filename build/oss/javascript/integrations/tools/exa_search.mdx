...
Exa（原名 Metaphor Search）是一个完全为大语言模型（LLMs）设计的搜索引擎。它允许您使用自然语言查询来搜索互联网上的文档，然后从目标文档中检索清洗过的 HTML 内容。

与基于关键词的搜索（如 Google）不同，Exa 的神经搜索能力使其能够**语义理解**查询并返回相关的文档。例如，我们可以搜索 `"fascinating article about cats"`，然后对比 Google 和 Exa 的搜索结果。Google 会返回基于关键词“fascinating”的 SEO 优化列表文章。而 Exa 则能直接工作。

本页面将介绍如何在 LangChain 中使用 `ExaSearchResults`。

## 概述

### 集成详情

| 类 (Class) | 包 (Package) | 可序列化 (Serializable) | [Python 支持](https://python.langchain.com/docs/integrations/tools/exa_search/) | 版本 (Version) |
| :--- | :--- | :---: | :---: | :---: |
...
首先，获取一个 Exa API 密钥，并将其添加为环境变量。注册 [此处](https://dashboard.exa.ai/login) 可获得每月 1000 次免费搜索。

```typescript
process.env.EXASEARCH_API_KEY="your-api-key"
```

设置 LangSmith 以获得一流的观测能力也是有帮助的（但不是必需的）：
...
我们可以通过将工具绑定到[工具调用模型](/oss/javascript/langchain/tools)并随后调用它，在链中（chain）使用我们的工具：
...
我们可以创建使用 `ExaRetriever` 的 LangChain 工具，并通过 `createRetrieverTool` 来使用它们。使用这些工具，我们可以构建一个简单的搜索代理 (agent)，该代理可以回答关于任何主题的问题。

我们将使用 LangGraph 来创建代理。请确保已安装 `@langchain/langgraph`：
...
有关所有 `ExaSearchResults` 功能和配置的详细文档，请访问 [API 参考](https://api.js.langchain.com/classes/langchain_exa.ExaSearchResults.html)。

---

<Callout icon="pen-to-square" iconType="regular">
    [Edit the source of this page on GitHub.](https://github.com/langchain-ai/docs/edit/main/src/oss\javascript\integrations\tools\exa_search.mdx)
</Callout>
