...
这将帮助您使用 LangChain 入门 TogetherAIEmbeddings [嵌入模型](/oss/javascript/integrations/text_embedding)。有关 `TogetherAIEmbeddings` 特性和配置选项的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_community_embeddings_togetherai.TogetherAIEmbeddings.html)。

## 概述

### 集成详情

| 类 (Class) | 包 (Package) | 本地 (Local) | [Python 支持](https://python.langchain.com/docs/integrations/text_embedding/together/) | 下载量 (Downloads) | 版本 (Version) |
| :--- | :--- | :---: | :---: | :---: | :---: |
...
嵌入模型通常用于检索增强生成 (RAG) 流程中，既可用于数据索引，也可用于后续的检索。有关更详细的指导，请参阅 [**学习 (Learn)** 选项卡](/oss/javascript/learn/) 下的 RAG 教程。

下面展示了如何使用我们上面初始化的 `embeddings` 对象来索引和检索数据。在此示例中，我们将使用演示用的 [`MemoryVectorStore`](/oss/javascript/integrations/vectorstores/memory) 对一个示例文档进行索引并进行检索。
...
您可以使用 `embedDocuments` 来嵌入多个文本用于索引。此方法的内部实现可能（但不一定）与查询嵌入的方式不同：
...
有关 TogetherAIEmbeddings 所有特性和配置的详细文档，请前往 [API 参考](https://api.js.langchain.com/classes/langchain_community_embeddings_togetherai.TogetherAIEmbeddings.html)。
