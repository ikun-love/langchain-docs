...
这将帮助您开始使用 LangChain 中的 OpenAIEmbeddings [嵌入模型](/oss/javascript/integrations/text_embedding)。有关 `OpenAIEmbeddings` 功能和配置选项的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_openai.OpenAIEmbeddings.html)。

## 概述

### 集成详情

| 类 (Class) | 包 (Package) | 本地 (Local) | [Python 支持](https://python.langchain.com/docs/integrations/text_embedding/openai/) | 下载量 (Downloads) | 版本 (Version) |
| :--- | :--- | :---: | :---: |  :---: | :---: |
...
如果您想对模型调用进行自动化的跟踪（tracing），也可以通过取消下面的注释来设置您的 [LangSmith](https://docs.smith.langchain.com/) API 密钥：
...
const embeddings = new OpenAIEmbeddings({
  ...
  batchSize: 512, // 默认值是 512。最大值为 2048
  model: "text-embedding-3-large",
});
```

如果您是某个组织的一部分，可以将 `process.env.OPENAI_ORGANIZATION` 设置为您的 OpenAI 组织 ID，或者在初始化模型时将其作为 `organization` 参数传入。

## 索引和检索 (Indexing and Retrieval)

嵌入模型通常用于检索增强生成（RAG）流程中，无论是在索引数据阶段还是在后续的检索阶段。有关更详细的说明，请参阅 [**学习 (Learn)** 选项卡下的 RAG 教程](/oss/javascript/learn/)。

下面展示了如何使用我们上面初始化的 `embeddings` 对象来索引和检索数据。在此示例中，我们将使用演示用的 [`MemoryVectorStore`](/oss/javascript/integrations/vectorstores/memory) 来索引和检索一个示例文档。
...
您可以使用 `embedQuery` 嵌入用于搜索的查询。这会生成一个特定于查询的向量表示：
...
您可以使用 `embedDocuments` 嵌入多个文本用于索引。此方法内部使用的机制可能（但不一定）与嵌入查询的方式有所不同：
...
## 指定维度 (Specifying dimensions)

对于 `text-embedding-3` 系列模型，您可以指定返回嵌入向量的维度大小。例如，默认情况下 `text-embedding-3-large` 返回 3072 维的嵌入向量：
...
但是，通过传入 `dimensions: 1024`，我们可以将嵌入向量的大小减少到 1024：
...
您可以通过传入 `configuration` 参数来自定义 SDK 发送请求的基础 URL，如下所示：
...
您还可以传入官方 SDK 所接受的其他 `ClientOptions` 参数。

如果您正在 Azure OpenAI 上托管服务，请参阅[专用页面](/oss/javascript/integrations/text_embedding/azure_openai)。

---

## API 参考

有关 OpenAIEmbeddings 所有功能和配置的详细文档，请访问 [API 参考](https://api.js.langchain.com/classes/langchain_openai.OpenAIEmbeddings.html)。
