...
这将帮助您使用 LangChain 开始使用 Google 生成式 AI [嵌入模型](/oss/javascript/integrations/text_embedding)。有关 `GoogleGenerativeAIEmbeddings` 功能和配置选项的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_google_genai.GoogleGenerativeAIEmbeddings.html)。

## 概述

### 集成详情

| 类 (Class) | 包 (Package) | 本地 (Local) | [Py 支持](https://python.langchain.com/docs/integrations/text_embedding/google_generative_ai/) | 下载量 (Downloads) | 版本 (Version) |
| :--- | :--- | :---: | :---: | :---: | :---: |
...
要访问 Google 生成式 AI 嵌入模型，您需要注册一个 Google AI 帐户、获取一个 API 密钥，并安装 `@langchain/google-genai` 集成包。
...
如果您希望获得模型调用的自动追踪（tracing），您还可以通过取消注释以下行来设置您的 [LangSmith](https://docs.smith.langchain.com/) API 密钥：
...
LangChain 的 `GoogleGenerativeAIEmbeddings` 集成位于 `@langchain/google-genai` 包中。您可能还希望安装官方 SDK：
...
您可以使用 `embedQuery` 来嵌入查询，以用于搜索。这会生成一个特定于查询的向量表示：
...
有关所有 `GoogleGenerativeAIEmbeddings` 功能和配置的详细文档，请访问 [API 参考](https://api.js.langchain.com/classes/langchain_google_genai.GoogleGenerativeAIEmbeddings.html)。
