...
这将帮助您开始使用 LangChain 中的 Ollama [嵌入模型](/oss/javascript/integrations/text_embedding)。有关 `OllamaEmbeddings` 功能和配置选项的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_ollama.OllamaEmbeddings.html)。

## 概述

### 集成详情

| 类名 | 包名 | 本地支持 | [Py 支持](https://python.langchain.com/docs/integrations/text_embedding/ollama/) | 下载量 | 版本 |
| :--- | :--- | :---: | :---: |  :---: | :---: |
...
如果您希望对模型调用进行自动追踪，您还可以通过取消注释以下代码来设置您的 [LangSmith](https://docs.smith.langchain.com/) API 密钥：
...
嵌入模型通常用于检索增强生成 (RAG) 流程中，既可以作为数据索引的一部分，也可以在后续检索中使用。有关更详细的说明，请参阅 [**学习** 选项卡](/oss/javascript/learn/) 下的 RAG 教程。
...
您可以使用 `embedDocuments` 来嵌入多个文本用于索引。此方法内部使用的机制可能（但不一定）与嵌入查询的方式不同：
...
Ollama 的[模型参数](https://github.com/jmorganca/ollama/blob/main/docs/modelfile.md#valid-parameters-and-values)也受支持：
...
const embeddingsCustomParams = new OllamaEmbeddings({
  requestOptions: {
    useMmap: true, // 对应 use_mmap 1
    numThread: 6, // 对应 num_thread 6
    numGpu: 1, // 对应 num_gpu 1
  },
});
```
...
有关 `OllamaEmbeddings` 所有功能和配置的详细文档，请访问 [API 参考](https://api.js.langchain.com/classes/langchain_ollama.OllamaEmbeddings.html)
```

---

<Callout icon="pen-to-square" iconType="regular">
    [Edit the source of this page on GitHub.](https://github.com/langchain-ai/docs/edit/main/src/oss\javascript\integrations\text_embedding\ollama.mdx)
</Callout>
