...
这将帮助您开始使用 LangChain 中的 CohereEmbeddings [嵌入模型](/oss/javascript/integrations/text_embedding)。有关 `CohereEmbeddings` 功能和配置选项的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_cohere.CohereEmbeddings.html)。

## 概览

### 集成详情

| 类 | 包 | 本地 | [Py 支持](https://python.langchain.com/docs/integrations/text_embedding/cohere/) | 下载量 | 版本 |
| :--- | :--- | :---: | :---: |  :---: | :---: |
...
请访问 [cohere.com](https://cohere.com) 注册 `Cohere` 账户并生成 API 密钥。完成此操作后，设置 `COHERE_API_KEY` 环境变量：
...
const embeddings = new CohereEmbeddings({
  apiKey: "YOUR-API-KEY", // 在 Node.js 中，默认使用 process.env.COHERE_API_KEY
  batchSize: 48, // 如果省略，默认值为 48。最大值为 96
  model: "embed-english-v3.0",
});
```

### Azure 上的 Cohere、AWS Bedrock 上的 Cohere 和独立 Cohere 实例的自定义客户端

我们可以实例化一个自定义的 `CohereClient` 并将其传递给 `ChatCohere` 构造函数。

**注意：** 如果提供了自定义客户端，构造函数中的 `COHERE_API_KEY` 环境变量和 `apiKey` 参数都将被忽略。
...
下面展示了如何使用我们上面初始化的 `embeddings` 对象来索引和检索数据。在这个例子中，我们将使用演示的 [`MemoryVectorStore`](/oss/javascript/integrations/vectorstores/memory) 来索引和检索一个示例文档。
...
在底层，向量存储和检索器实现是通过调用 `embeddings.embedDocument(...)` 和 `embeddings.embedQuery(...)` 来创建用于 `fromDocuments` 和检索器 `invoke` 操作的文本的嵌入向量的。

您可以直接调用这些方法来为您自己的用例获取嵌入向量。

### 嵌入单个文本

您可以使用 `embedQuery` 来嵌入用于搜索的查询。这将生成一个针对查询的特定向量表示：
...
您可以使用 `embedDocuments` 嵌入多个文本用于索引。此方法内部用于嵌入查询的方法（但不限于此）可能与 `embedQuery` 不同：
