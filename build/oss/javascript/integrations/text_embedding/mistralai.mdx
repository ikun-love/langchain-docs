...
这将帮助您开始使用 LangChain 中的 MistralAIEmbeddings [嵌入模型](/oss/javascript/integrations/text_embedding)。有关 `MistralAIEmbeddings` 功能和配置选项的详细文档，请参阅[^1]。

## 概述

### 集成详情

| 类 | 包 | 本地 | [Python 支持](https://python.langchain.com/docs/integrations/text_embedding/mistralai/) | 下载量 | 版本 |
| :--- | :--- | :---: | :---: |  :---: | :---: |
...
要访问 MistralAI 嵌入模型，您需要创建一个 MistralAI 账户，获取一个 API 密钥，并安装 `@langchain/mistralai` 集成包。

### 凭证 (Credentials)

前往 [console.mistral.ai](https://console.mistral.ai/) 注册 `MistralAI` 账户并生成一个 API 密钥。完成操作后，设置 `MISTRAL_API_KEY` 环境变量：

```bash
export MISTRAL_API_KEY="your-api-key"
```

如果您想对模型调用进行自动化的跟踪（tracing），您还可以设置您的 [LangSmith](https://docs.smith.langchain.com/) API 密钥（取消下面的注释代码）：
...
现在我们可以实例化模型对象并生成聊天填充（chat completions）：

```typescript
import { MistralAIEmbeddings } from "@langchain/mistralai";

const embeddings = new MistralAIEmbeddings({
  model: "mistral-embed", // 默认值
});
```

## 索引和检索 (Indexing and Retrieval)

嵌入模型通常用于检索增强生成（RAG）流程中，既用于数据索引，也用于后续的检索。有关更详细的说明，请参阅 [**学习** 选项卡](/oss/javascript/learn/) 下的 RAG 教程。

下面展示了如何使用我们上面初始化的 `embeddings` 对象来索引和检索数据。在此示例中，我们将使用演示用的 [`MemoryVectorStore`](/oss/javascript/integrations/vectorstores/memory) 对示例文档进行索引和检索。
...
在底层，向量存储和检索器的实现分别调用 `embeddings.embedDocument(...)` 和 `embeddings.embedQuery(...)` 来为 `fromDocuments` 中使用的文本和检索器的 `invoke` 操作生成的文本创建嵌入向量。

您可以直接调用这些方法来为自己的用例获取嵌入向量。

### 嵌入单个文本

您可以使用 `embedQuery` 来嵌入查询（queries）以进行搜索。这会生成一个特定于查询的向量表示：
...
您可以使用 `embedDocuments` 嵌入多个文本用于索引。此方法的内部机制可能（但不一定）与嵌入查询的方式不同：
...
Mistral AI 支持针对三个事件的自定义钩子：`beforeRequest`（请求前）、`requestError`（请求错误）和 `response`（响应）。每个钩子类型的函数签名示例可以在下面看到：

```typescript
const beforeRequestHook = (req: Request): Request | void | Promise<Request | void> => {
    // 在 Mistral 处理请求之前要运行的代码
};

const requestErrorHook = (err: unknown, req: Request): void | Promise<void> => {
    // 在 Mistral 处理请求过程中发生错误时要运行的代码
};

const responseHook = (res: Response, req: Request): void | Promise<void> => {
    // 在 Mistral 发送成功响应之前要运行的代码
};
```

要将这些钩子添加到聊天模型中，您可以将它们作为参数传递，它们会自动添加：
...
`addAllHooksToHttpClient` 方法会在分配整个更新后的钩子列表之前，清除所有当前已添加的钩子，以避免钩子重复。
...
有关 `MistralAIEmbeddings` 所有功能和配置的详细文档，请前往 [API 参考](https://api.js.langchain.com/classes/langchain_mistralai.MistralAIEmbeddings.html)。

[^1]: 请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_mistralai.MistralAIEmbeddings.html)，了解有关 `MistralAIEmbeddings` 功能和配置选项的详细文档。
```
