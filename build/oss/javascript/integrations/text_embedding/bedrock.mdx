...
[Amazon Bedrock](https://aws.amazon.com/bedrock/) 是一项全托管服务，它通过统一的 API 提供了来自领先 AI 公司（如 AI21 Labs、Anthropic、Cohere、Meta、Stability AI 和 Amazon）的高性能基础模型（FMs）供您选择，并提供构建生成式 AI 应用所需的广泛功能，同时确保安全、隐私和负责任的 AI。

本文将指导您使用 LangChain 开始使用 Amazon Bedrock [嵌入模型](/oss/javascript/integrations/text_embedding)。有关 `Bedrock` 功能和配置选项的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_aws.BedrockEmbeddings.html)。

## 概述

### 集成详情

| 类 | 包 | 本地 | [Py 支持](https://python.langchain.com/docs/integrations/text_embedding/bedrock/) | 下载量 | 版本 |
| :--- | :--- | :---: | :---: |  :---: | :---: |
...
要使用 Bedrock 嵌入模型，您需要创建一个 AWS 账户，获取 API 密钥，并安装 `@langchain/aws` 集成包。

请访问 [AWS 文档](https://docs.aws.amazon.com/bedrock/latest/userguide/getting-started.html) 注册 AWS 并设置您的凭证。您还需要为您的账户启用模型访问权限，可以按照 [这些说明](https://docs.aws.amazon.com/bedrock/latest/userguide/model-access.html) 进行操作。

### 凭证

如果您希望对模型调用进行自动跟踪，还可以通过取消注释以下内容来设置您的 [LangSmith](https://docs.smith.langchain.com/) API 密钥：
...
现在我们可以实例化我们的模型对象并嵌入文本了。

有几种不同的方式可以向 AWS 进行身份验证——以下示例依赖于在您的环境变量中设置的访问密钥 (`access key`)、密钥访问密钥 (`secret access key`) 和区域 (`region`)：
...
嵌入模型通常用于检索增强生成（RAG）流程中，既作为索引数据的一部分，也用于后续的检索。有关更详细的说明，请参阅 [**学习** 选项卡](/oss/javascript/learn/) 下的 RAG 教程。
...
在底层，向量存储和检索器实现分别调用 `embeddings.embedDocument(...)` 和 `embeddings.embedQuery(...)` 来为 `fromDocuments` 中使用的文本和检索器的 `invoke` 操作创建嵌入向量。

您可以直接调用这些方法来获取嵌入向量，以用于您自己的用例。

### 嵌入单个文本

您可以使用 `embedQuery` 来嵌入用于搜索的查询。这会生成一个特定于该查询的向量表示：
...
您可以使用 `embedDocuments` 来嵌入多个文本，以便进行索引。此方法内部使用的逻辑可能（但不一定）与嵌入查询的方式不同：
...
## 配置 Bedrock Runtime 客户端

如果您想自定义选项，例如 `credentials`、`region`、`retryPolicy` 等，您可以传入自己实例化的 `BedrockRuntimeClient`。
