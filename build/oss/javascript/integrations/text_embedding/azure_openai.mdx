...
这将帮助您使用 LangChain 开始使用 AzureOpenAIEmbeddings [嵌入模型](/oss/javascript/integrations/text_embedding)。有关 `AzureOpenAIEmbeddings` 功能和配置选项的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_openai.AzureOpenAIEmbeddings.html)。
...
嵌入模型通常用于检索增强生成 (RAG) 流程中，既可用于数据索引，也可用于后续检索。有关更详细的说明，请参阅 [**学习** 选项卡](/oss/javascript/learn/) 下的 RAG 教程。

下面展示了如何使用上面初始化的 `embeddings` 对象来索引和检索数据。在此示例中，我们将使用演示的 [`MemoryVectorStore`](/oss/javascript/integrations/vectorstores/memory) 来索引和检索一个示例文档。
