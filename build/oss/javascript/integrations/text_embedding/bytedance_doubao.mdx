...
这将帮助您开始使用 LangChain 中的字节跳动豆包 (ByteDanceDoubao) [嵌入模型](/oss/javascript/integrations/text_embedding)。有关 `ByteDanceDoubaoEmbeddings` 功能和配置选项的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/_langchain_community.embeddings_bytedance_doubao.ByteDanceDoubaoEmbeddings.html)。
...
您需要注册一个 [ARK API 密钥](https://console.volcengine.com/ark/region:ark+cn-beijing/apiKey)，并将其设置为名为 `ARK_API_KEY` 的环境变量。然后，您应该为嵌入模型 [创建一个入口点](https://console.volcengine.com/ark/region:ark+cn-beijing/endpoint)，并使用该入口点的名称作为 `model` 的值。

接下来，您需要安装 [`@langchain/community`](https://www.npmjs.com/package/@langchain/community) 包。

### 凭证

如果您希望获得模型调用的自动化追踪，还可以取消注释以下行来设置您的 [LangSmith](https://docs.smith.langchain.com/) API 密钥：
...
LangChain 的字节跳动豆包嵌入模型集成位于 `@langchain/community` 包中：
...
嵌入模型通常用于检索增强生成 (RAG) 流程中，既可以作为数据索引的一部分，也可以在后续检索中使用。有关更详细的说明，请参阅 [**学习** 标签页](/oss/javascript/learn/) 下的 RAG 教程。

下面展示了如何使用我们上面初始化的 `embeddings` 对象来进行数据索引和检索。在此示例中，我们将使用演示用的 [`MemoryVectorStore`](/oss/javascript/integrations/vectorstores/memory) 来索引和检索一个示例文档。
...
您可以使用 `embedDocuments` 来嵌入多个文本用于索引。此方法内部使用的机制可能（但不一定）与嵌入查询的方法不同：
...
有关所有 ByteDanceDoubaoEmbeddings 功能和配置的详细文档，请访问 [API 参考](https://api.js.langchain.com/classes/_langchain_community.embeddings_bytedance_doubao.ByteDanceDoubaoEmbeddings.html)。
```
