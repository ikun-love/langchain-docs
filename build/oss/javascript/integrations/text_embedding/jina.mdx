...
`JinaEmbeddings` 类利用 Jina API 为给定文本输入生成嵌入（embeddings）。本指南将指导您完成 `JinaEmbeddings` 类的设置和使用，帮助您无缝地将其集成到项目中。
...
首先，您需要在 Jina 网站上注册并从[此处](https://jina.ai/embeddings)获取 API 令牌。您可以从 API 游乐场（api playground）的下拉菜单中复制模型名称。

要使用 `JinaEmbeddings` 类，您需要一个来自 Jina 的 API 令牌。您可以直接将此令牌传递给构造函数，或者将其设置为环境变量（`JINA_API_KEY`）。
...
如果没有提供 API 令牌且无法在环境变量中找到它，系统将抛出一个错误：
...
如有反馈或疑问，请联系 [support@jina.ai](mailto:support@jina.ai)。
...
<Callout icon="pen-to-square" iconType="regular">
    [Edit the source of this page on GitHub.](https://github.com/langchain-ai/docs/edit/main/src/oss\javascript\integrations\text_embedding\jina.mdx)
</Callout>
