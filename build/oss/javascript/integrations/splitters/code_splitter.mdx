...
`[RecursiveCharacterTextSplitter](https://reference.langchain.com/javascript/classes/_langchain_textsplitters.RecursiveCharacterTextSplitter.html)` 包含预构建的分隔符列表，这些列表在根据特定编程语言**分割文本**（[查看拆分器](/oss/javascript/integrations/splitters/)）时非常有用。
...
要实例化一个针对特定语言定制的分割器，请将此枚举中的值传递给：
```ts
