...
本 Notebook 提供了关于如何开始使用 `PDFLoader` 文档加载器的快速指南，该加载器属于 [文档加载器](/oss/javascript/integrations/document_loaders) 系列。如需了解 `PDFLoader` 所有功能和配置的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_community_document_loaders_fs_pdf.PDFLoader.html)。

## 概述

### 集成详情

| 类 (Class) | 包 (Package) | 兼容性 | 本地 (Local) | PY 支持 |
| :--- | :--- | :---: | :---: |  :---: |
| [PDFLoader](https://api.js.langchain.com/classes/langchain_community_document_loaders_fs_pdf.PDFLoader.html) | [@langchain/community](https://api.js.langchain.com/modules/langchain_community_document_loaders_fs_pdf.html) | 仅限 Node | ✅ | 🟠 (见下注) |

## 设置

要使用 `PDFLoader` 文档加载器，您需要安装 `@langchain/community` 集成包，以及 `pdf-parse` 包。
...
默认情况下，我们使用 `pdf-parse` 中捆绑的 `pdfjs` 构建，它与包括 Node.js 和现代浏览器在内的大多数环境兼容。如果您想使用更新版本的 `pdfjs-dist`，或者想使用自定义的 `pdfjs-dist` 构建，可以通过提供一个自定义的 `pdfjs` 函数来实现，该函数返回一个解析到 `PDFJS` 对象的 Promise。

在以下示例中，我们使用了 `pdfjs-dist` 的“旧版”（参见 [pdfjs 文档](https://github.com/mozilla/pdf.js/wiki/Frequently-Asked-Questions#which-browsersenvironments-are-supported)）构建，其中包含默认构建中不包含的多个 Polyfill（垫片）。
...
PDF 文件种类繁多，这使得阅读成为一项挑战。该加载器默认解析单个文本元素并将它们用一个空格连接起来。但是，如果您看到的空格过多，这可能不是您期望的行为。在这种情况下，您可以将分隔符覆盖为空字符串，如下所示：
...
/* 附加步骤：使用任何 TextSplitter 将文本分割成块。然后您可以将其用作上下文或保存到内存中。 */
...
如需了解 `PDFLoader` 所有功能和配置的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_community_document_loaders_fs_pdf.PDFLoader.html)。
```
