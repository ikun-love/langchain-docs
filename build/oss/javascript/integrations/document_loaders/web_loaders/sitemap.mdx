...
本笔记将介绍如何使用 [`SitemapLoader`](https://api.js.langchain.com/classes/_langchain_community.document_loaders.web_sitemap.SitemapLoader.html) 类将站点地图（sitemaps）加载为 `Document` 对象。
...
传入的 URL 必须包含 `.xml` 路径指向站点地图，否则会自动在 URL 后追加默认的 `/sitemap.xml`。
...
或者，如果你只想加载站点地图本身，而不加载站点地图中包含的每个页面的内容，你可以使用 `parseSitemap` 方法：
