...
本 Notebook 提供了关于如何开始使用 [WebPDFLoader](/oss/javascript/integrations/document_loaders/) 的快速概述。如需了解 WebPDFLoader 所有功能和配置的详细文档，请查阅 [API 参考](https://api.js.langchain.com/classes/langchain_community_document_loaders_web_pdf.WebPDFLoader.html)。

## 概述

### 集成详情

| 类名 | 包名 | 本地支持 | 可序列化 | Python 支持 |
| :--- | :--- | :---: | :---: |  :---: |
...
要访问 `WebPDFLoader` 文档加载器，您需要安装 `@langchain/community` 集成，以及 `pdf-parse` 包：

### 凭证

如果您想对模型的调用进行自动化跟踪，可以通过取消注释以下内容来设置您的 [LangSmith](https://docs.smith.langchain.com/) API 密钥：
...
默认情况下，我们使用 `pdf-parse` 捆绑的 `pdfjs` 构建，它与大多数环境（包括 Node.js 和现代浏览器）兼容。如果您想使用更新版本的 `pdfjs-dist`，或者想使用自定义的 `pdfjs-dist` 构建，您可以通过提供一个返回解析为 `PDFJS` 对象的 Promise 的自定义 `pdfjs` 函数来实现。

在下面的示例中，我们使用了 `pdfjs-dist` 的“旧版”（参见 [pdfjs 文档](https://github.com/mozilla/pdf.js/wiki/Frequently-Asked-Questions#which-browsersenvironments-are-supported)）构建，它包含了默认构建中不包含的某些垫片 (polyfills)。
...
PDF 文件种类繁多，这使得读取它们具有挑战性。加载器解析单个文本元素，默认情况下用一个空格将它们连接起来，但是
如果您看到多余的空格，这可能不是您期望的行为。在这种情况下，您可以将分隔符覆盖为空字符串，如下所示：
...
如需了解 WebPDFLoader 所有功能和配置的详细文档，请查阅 [API 参考](https://api.js.langchain.com/classes/langchain_community_document_loaders_web_pdf.WebPDFLoader.html)。
```
