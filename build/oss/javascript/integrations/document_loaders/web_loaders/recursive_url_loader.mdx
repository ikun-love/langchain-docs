---
title: 递归URL加载器 (RecursiveUrlLoader)
---
...
本笔记快速概述如何开始使用 `RecursiveUrlLoader`（[递归URL加载器](/oss/javascript/integrations/document_loaders/)）。有关 `RecursiveUrlLoader` 所有功能和配置的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_community_document_loaders_web_recursive_url.RecursiveUrlLoader.html)。

## 概述

### 集成详情

| 类 | 包 (Package) | 本地 (Local) | 可序列化 (Serializable) | PY 支持 (PY support) |
| :--- | :--- | :---: | :---: |  :---: |
...
| 来源 (Source) | Web 加载器 (Web Loader) | 仅限 Node.js 环境 (Node Envs Only) |
| :---: | :---: | :---: |
| RecursiveUrlLoader | ✅ | ✅ |

当从网站加载内容时，我们可能希望处理并加载页面上的所有 URL。

例如，我们来看一下 LangChain.js 的介绍文档。

这个介绍文档包含许多有趣的子页面，我们可能希望将它们全部加载、分割，并稍后批量检索。

这里的挑战在于如何遍历子页面的树状结构并收集所有内容！

我们使用 `RecursiveUrlLoader` 来完成这个任务。

这还为我们提供了排除某些子链接、自定义内容提取器等的灵活性。

## 设置

要使用 `RecursiveUrlLoader` 文档加载器，您需要安装 `@langchain/community` 集成包，以及 [`jsdom`](https://www.npmjs.com/package/jsdom) 包。

### 凭证 (Credentials)

如果您希望对您的模型调用进行自动化跟踪，您可以通过取消注释以下内容来设置您的 [LangSmith](/langsmith/home) API 密钥：
...
LangChain 的 `RecursiveUrlLoader` 集成位于 `@langchain/community` 包中：
...
我们还建议添加像 [`html-to-text`](https://www.npmjs.com/package/html-to-text) 或 [`@mozilla/readability`](https://www.npmjs.com/package/@mozilla/readability) 这样的包，用于从页面中提取纯文本内容。
...
```output
{
  ...
  metadata: {
    ...
    title: 'LangChain',
    description: 'LangChain 的产品套件支持开发人员在其开发旅程的每一步。',
    language: 'en'
  }
}
```
...
interface Options {
  ...
  extractor?: (text: string) => string; // 从网页中提取文档文本的函数，默认情况下它会返回页面本身。推荐使用 html-to-text 等工具来提取文本。默认情况下，它只返回页面本身。
  maxDepth?: number; // 爬取的最大深度。默认为 2。如果需要爬取整个网站，设置一个足够大的数字即可。
  timeout?: number; // 每次请求的超时时间（单位：秒）。默认为 10000 (10 秒)。
  preventOutside?: boolean; // 是否阻止爬取根 URL 之外的内容。默认为 true。
  callerOptions?: AsyncCallerConstructorParams; // 调用 AsyncCaller 的选项，例如设置最大并发数 (默认为 64)
}
```

然而，由于很难进行完美的过滤，您在结果中可能仍然会看到一些不相关的结果。如果需要，您可以对返回的文档自行进行过滤。在大多数情况下，返回的结果已经足够好。

---

## API 参考

有关 `RecursiveUrlLoader` 所有功能和配置的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_community_document_loaders_web_recursive_url.RecursiveUrlLoader.html)。
