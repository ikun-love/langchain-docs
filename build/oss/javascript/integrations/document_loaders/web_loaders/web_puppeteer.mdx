...
本指南提供了关于如何开始使用 `[PuppeteerWebBaseLoader](/oss/javascript/integrations/document_loaders/)` 的快速概览。有关 `PuppeteerWebBaseLoader` 所有功能和配置的详细文档，请查阅 [API 参考](https://api.js.langchain.com/classes/langchain_community_document_loaders_web_puppeteer.PuppeteerWebBaseLoader.html)。

Puppeteer 是一个 Node.js 库，它提供了一个高级 API 用于控制无头 Chrome 或 Chromium 浏览器。您可以使用 Puppeteer 自动化网页交互，包括从需要 JavaScript 渲染的动态网页中提取数据。

如果您需要一个更轻量级的解决方案，并且您想加载的网页不需要 JavaScript 渲染，那么可以使用 `[CheerioWebBaseLoader](/oss/javascript/integrations/document_loaders/web_loaders/web_cheerio)` 代替。
...
要访问 `PuppeteerWebBaseLoader` 文档加载器，您需要安装 `@langchain/community` 集成包，以及 `puppeteer` 对等依赖项。

### 凭证

如果您想对模型调用进行自动化的追踪，可以通过取消注释以下内容来设置您的 [LangSmith](https://docs.smith.langchain.com/) API 密钥：
...
以下是关于您可以通过 `PuppeteerWebBaseLoaderOptions` 接口传递给 `PuppeteerWebBaseLoader` 构造函数的参数的解释：
...
1. `launchOptions`：一个可选对象，用于指定要传递给 `puppeteer.launch()` 方法的附加选项。这可以包括诸如 `headless` 标志（用于以无头模式启动浏览器）或 `slowMo` 选项（用于减慢 Puppeteer 的操作以便更容易跟踪）。

2. `gotoOptions`：一个可选对象，用于指定要传递给 `page.goto()` 方法的附加选项。这可以包括 `timeout` 选项（用于以毫秒为单位指定最大导航时间）或 `waitUntil` 选项（用于指定何时认为导航成功）。

3. `evaluate`：一个可选函数，可用于使用 `page.evaluate()` 方法在页面上执行 JavaScript 代码。这对于从页面中提取数据或与页面元素进行交互非常有用。该函数应返回一个解析为包含评估结果的字符串的 `Promise`。

通过将这些选项传递给 `PuppeteerWebBaseLoader` 构造函数，您可以自定义加载器的行为，并利用 Puppeteer 强大的功能来抓取和交互网页。

## 屏幕截图

要截取网站的屏幕截图，像上面一样初始化加载器，然后调用 `.screenshot()` 方法。
这将返回一个 `Document` 实例，其中 `pageContent` 是一个 base64 编码的图像，`metadata` 包含一个带有页面 URL 的 `source` 字段。
...
有关 `PuppeteerWebBaseLoader` 所有功能和配置的详细文档，请查阅 [API 参考](https://api.js.langchain.com/classes/langchain_community_document_loaders_web_puppeteer.PuppeteerWebBaseLoader.html)。
