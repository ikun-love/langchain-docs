...
const callSubagent1 = tool(
  ...
  },
  {
    ...
    schema: z.object({
      query: z.string().describe("要发送给 subagent1 的查询。"),
    }),
  }
);
...
* **自定义输出格式化 (Custom output formatting)** – 在将子智能体的响应交还给主智能体之前，对其进行调整或丰富。
  ...
  * 这需要在代码中将结果包装在 [`Command`](https://reference.langchain.com/javascript/classes/_langchain_langgraph.index.Command.html)（或等效结构）中，以便可以将自定义状态与子智能体的响应合并。
