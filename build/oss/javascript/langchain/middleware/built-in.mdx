...
<Tip>
    如果使用 `langchain@1.1.0`，`trigger` 和 `keep` 的 `fraction` 条件依赖于聊天模型的 [模型配置文件](/oss/javascript/langchain/models#model-profiles)。如果数据不可用，请使用其他条件或手动指定：
    ```typescript
...
在工具调用执行之前，暂停 Agent 执行以供人工批准、编辑或拒绝工具调用。[人工干预](/oss/javascript/langchain/human-in-the-loop) 在以下场景中很有用：
...
<Warning>
    人工干预中间件需要一个 [检查点 (checkpointer)](/oss/javascript/langgraph/persistence#checkpoints) 来在中断期间维护状态。
</Warning>
...
<Tip>
    有关完整的示例、配置选项和集成模式，请参阅 [人工干预文档](/oss/javascript/langchain/human-in-the-loop)。
</Tip>
