...
[Graph API](/oss/javascript/langgraph/graph-api) 采用声明式方法，您需要定义节点（nodes）、边（edges）和共享状态（shared state）来创建可视化的图结构。
...
[Functional API](/oss/javascript/langgraph/functional-api) 采用命令式方法，将 LangGraph 特性集成到标准的程序化代码中。
