...
将一个或多个以下流式处理模式作为列表传递给 [`stream`](https://reference.langchain.com/javascript/classes/_langchain_langgraph.index.CompiledStateGraph.html#stream) 方法：
...
LangGraph 图公开了 [`stream`](https://reference.langchain.com/javascript/classes/_langchain_langgraph.pregel.Pregel.html#stream) 方法，该方法以迭代器的形式生成流式输出。
...
要在流式输出中包含来自[子图](/oss/javascript/langgraph/use-subgraphs)的输出，可以在父图的 `.stream()` 方法中设置 `subgraphs: true`。这将流式传输来自父图和任何子图的输出。
