...
本指南解释了使用子图（subgraphs）的机制。子图是一个被用作另一个图中的[节点](/oss/javascript/langraph/graph-api#nodes)的[图](/oss/javascript/langgraph/graph-api#graphs)。
...
* [将图添加为节点](#add-a-graph-as-a-node) — 子图直接作为父图中的一个节点添加，并与父图**共享[状态键](/oss/javascript/langgraph/graph-api#state)**
...
实现子图的一种简单方法是从另一个图的节点内部调用一个图。在这种情况下，子图可以拥有与父图**完全不同的模式**（不共享键）。例如，您可能希望在[多智能体](/oss/javascript/langchain/multi-agent)系统的每个智能体中维护私有的消息历史记录。
...
如果您希望子图**拥有自己的内存**，可以通过设置适当的检查点选项来编译它。这在[多智能体](/oss/javascript/langchain/multi-agent)系统中非常有用，如果您希望智能体跟踪其内部消息历史记录：
...
当您启用[持久化](/oss/javascript/langgraph/persistence)时，您可以通过适当的方法[检查图状态（检查点）](/oss/javascript/langgraph/persistence#checkpoints)。要查看子图状态，您可以使用 `subgraphs` 选项。
