---
title: 如何在同一线程中运行多个智能体 (Agent)
sidebarTitle: 在同一线程中运行多个智能体
---
在 LangSmith 部署中，一个线程并不会显式地绑定到特定的智能体。
这意味着你可以在**同一个线程**上运行多个智能体，这使得后续的智能体可以基于第一个智能体在线程中产生的进度（Checkpoints）继续工作。

在本示例中，我们将创建两个智能体，然后让它们在相同的线程上被调用。
你将看到第二个智能体会利用第一个智能体在该线程中通过[检查点 (checkpoint)](/oss/python/langsmith/deployment#checkpointer-state) 生成的信息作为上下文来进行响应。
...
<Tabs>
    <Tab title="Python">
    ```python
    from langsmith_sdk import get_client
    ...
    ```js
    import { Client } from "@langchain/langsmith-sdk";
...
## 在线程上运行智能体 (Run assistants on thread)

### 运行 OpenAI 智能体

我们现在首先在线程上运行 OpenAI 智能体。
...
### 运行默认智能体 (Run default assistant)

现在，我们可以在默认智能体上运行它，可以看到第二个智能体**了解**了初始问题，并且能够回答“and you?”这个问题：
