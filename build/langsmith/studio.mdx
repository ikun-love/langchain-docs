...
* 通过 [时间旅行（time travel）](/oss/python/langgraph/use-time-travel) 调试智能体状态
...
聊天模式（Chat mode）是用于迭代和测试特定于聊天的智能体的简化 UI。它对业务用户以及希望测试整体智能体行为的用户非常有用。聊天模式仅支持状态包含或扩展了 [`MessagesState`](/oss/python/langgraph/use-graph-api#messagesstate) 的图。
