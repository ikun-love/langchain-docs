...
本指南假定您已经了解什么是“双重文本（double-texting）”，相关的概念可以在[双重文本概念指南](/langsmith/double-texting)中学习。

本指南将介绍双重文本的 `enqueue` 选项，该选项会将中断操作加入一个队列，并按照客户端接收到的顺序执行它们。下面是使用 `enqueue` 选项的一个快速示例。

在 LanguSmith Agent Server 中创建运行实例时，`enqueue` 是默认的双重文本（多任务处理）策略。

## 设置

首先，我们将定义一个快速的辅助函数，用于打印 JavaScript 和 CURL 模型输出（如果使用 Python，则可以跳过此步骤）：
...
接下来，我们导入所需的包并实例化客户端（client）、助手（assistant）和线程（thread）。
...
现在让我们启动两个运行实例，第二个实例将使用 "enqueue" 的多任务策略来中断第一个实例：
