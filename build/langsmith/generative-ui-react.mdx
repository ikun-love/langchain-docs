...
生成式用户界面（Generative UI）允许智能体超越纯文本，生成丰富的用户界面。这使得我们可以创建更具交互性和上下文感知的应用程序，其中 UI 可以根据对话流程和 AI 响应动态调整。

![Agent Chat 显示一个关于预订/住宿的提示以及渲染的内联 UI 组件（图像、标题、价格、位置）组成的酒店列表卡片。](/langsmith/images/generative-ui-sample.jpg)

LangSmith 支持将您的 React 组件与图代码（graph code）放在一起。这使得您可以在构建特定 UI 组件的同时，轻松地将其集成到现有的聊天界面中，例如 [Agent Chat](https://agentchat.vercel.app)，并且仅在真正需要时才加载相关代码。
...
接下来，在您的 `langgraph.json` 配置文件中定义您的 UI 组件：
...
`ui` 部分指向将由图使用的 UI 组件。默认情况下，我们建议使用与图名称相同的键，但您可以根据需要拆分组件，更多详细信息请参见 [自定义 UI 组件的命名空间](#customise-the-namespace-of-ui-components)。

LangSmith 将自动打包您的 UI 组件代码和样式，并将它们作为外部资源提供，供 `LoadExternalComponent` 组件加载。像 `react` 和 `react-dom` 这样的某些依赖项将自动从打包中排除。

CSS 和 Tailwind 4.x 也开箱即用提供支持，因此您可以自由使用 Tailwind 类以及 `shadcn/ui` 组件。
...
在底层，`LoadExternalComponent` 会从 LangSmith 获取 UI 组件的 JS 和 CSS，并将其渲染到 Shadow DOM 中，从而确保样式与应用程序的其余部分隔离。

## 操作指南 (How-to guides)

### 在客户端提供自定义组件

如果您已经在客户端应用程序中加载了这些组件，您可以提供一个组件映射，以便直接渲染它们，而无需从 LangSmith 中获取 UI 代码。
...
您可以提供一个 fallback UI，以便在组件加载时进行渲染。
...
默认情况下，`LoadExternalComponent` 会使用 `useStream()` Hook 中的 `assistantId` 来获取 UI 组件的代码。您可以通过向 `LoadExternalComponent` 组件提供 `namespace` 属性来定制这一点。
...
您可以通过使用 `useStreamContext` Hook 在 UI 组件内部访问线程状态。
...
您可以通过向 `LoadExternalComponent` 组件提供 `meta` 属性来传递附加的上下文信息。

```tsx
<LoadExternalComponent stream={thread} message={ui} meta={{ userId: "123" }} />
```

然后，您可以通过使用 `useStreamContext` Hook 在 UI 组件中访问 `meta` 属性。
...
您可以通过使用 `useStream()` Hook 的 `onCustomEvent` 回调，在节点执行完成之前流式传输 UI 消息。当 LLM 正在生成响应时，这对于更新 UI 组件特别有用。
...
类似于可以通过附加 `RemoveMessage` 从状态中移除消息，您可以通过调用 `remove_ui_message` / `ui.delete` 并传入 UI 消息的 ID 来从状态中移除 UI 消息。
