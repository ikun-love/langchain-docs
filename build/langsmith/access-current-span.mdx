---
title: 访问被追踪函数中的当前运行（Span）
sidebarTitle: 访问被追踪函数中的当前运行（Span）
---

在某些情况下，您可能希望在被追踪函数（traced function）内部访问当前的运行（run，即 Span）。这对于从当前运行中提取 UUID、标签（tags）或其他信息非常有用。

您可以通过在 Python 或 TypeScript SDK 中分别调用 `get_current_run_tree`/`getCurrentRunTree` 函数来访问当前的运行。
