...
[Python](https://reference.langchain.com/python/langsmith/) 和 [TypeScript](https://reference.langchain.com/javascript/modules/langsmith.html) SDK 是在 LangSmith 中运行 [评估](/langsmith/evaluation-concepts) 的推荐方式。它们包含了增强性能和可靠性的优化和功能。

如果您无法使用 SDKs（例如，您正在使用其他语言或受限环境），您可以直接使用 REST API。本指南演示了如何使用 Python 的 [`requests`](https://requests.readthedocs.io/) 库通过 [REST API](https://api.smith.langchain.com/redoc) 运行评估，但同样的原则适用于任何语言。

在深入了解内容之前，阅读以下内容可能会有所帮助：
- [评估 LLM 应用](/langsmith/evaluate-llm-application)。
- [LangSmith API 参考](https://api.smith.langchain.com/redoc)：本指南中使用的所有端点的完整 API 文档。

## 创建数据集

在此示例中，我们使用 Python SDK 快速创建一个 [数据集](/langsmith/evaluation-concepts#datasets)。如需通过 API 或 UI 创建数据集，请参阅 [在应用中管理数据集](/langsmith/manage-datasets-in-application)。
...
1. 对于每个示例，创建引用示例和实验的运行记录。
1. 通过设置 `end_time` 来关闭实验。
...
接下来，定义一个函数，该函数将在单个示例上运行您的模型并将结果记录到 LangSmith。当直接使用 API 时，您需要负责：

- 通过 POST 请求到 `/runs` 创建运行对象，并设置 `reference_example_id` 和 `session_id`。
- 跟踪运行之间的父子关系（例如，包含子“llm”运行的父级“chain”运行）。
- 通过 PATCH 请求到 `/runs/{run_id}` 使用输出来更新运行记录。
...
def run_completion_on_example(example, model_name, experiment_id):
    ...
    def _post_run(run_id, name, run_type, inputs, parent_id=None):
        ...
        resp = requests.post(
            "https://api.smith.langchain.com/api/v1/runs", # 根据自托管安装或欧盟地区进行相应更新
            json=data,
            headers=headers
        )
    ...
    messages = [
        {
            "role": "system",
            "content": "请审查以下用户查询，确定其中是否包含任何形式的攻击性行为，如侮辱、威胁或高度负面评论。如果包含，请回复 'Toxic'；如果不包含，请回复 'Not toxic'。",
        },
...
现在创建实验并在所有示例上运行完成请求。在 API 中，“实验”表示为一个会话（或“示踪剂会话”），它通过 `reference_dataset_id` 引用一个数据集。与常规示踪（tracing）的主要区别在于：实验中的运行记录必须包含一个 `reference_example_id`，将每次运行链接到数据集中的特定示例。

```python
#  使用 /sessions 端点创建一个新实验
#  实验是运行的集合，通过 reference_dataset_id 引用数据集
...
for model_name in model_names:
    ...
    # 发出 patch 请求以通过更新 end_time 来“结束”实验
    requests.patch(
...
运行完 [实验](/langsmith/evaluation-concepts#experiment) 后，您通常需要通过添加反馈分数来评估结果。这使您能够跟踪正确性、准确性或任何自定义评估标准等指标。

在本例中，评估会检查每个模型的输出是否与数据集中的预期标签匹配。代码将“正确性”分数（正确为 1.0，不正确为 0.0）发布到运行记录中，以跟踪每个模型对有毒与无毒文本分类的准确性。

以下代码向 [单个实验示例](#run-a-single-experiment) 中的运行记录添加反馈：
...
您可以向不同的键添加多个反馈分数，以跟踪各种指标。例如，您可以同时添加“正确性”分数和“toxicity\_detected”分数。

## 运行配对实验

接下来，我们将演示如何运行配对实验。在配对实验中，您将两个示例彼此进行比较。

有关更多信息，请查看 [本指南](/langsmith/evaluate-pairwise)。
...
runs = requests.post(
    ...
    json={
        ...
        "is_root": True, # 仅获取根运行记录（跨度），其中包含最终输出
        "select": ["id", "reference_example_id", "outputs"],
    }
).json()
runs = runs["runs"]
...
for example_id, runs in example_id_to_runs_map.items():
    ...
    # 优先对输出进行排序，在本例中我们将始终优先选择第一个输出
    # 在实际应用中，您可以使用 LLM 来对输出进行排序
    feedback_group_id = uuid4()

    # 为每个运行记录发布一个反馈分数，其中第一个运行记录是首选的
    # API 参考: https://api.smith.langchain.com/redoc#tag/feedback/operation/create_feedback_api_v1_feedback_post
    # 我们使用 feedback group ID 将反馈分数与同一组关联起来
