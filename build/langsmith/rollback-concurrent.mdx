...
本指南假设您了解什么是双重执行（double-texting），您可以在[双重执行概念指南](/langsmith/double-texting)中了解相关信息。

本指南涵盖了双重执行（double texting）的 `rollback` 选项，该选项会中断图（graph）的先前的运行，并使用新的双重执行启动一次新的运行。此选项与 `interrupt` 选项非常相似，但在此情况下，第一次运行将从数据库中**完全删除**，并且无法重新启动。下面是使用 `rollback` 选项的快速示例。

## 设置 (Setup)

首先，我们将定义一个快速的辅助函数来打印 JS 和 CURL 模型输出（如果使用 Python，则可以跳过此部分）：
...
现在，让我们导入所需的包并实例化我们的客户端（client）、助手（assistant）和线程（thread）。
...
现在让我们使用 `multitask` 参数设置为 `"rollback"` 来运行一个线程：
