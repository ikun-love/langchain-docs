...
您将保留相同的 [`Auth`](https://reference.langchain.com/python/langsmith/deployment/sdk/#langgraph_sdk.auth.Auth) 对象和[资源级别访问控制](/langsmith/auth#single-owner-resources)，但会将身份验证升级为使用 Supabase 作为您的身份提供商。虽然本教程使用了 Supabase，但这些概念适用于任何 OAuth2 提供商。您将学习如何：
...
在前面的教程中，您使用了 [`Auth`](https://reference.langchain.com/python/langsmith/deployment/sdk/#langgraph_sdk.auth.Auth) 对象来[验证硬编码的令牌](/langsmith/set-up-custom-auth)并[添加资源所有权](/langsmith/resource-auth)。

现在，您将升级身份验证，以验证来自 Supabase 的真实 JWT 令牌。主要更改将全部集中在带有 [`@auth.authenticate`](https://reference.langchain.com/python/langsmith/deployment/sdk/#langgraph_sdk.auth.Auth.authenticate) 装饰器的函数中：
...
3. 在阅读 [参考文档](https://reference.langchain.com/python/langsmith/deployment/sdk/#langgraph_sdk.auth.Auth) 后，进一步定制您的处理程序和设置。
